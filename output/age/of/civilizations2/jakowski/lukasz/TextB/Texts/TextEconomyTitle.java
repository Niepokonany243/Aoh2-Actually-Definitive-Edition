...
public class TextEconomyTitle
extends Text {
    public TextEconomyTitle(String sText, int iTextPositionX, int iPosX, int iPosY, int iWidth, int iHeight) {
        super(sText, iTextPositionX, iPosX, iPosY, iWidth, iHeight, CFG.FONT_BOLD_SMALL);
    }

    @Override
    public void drawE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive, boolean scrollableY) {
        oSB.setColor(new Color(0.08f, 0.08f, 0.08f, 0.5f));
    ...
    public Color getColor(boolean isActive) {
        return isActive ? new Color(0.9f, 0.9f, 0.9f, 1.0f) : (this.getIsClickable() ? (this.getIsHovered() ? new Color(0.95f, 0.95f, 0.95f, 1.0f) : Color.WHITE) : new Color(0.78f, 0.78f, 0.78f, 0.7f));
    }
}
