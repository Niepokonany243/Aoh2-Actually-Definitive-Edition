...
public class TextOutliner_DiploInfo_Alliance
extends Text {
    ...
    public TextOutliner_DiploInfo_Alliance(int iCivLeft, int iCivRight, int iTurnID, int iPosX, int iPosY, int nWidth) {
        ...
        iCivRight = -1;
        if (CFG.core.getCiv(iCivLeft).getAlliance() > 0) {
            for (int k = 0; k < CFG.core.getAlliance(CFG.core.getCiv(iCivLeft).getAlliance()).getCivilizationsSize(); ++k) {
                if (CFG.core.getAlliance(CFG.core.getCiv(iCivLeft).getAlliance()).getCivilization(k) == iCivLeft) continue;
                iCivRight = CFG.core.getAlliance(CFG.core.getCiv(iCivLeft).getAlliance()).getCivilization(k);
                break;
            }
        }
        this.iCivRight = iCivRight > 0 ? (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getMetCiv(iCivRight) ? iCivRight : -iCivRight) : -1;
    ...
    public void buildElemHover() {
        ...
        nElements.add(new MEHover_2E(nData));
        nData.clear();
        nData.add(new ME_Hover_2Type_Text(CFG.lang.get("Alliance"), new Color(CFG.diplomacyColors_GameData.COLOR_DIPLOMACY_ALLIANCE.getR(), CFG.diplomacyColors_GameData.COLOR_DIPLOMACY_ALLIANCE.getG(), CFG.diplomacyColors_GameData.COLOR_DIPLOMACY_ALLIANCE.getB(), 1.0f)));
    ...
    public void actionElem(int iID) {
        int tWarID = CFG.core.getCiv(this.iCivLeft).getAlliance();
        if (tWarID > 0) {
            if (tWarID == Menu_InGame_Alliance.ALLIANCE_ID && CFG.menus.getVisible_InGame_Alliance()) {
                CFG.menus.setVisible_InGame_Alliance(false);
            } else {
                CFG.menus.rebuildInGame_Alliance(tWarID);
            }
        }
    }
}
