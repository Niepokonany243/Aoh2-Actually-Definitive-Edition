...
public class TextIcon_Religion
extends ButtonM {
    public int religionID = 0;
    public int iconWidth;
    public int iconHeight;

    public TextIcon_Religion(String sText, int religionID, int nPosX, int nPosY, int nWidth, int nHeight) {
        this.init(sText, 0, nPosX, nPosY, nWidth, nHeight, true, true, false, false);
        this.religionID = religionID;
        float iconScale = this.getImageScale(religionID);
        this.iconWidth = (int)((float)CFG.religionManager.religionImages.get(religionID).getWidth() * iconScale);
        this.iconHeight = (int)((float)CFG.religionManager.religionImages.get(religionID).getHeight() * iconScale);
    }

    public TextIcon_Religion(String sText, int religionID, int nPosX, int nPosY, int nWidth, int nHeight, int fontID) {
        this.fontID = fontID;
        this.init(sText, 0, nPosX, nPosY, nWidth, nHeight, true, true, false, false);
        this.religionID = religionID;
        float iconScale = this.getImageScale(religionID);
    ...
    public void drawTextE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
        ...
        oSB.setColor(TextIcon_Religion.getColor_gradientXY());
        ...
        oSB.setColor(TextIcon_Religion.getColor_gradientFull());
        ...
        CFG.religionManager.religionImages.get(this.religionID).draw(oSB, this.getPosXE() + this.getWidthE() / 2 - this.iconWidth / 2 + iTranslateX, this.getPosY() + (this.getHeightE() - this.getTextH()) / 2 - this.iconHeight / 2 + iTranslateY, this.iconWidth, this.iconHeight);
    ...
    public void setCurr(int nCurrent) {
        ...
        float iconScale = this.getImageScale(this.religionID);
        this.iconWidth = (int)((float)CFG.religionManager.religionImages.get(this.religionID).getWidth() * iconScale);
        this.iconHeight = (int)((float)CFG.religionManager.religionImages.get(this.religionID).getHeight() * iconScale);
    }
    ...
    private final float getImageScale(int iImageID) {
        return Math.min(1.0f, (float)(this.getHeightE() - this.getTextH() - CFG.PADD * 2) / (float)CFG.religionManager.religionImages.get(iImageID).getHeight());
    }
