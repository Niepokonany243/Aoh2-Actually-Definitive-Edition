...
public class TextActionInfo_RecruitInstantly
extends TextActionInfo {
    public TextActionInfo_RecruitInstantly(String sText, int iPosX, int iPosY) {
    ...
    public void drawE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive, boolean scrollableY) {
        ...
        IMGManager.getIMG(Images.diploArmy).draw(oSB, this.getPosXE() + CFG.PADD + iTranslateX, this.getPosY() + (int)(((float)this.getHeightE() - (float)IMGManager.getIMG(Images.diploArmy).getHeight() * this.getImageScale()) / 2.0f) + iTranslateY, (int)((float)IMGManager.getIMG(Images.diploArmy).getWidth() * this.getImageScale()), (int)((float)IMGManager.getIMG(Images.diploArmy).getHeight() * this.getImageScale()));
        CFG.fontMain.get(0).getData().setScale(0.8f);
        CFG.drawTextDefaultWithShadow(oSB, this.sText, this.getPosXE() + (int)((float)IMGManager.getIMG(Images.diploArmy).getWidth() * this.getImageScale()) + CFG.PADD * 2 + iTranslateX, this.getPosY() + (int)(((float)this.getHeightE() - (float)this.iTextHeight * 0.8f) / 2.0f) + iTranslateY, this.getColor(isActive));
        CFG.fontMain.get(0).getData().setScale(1.0f);
    }

    private final float getImageScale() {
        return (float)CFG.TEXT_HEIGHT_DEFAULT / (float)IMGManager.getIMG(Images.diploArmy).getHeight();
    }

    @Override
    public int getWidthE() {
        return super.getWidthE() + (int)((float)IMGManager.getIMG(Images.diploArmy).getWidth() * this.getImageScale()) + CFG.PADD * 2;
    }
