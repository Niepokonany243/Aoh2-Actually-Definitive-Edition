...
public class Event_Conditions_HaveCore
extends Event_Conditions {
    ...
    public boolean outCondition() {
        ...
        try {
            for (int i = 0; i < this.lProvinces.size(); ++i) {
                if (!CFG.core.getProv(this.lProvinces.get(i)).getCores().getHaveACore(this.getCivID())) continue;
                ++numOut;
            }
        }
    ...
    public String getConditionText() {
        try {
            return CFG.lang.get("HaveACore") + ": " + CFG.core.getCiv(this.getCivID()).getCivName();
        }
        catch (IndexOutOfBoundsException ex) {
            return CFG.lang.get("HaveACore");
        }
    }
