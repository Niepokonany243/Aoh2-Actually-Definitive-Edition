...
public class Event_Outcome_UpdateHappiness
extends Event_Outcome {
    private static final long serialVersionUID = 5800711357527950155L;
    public int iCivID = -1;
    ...
    public void outcomeAction() {
        if (this.getCivID() >= 0 && this.getCivID() < CFG.core.getCivsSize()) {
            for (int i = 0; i < this.getProvinces().size(); ++i) {
                ...
                CFG.core.getProv(this.getProvinces().get(i)).setHappi(CFG.core.getProv(this.getProvinces().get(i)).getHappi() + (float)this.getValue() / 100.0f);
            }
        }
    }
    ...
    public String getConditionText() {
        try {
            return CFG.lang.get("UpdateHappiness") + ": " + CFG.core.getCiv(this.getCivID()).getCivName() + ", " + (this.getValue() > 0 ? "+" : "") + this.getValue();
        }
        catch (IndexOutOfBoundsException ex) {
            return CFG.lang.get("UpdateHappiness");
        }
    }
