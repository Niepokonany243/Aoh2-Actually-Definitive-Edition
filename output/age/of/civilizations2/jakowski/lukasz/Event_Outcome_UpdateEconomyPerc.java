...
public class Event_Outcome_UpdateEconomyPerc
extends Event_Outcome {
    private static final long serialVersionUID = -3794590385449029264L;
    public int iCivID = -1;
    ...
    public void outcomeAction() {
        if (this.getCivID() >= 0 && this.getCivID() < CFG.core.getCivsSize()) {
            for (int i = 0; i < this.getProvinces().size(); ++i) {
                ...
                CFG.core.getProv(this.getProvinces().get(i)).setEco(CFG.core.getProv(this.getProvinces().get(i)).getEco() + (int)((float)CFG.core.getProv(this.getProvinces().get(i)).getEco() * ((float)this.getValue() / 100.0f)));
            }
        }
    }
    ...
    public String getConditionText() {
        try {
            return CFG.lang.get("UpdateEconomyPercentage") + ": " + CFG.core.getCiv(this.getCivID()).getCivName() + ", " + this.getValue();
        }
        catch (IndexOutOfBoundsException ex) {
            return CFG.lang.get("UpdateEconomyPercentage");
        }
    }
