...
import age.of.civilizations2.jakowski.lukasz.RegroupArmy.RegroupArmy_ExpandCheck;

public class CivArmyMission_ExpandNeutral_Check
extends CivArmyMission {
    public int iTurnsRequiredToMoveToFrontLine = 1;

    public CivArmyMission_ExpandNeutral_Check(int nCivID, int fromProvinceID, int toProvinceID, long iArmy) {
        this.iArmy = iArmy;
    ...
    public boolean action(int nCivID) {
        RegroupArmy_ExpandCheck tryRegroupArmy;
        if (this.iProvinceID != this.toProvinceID && (tryRegroupArmy = new RegroupArmy_ExpandCheck(nCivID, this.iProvinceID, this.toProvinceID)).getRouteSize() > 0) {
