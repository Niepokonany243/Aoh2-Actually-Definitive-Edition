...
public class Event_Conditions_OccupyProvinces
extends Event_Conditions {
    private static final long serialVersionUID = -7077789239154881933L;
    public int iCivID = -1;
    ...
    public boolean outCondition() {
        ...
        try {
            for (int i = 0; i < this.lProvinces.size(); ++i) {
                if (CFG.core.getProv(this.lProvinces.get(i)).getCivId() != this.getCivID() || !CFG.core.getProv(this.lProvinces.get(i)).isOccupied()) continue;
        ...
        return !(numOfControlledProvinces / (float)this.lProvinces.size() <= (float)this.getValue() / 100.0f);
    }

    @Override
    public String getConditionText() {
        try {
            return CFG.lang.get("OccupiedProvinces") + ": " + CFG.core.getCiv(this.getCivID()).getCivName();
        }
        catch (IndexOutOfBoundsException ex) {
            return CFG.lang.get("OccupiedProvinces");
        }
    }
