...
public class Event_Outcome_JoinUnion
extends Event_Outcome {
    private static final long serialVersionUID = 3302798704622558635L;
    public int iCivID = -1;
    ...
    public void outcomeAction() {
        if (this.canMakeAction()) {
            CFG.createUnionCivs(this.getCivID(), this.getCivID2());
        }
    }

    public boolean canMakeAction() {
        try {
            return this.getCivID() >= 0 && this.getCivID() < CFG.core.getCivsSize() && this.getCivID2() >= 0 && this.getCivID2() < CFG.core.getCivsSize() && this.getCivID() != this.getCivID2();
        }
    ...
    public List<MEHover_2E> getHoverText() {
        try {
            ...
            if (this.canMakeAction()) {
                tData.add(new ME_Hover_2Type_Text(CFG.lang.get("Union") + ":", CFG.COLOR_HOVER_TITLE));
    ...
    public String getConditionText() {
        try {
            return CFG.lang.get("CreateUnion") + ": " + CFG.core.getCiv(this.getCivID()).getCivName() + ", " + CFG.core.getCiv(this.getCivID2()).getCivName();
        }
        catch (IndexOutOfBoundsException ex) {
            return CFG.lang.get("CreateUnion");
        }
    }
