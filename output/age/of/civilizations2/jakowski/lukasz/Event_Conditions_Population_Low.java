...
public class Event_Conditions_Population_Low
extends Event_Conditions {
    ...
    public boolean outCondition() {
        try {
            ...
            for (int i = 0; i < CFG.core.getCiv(this.getCivID()).getNumOfProvs(); ++i) {
                tNum += CFG.core.getProv(CFG.core.getCiv(this.getCivID()).getProvID(i)).getPop().getPops();
            }
    ...
    public String getConditionText() {
        try {
            return CFG.lang.get("Population") + " < " + this.getValue() + ", " + CFG.core.getCiv(this.getCivID()).getCivName();
        }
        catch (IndexOutOfBoundsException ex) {
