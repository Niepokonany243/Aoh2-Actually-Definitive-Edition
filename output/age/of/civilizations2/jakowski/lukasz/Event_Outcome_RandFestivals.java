...
public class Event_Outcome_RandFestivals
extends Event_Outcome {
        ...
        try {
            if (this.canMakeAction()) {
                for (int i = 0; i < this.getValue(); ++i) {
                    int randID = CFG.core.getCiv(this.getCivID()).getProvID(CFG.oR.nextInt(CFG.core.getCiv(this.getCivID()).getNumOfProvs()));
                    Festival.addFestivalFree(this.getCivID(), randID);
                }
    ...
    public String getConditionText() {
        try {
            return CFG.lang.get("Festival") + " -> " + CFG.lang.get("Random") + ": " + CFG.core.getCiv(this.getCivID()).getCivName() + ", " + (this.getValue() > 0 ? "+" : "") + this.getValue();
        }
        catch (IndexOutOfBoundsException ex) {
            return CFG.lang.get("Festival") + " -> " + CFG.lang.get("Random");
        }
    }
    ...
    public List<MEHover_2E> getHoverText() {
        try {
            ...
            if (this.canMakeAction()) {
                ...
                tData.add(new ME_Hover_2Type_Text(CFG.lang.get("Festival") + ": ", CFG.COLOR_HOVER_TITLE));
