...
public class Button_TodayPartOf
extends ButtonM {
    private int iCivID;
    ...
    public Button_TodayPartOf(int nCivID, String nPerc, int iPosX, int iPosY, int iWidth, int iHeight, boolean isClickable) {
        this.fontID = CFG.FONT_BOLD_SMALL;
        super.init("" + (nCivID < 0 ? CFG.lang.get("Undiscovered") : CFG.core.getCiv(nCivID).getCivName()), 0, iPosX, iPosY, iWidth, iHeight, isClickable, true, false, false, null);
        this.iCivID = nCivID;
        this.sPerc = "[" + nPerc + "%]";
    ...
    public void drawButtonBGE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
        ...
        Core.drawFlagRect(oSB, this.getPosXE() - CFG.PADD * 2 - IMGManager.getIMG(Images.flagRect2).getWidth() + this.getWidthE() + iTranslateX, this.getPosY() + this.getHeightE() / 2 - IMGManager.getIMG(Images.flagRect2).getHeight() / 2 + iTranslateY, this.iCivID);
    }

    @Override
    public void drawTextE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
        Renderer.drawTextWithShadow(oSB, this.fontID, this.sText, this.getPosXE() + this.getWidthE() - CFG.PADD * 2 - IMGManager.getIMG(Images.flagRect2).getWidth() - CFG.PADD - this.getTextWidthU() + iTranslateX, this.getPosY() + this.getHeightE() / 2 - this.getTextHeight() / 2 + iTranslateY, this.getColorE(isActive));
        Renderer.drawTextWithShadow(oSB, this.fontID, this.sPerc, this.getPosXE() + this.getWidthE() - CFG.PADD * 2 - IMGManager.getIMG(Images.flagRect2).getWidth() - CFG.PADD - this.getTextWidthU() - CFG.PADD - this.iPercWidth + iTranslateX, this.getPosY() + this.getHeightE() / 2 - this.getTextHeight() / 2 + iTranslateY, new Color(0.56f, 0.56f, 0.56f, 1.0f));
    }
    ...
    public void buildElemHover() {
        try {
            ...
            for (int i = 0; i < CFG.formableCivs_GameData.getProvincesSize(); ++i) {
                if (CFG.core.getProv(CFG.formableCivs_GameData.getProvinceID(i)).getCivId() != this.iCivID) continue;
                ++tempProvinces;
            }
            ...
            nData.add(new ME_Hover_2Type_Text("" + tempProvinces, CFG.COLOR_TEXT_NUM_OF_PROVINCES));
            nData.add(new ME_Hover_2Type_Text(" " + this.sPerc, CFG.COLOR_NEUTRAL));
