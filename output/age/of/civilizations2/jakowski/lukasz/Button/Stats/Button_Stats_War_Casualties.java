...
public class Button_Stats_War_Casualties
extends ButtonStats {
    private long iCasualties;

    public Button_Stats_War_Casualties(long iCasualties, int iPosX, int iPosY, int iWidth) {
        super("" + CFG.getNumber_SHORT(iCasualties), 0, iPosX, iPosY, iWidth, Math.max(CFG.BUTTON_H, CFG.TEXT_HEIGHT_DEFAULT * 2 + CFG.PADD * 4));
        this.iCasualties = iCasualties;
    }

    public Button_Stats_War_Casualties(int iCasualties, int iPosX, int iPosY, int iWidth) {
        this((long)iCasualties, iPosX, iPosY, iWidth);
    }
    ...
    public void drawTextE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
        Renderer.drawTextWithShadow(oSB, this.fontID, this.getTextE(), this.getPosXE() + (this.getWidthE() - this.getTextWidthU()) / 2 + iTranslateX, this.getPosY() + (int)((float)(this.getHeightE() - CFG.TEXT_HEIGHT_DEFAULT) / 2.0f) + iTranslateY, this.getColorE(isActive));
    }
    ...
    public void buildElemHover() {
        ...
        nData.add(new ME_Hover_2Type_Text("" + CFG.getNumberWthSpaces("" + this.iCasualties), CFG.COLOR_NEGATIVE_2));
    ...
    public Color getColorE(boolean isActive) {
        return isActive ? CFG.COLOR_NEGATIVE_ACTIVE : (this.getIsHovered() ? CFG.COLOR_NEGATIVE_HOVER : CFG.COLOR_NEUTRAL);
    }
}
