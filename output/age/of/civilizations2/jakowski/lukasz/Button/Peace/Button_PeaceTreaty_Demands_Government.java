...
public class Button_PeaceTreaty_Demands_Government
extends ButtonM {
    public int iCivID = 0;
    ...
    public Button_PeaceTreaty_Demands_Government(int nCivID, int toPayCivID, int iPosX, int iPosY, int iWidth, int iHeight, boolean isClickable) {
        this.fontID = CFG.FONT_BOLD_SMALL;
        super.init(CFG.lang.get("ChangeTypeOfGovernment"), 0, iPosX, iPosY, iWidth, iHeight, isClickable, true, false, false);
        this.iCivID = nCivID;
        this.toPayCivID = toPayCivID;
        this.sValue = "" + CFG.peaceTreatyData.getGovernment_Cost(toPayCivID);
    ...
    public void drawTextE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
        ...
        try {
            if (this.iCivID > 0) {
                ...
                IMGManager.getIMG(Images.topDiplomacyPoints).drawO(oSB, this.getPosXE() + MAX_WDITH_LEFT / 2 - (int)((float)IMGManager.getIMG(Images.topDiplomacyPoints).getWidth() * this.getImageScale(IMGManager.getIMG(Images.topDiplomacyPoints).getHeight())) / 2 + iTranslateX, this.getPosY() + 1 + this.getHeightE() / 2 - (int)((float)IMGManager.getIMG(Images.topDiplomacyPoints).getHeight() * this.getImageScale(IMGManager.getIMG(Images.topDiplomacyPoints).getHeight())) / 2 - IMGManager.getIMG(Images.topDiplomacyPoints).getHeight() + iTranslateY, (int)((float)IMGManager.getIMG(Images.topDiplomacyPoints).getWidth() * this.getImageScale(IMGManager.getIMG(Images.topDiplomacyPoints).getHeight())), (int)((float)IMGManager.getIMG(Images.topDiplomacyPoints).getHeight() * this.getImageScale(IMGManager.getIMG(Images.topDiplomacyPoints).getHeight())));
            }
        }
    ...
    public void buildElemHover() {
        ...
        nData.add(new ME_Hover_2Type_Flag_Big(this.toPayCivID, CFG.PADD, 0));
    ...
    public void actionElem(int iID) {
        this.iCivID = CFG.peaceTreatyData.takeGovernment(this.toPayCivID, CFG.peaceTreatyData.brushCivID, this.iCivID);
        if (CFG.menus.getInGame_PeaceTreaty()) {
            CFG.menus.rebuildInGame_PeaceTreaty_Scores();
        }
        CFG.toastM.addM(CFG.lang.get("ChangeTypeOfGovernment"), CFG.COLOR_TEXT_NUM_OF_PROVINCES);
