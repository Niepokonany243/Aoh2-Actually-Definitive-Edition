...
extends Button_Classic {
    ...
    public Button_FlagAction_GraphModes(String sText, int nModeID, int iPosX, int iPosY, int iWidth, int iHeight, boolean isClickable) {
        super(sText, -1, iPosX, iPosY, iWidth, iHeight, isClickable, CFG.FONT_BOLD_SMALL);
        this.iModeID = nModeID;
    }

    @Override
    public void drawButtonBGE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
        oSB.setColor(new Color(CFG.COLOR_BOX_GRADIENT.r, CFG.COLOR_BOX_GRADIENT.g, CFG.COLOR_BOX_GRADIENT.b, 0.125f));
        ...
        if (isActive || this.getIsHovered() || this.iModeID == Menu_InGame_GraphManager.iActiveGraphID) {
            ...
            IMGManager.getIMG(Images.line32Off1).drawO(oSB, this.getPosXE() + (this.getWidthE() - this.getTextWidthU()) / 2 + iTranslateX, this.getPosY() + (this.getHeightE() + this.getTextHeight()) / 2 + CFG.PADD + iTranslateY - IMGManager.getIMG(Images.line32Off1).getHeight(), this.getTextWidthU(), 1);
        } else {
        ...
        IMGManager.getIMG(Images.line32Vertical).drawO(oSB, this.getPosXE() + this.getWidthE() - 1 + iTranslateX, this.getPosY() - IMGManager.getIMG(Images.line32Vertical).getHeight() + iTranslateY, 1, this.getHeightE(), false, false);
    ...
    public void drawTextE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
        if (isActive) {
            Renderer.drawText(oSB, this.fontID, this.getTextToDrawElem(), this.getPosXE() + this.getWidthE() / 2 - this.getTextWidthU() / 2 + iTranslateX, this.getPosY() + (this.getHeightE() - this.getTextHeight()) / 2 + iTranslateY, this.getColorE(isActive));
        } else {
            Renderer.drawTextWithShadow(oSB, this.fontID, this.getTextToDrawElem(), this.getPosXE() + this.getWidthE() / 2 - this.getTextWidthU() / 2 + iTranslateX, this.getPosY() + (this.getHeightE() - this.getTextHeight()) / 2 + iTranslateY, this.getColorE(isActive));
        }
    }

    @Override
    public Color getColorE(boolean isActive) {
        return isActive ? CFG.COLOR_HOVER_TITLE : (this.getIsClickable() ? (this.getIsHovered() ? CFG.COLOR_BUTTON_GAME_TEXT_HOVERED : (Menu_InGame_GraphManager.iActiveGraphID == this.iModeID ? CFG.COLOR_BUTTON_GAME_TEXT_HOVERED : CFG.COLOR_BUTTON_GAME_TEXT)) : CFG.COLOR_BUTTON_GAME_TEXT_NOT_CLICKABLE);
    }
    ...
    public void buildElemHover() {
        ...
        nData.add(new ME_Hover_2Type_Text(CFG.lang.get("GraphMode") + ": "));
