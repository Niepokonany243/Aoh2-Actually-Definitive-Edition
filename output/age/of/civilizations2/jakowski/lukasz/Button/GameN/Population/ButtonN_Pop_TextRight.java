...
public class ButtonN_Pop_TextRight
extends ButtonN_Pop {
    public String sTime;
    public int iTimeWidth;
    public int imageRight = 0;

    public ButtonN_Pop_TextRight(Color nColor, String sText, int nCivID, String sTextLeft, String nPop, int iImageID, Color textColor, int iPosX, int iPosY, int iWidth, String textRight, int imageRight) {
    ...
    public void drawButtonBGE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
        ...
        Renderer.drawTextWithShadow(oSB, this.fontID2, this.sTime, this.getPosXE() + this.getWidthE() - CFG.PADD * 2 - this.iTimeWidth - (int)((float)IMGManager.getIMG(this.imageRight).getWidth() * this.getImageScale(this.imageRight, 1.0f)) + iTranslateX, this.getPosY() + this.getHeightE() / 2 - CFG.TEXT_HEIGHT_DEFAULT_SMALL / 2 + iTranslateY, this.getColorRight());
    }
