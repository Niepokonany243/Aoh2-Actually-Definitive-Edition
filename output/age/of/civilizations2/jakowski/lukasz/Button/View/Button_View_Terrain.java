...
public class Button_View_Terrain
extends ButtonM {
    ...
    public Button_View_Terrain(int iRow, int nTerrainID, int nNum, int iPosX, int iPosY, int iWidth) {
        this.fontID = CFG.FONT_BOLD_SMALL;
        super.init(CFG.terrainTypesManager.getName(nTerrainID), 0, iPosX, iPosY, iWidth, Math.max(CFG.terrainTypesManager.getIcon(0).getHeight() + CFG.PADD * 2, Menu_InGame_View_Army.getButtonHeight()), true, true, false, false);
        this.row = iRow % 2 == 0;
        this.iTerrainID = nTerrainID;
        this.sPopulation = "" + nNum;
        CFG.glyphLay.setText(CFG.fontMain.get(CFG.FONT_REGULAR_SMALL), "" + this.sPopulation);
    ...
    public void drawTextE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
        oSB.setColor(new Color(CFG.terrainTypesManager.getColor((int)this.iTerrainID).r, CFG.terrainTypesManager.getColor((int)this.iTerrainID).g, CFG.terrainTypesManager.getColor((int)this.iTerrainID).b, 1.0f));
        IMGManager.getIMG(Images.pix255).drawO(oSB, this.getPosXE() + CFG.PADD * 2 + iTranslateX, this.getPosY() + this.getHeightE() / 2 - CFG.terrainTypesManager.getIcon(this.iTerrainID).getHeight() / 2 - IMGManager.getIMG(Images.pix255).getHeight() + iTranslateY, CFG.CIV_COLOR_W, CFG.terrainTypesManager.getIcon(this.iTerrainID).getHeight());
        oSB.setColor(Color.WHITE);
        CFG.terrainTypesManager.getIcon(this.iTerrainID).drawO(oSB, this.getPosXE() + CFG.PADD * 2 + CFG.CIV_COLOR_W + iTranslateX, this.getPosY() + this.getHeightE() / 2 - CFG.terrainTypesManager.getIcon(this.iTerrainID).getHeight() / 2 + iTranslateY);
        Renderer.drawText(oSB, CFG.FONT_BOLD_SMALL, this.getTextE(), this.getPosXE() + CFG.PADD * 3 + CFG.terrainTypesManager.getIcon(this.iTerrainID).getWidth() + CFG.CIV_COLOR_W + iTranslateX, this.getPosY() + this.getHeightE() / 2 - this.getTextHeight() / 2 + iTranslateY, this.getColorE(isActive));
        Renderer.drawText(oSB, CFG.FONT_REGULAR_SMALL, this.sPopulation, this.getPosXE() + this.getWidthE() - CFG.PADD * 2 - this.iPopulationWidth + iTranslateX, this.getPosY() + this.getHeightE() / 2 - this.getTextHeight() / 2 + iTranslateY, CFG.COLOR_TEXT_GRAY_NS_ACTIVE);
    }
    ...
    public void buildElemHover() {
        this.menuElemHover = CFG.core.getHover_TerrainTypeInfo(this.iTerrainID, 0);
    }

    @Override
    public void actionElem(int iID) {
        CFG.toastM.addM(CFG.lang.get("Provinces") + ": " + this.sPopulation, CFG.COLOR_HOVER_TITLE);
    }
}
