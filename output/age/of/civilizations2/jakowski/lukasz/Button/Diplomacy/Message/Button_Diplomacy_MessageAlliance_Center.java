...
public class Button_Diplomacy_MessageAlliance_Center
extends ButtonStats {
    private int iCivA;

    public Button_Diplomacy_MessageAlliance_Center(String sText, int nAggressor, int iPosX, int iPosY, int iWidth) {
        super(sText, 0, iPosX, iPosY, iWidth, Math.max(CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 4, CFG.PADD * 4 + IMGManager.getIMG(Images.flagRect2).getHeight()));
    ...
    public void drawTextE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
        oSB.setColor(Color.WHITE);
        Core.drawFlagRect(oSB, this.getPosXE() + this.getWidthE() / 2 - (IMGManager.getIMG(Images.flagRect2).getWidth() + CFG.PADD + this.getTextWidthU()) / 2 + iTranslateX, this.getPosY() + this.getHeightE() / 2 - IMGManager.getIMG(Images.flagRect2).getHeight() / 2 + iTranslateY, this.iCivA);
        CFG.drawTextDefaultWithShadow(oSB, this.getTextE(), this.getPosXE() + this.getWidthE() / 2 - (IMGManager.getIMG(Images.flagRect2).getWidth() + CFG.PADD + this.getTextWidthU()) / 2 + IMGManager.getIMG(Images.flagRect2).getWidth() + CFG.PADD + iTranslateX, this.getPosY() + CFG.PADD * 2 + (int)((float)this.getTextHeight() / 2.0f) + iTranslateY, this.getColorE(isActive));
        oSB.setColor(Color.WHITE);
    }
