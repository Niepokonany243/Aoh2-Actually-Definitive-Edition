...
public class AI_Skills_Recruitable
extends AI_Skills {
    public AI_Skills_Recruitable(int iPoints, int iPointsMax) {
        super(iPoints, iPointsMax);
    }

    @Override
    public void addPoint_CivID(int nCivID) {
        SkillsManager.add_Recruitable(nCivID);
        this.iPoints = CFG.core.getCiv((int)nCivID).civGD.techPoints.POINTS_RECRUITABLE;
    }

    @Override
    public float getScore_Personality(int nCivID) {
        return CFG.core.getCiv((int)nCivID).TECH_RECRUITABLE;
    }
}
