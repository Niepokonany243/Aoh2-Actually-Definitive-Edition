...
extends AI_Build {
    ...
    public AI_Build_Armoury(int nCivID, long nMoney) {
        ...
        try {
            int i;
            for (i = 0; i < BuildingsManager.getArmoury_MaxLevel(); ++i) {
                this.lBuildCost.add(BuildingsManager.getArmoury_BuildCost(i + 1, CFG.core.getCiv(nCivID).getProvID(0)));
            ...
            if (nMoney >= (long)this.lBuildCost.get(0).intValue()) {
                for (i = 0; i < CFG.core.getCiv(nCivID).getNumOfProvs(); ++i) {
                    if (CFG.core.getProv(CFG.core.getCiv(nCivID).getProvID(i)).isOccupied() || !(CFG.core.getProv(CFG.core.getCiv(nCivID).getProvID(i)).getProviStability() > GameValues.gvAiProvince.BUILD_MIN_STABILITY) || !(CFG.core.getProv(CFG.core.getCiv(nCivID).getProvID(i)).getRevRisk() <= GameValues.gvAiProvince.BUILD_MAX_REV_RISK) || !BuildingsManager.canBuildArmoury(CFG.core.getCiv(nCivID).getProvID(i)) || CFG.core.getCiv(nCivID).isInConstruction(CFG.core.getCiv(nCivID).getProvID(i), ConstructionType.ARMOURY) != 0) continue;
                    try {
                        if (nMoney < (long)this.lBuildCost.get(CFG.core.getProv(CFG.core.getCiv(nCivID).getProvID(i)).getLvlOfArmoury()).intValue()) continue;
                        ((List)this.lProvincesToBuild.get(CFG.core.getProv(CFG.core.getCiv(nCivID).getProvID(i)).getLvlOfArmoury())).add(CFG.core.getCiv(nCivID).getProvID(i));
    ...
    public boolean build(int nCivID, int iteration, boolean out) {
        ...
        for (i = this.lProvincesToBuild.size() - 1; i >= 0; --i) {
            for (j = ((List)this.lProvincesToBuild.get(i)).size() - 1; j >= 0; --j) {
                if (CFG.gameAction.gMARY((Integer)((List)this.lProvincesToBuild.get(i)).get(j), nCivID) <= maxRecruitablePopulation) continue;
                maxRecruitablePopulation = CFG.gameAction.gMARY((Integer)((List)this.lProvincesToBuild.get(i)).get(j), nCivID);
            }
        }
        for (i = this.lProvincesToBuild.size() - 1; i >= 0; --i) {
            for (j = ((List)this.lProvincesToBuild.get(i)).size() - 1; j >= 0; --j) {
                ...
                if (!(this.getProvinceBuildScore(nCivID, (Integer)((List)this.lProvincesToBuild.get(i)).get(j), maxRecruitablePopulation) > iBestProvinceID_Score)) continue;
                iBestProvinceID = (Integer)((List)this.lProvincesToBuild.get(i)).get(j);
                iBestProvinceID_Score = this.getProvinceBuildScore(nCivID, iBestProvinceID, maxRecruitablePopulation);
            }
        }
