...
import age.of.civilizations2.jakowski.lukasz.AI.AI_Build.AI_Build_Fort;
import age.of.civilizations2.jakowski.lukasz.AI.AI_Build.AI_Build_Option;
import age.of.civilizations2.jakowski.lukasz.CFG;
import age.of.civilizations2.jakowski.lukasz.MapA.BuildingsManager;

public class AI_Build_Option_Fort
extends AI_Build_Option {
    @Override
    public float getScore(int nCivID) {
        return CFG.core.getCiv((int)nCivID).civGD.civPers.BUILD_FORT * (1.0f - (float)CFG.core.getCiv((int)nCivID).numOf_Forts / (float)Math.max(CFG.core.getCiv(nCivID).getNumOfProvs() * BuildingsManager.getFort_MaxLevel(), 1));
    }
