...
public class AI_Build_Library
extends AI_Build {
    public List<Integer> lBuildCost = new ArrayList<Integer>();

    public AI_Build_Library(int nCivID, long nMoney) {
        super(nCivID, nMoney);
        try {
            int i;
            for (i = 0; i < BuildingsManager.getLibrary_MaxLevel(); ++i) {
                this.lBuildCost.add(BuildingsManager.getLibrary_BuildCost(i + 1, CFG.core.getCiv(nCivID).getProvID(0)));
            ...
            if (nMoney >= (long)this.lBuildCost.get(0).intValue()) {
                for (i = 0; i < CFG.core.getCiv(nCivID).getNumOfProvs(); ++i) {
                    if (CFG.core.getProv(CFG.core.getCiv(nCivID).getProvID(i)).isOccupied() || !(CFG.core.getProv(CFG.core.getCiv(nCivID).getProvID(i)).getProviStability() > GameValues.gvAiProvince.BUILD_MIN_STABILITY) || !(CFG.core.getProv(CFG.core.getCiv(nCivID).getProvID(i)).getRevRisk() <= GameValues.gvAiProvince.BUILD_MAX_REV_RISK) || !BuildingsManager.canBuildLibrary(CFG.core.getCiv(nCivID).getProvID(i)) || CFG.core.getCiv(nCivID).isInConstruction(CFG.core.getCiv(nCivID).getProvID(i), ConstructionType.LIBRARY) != 0) continue;
                    try {
                        if (nMoney < (long)this.lBuildCost.get(CFG.core.getProv(CFG.core.getCiv(nCivID).getProvID(i)).getLvlOfLibrary()).intValue()) continue;
                        ((List)this.lProvincesToBuild.get(CFG.core.getProv(CFG.core.getCiv(nCivID).getProvID(i)).getLvlOfLibrary())).add(CFG.core.getCiv(nCivID).getProvID(i));
    ...
    public boolean build(int nCivID, int iteration, boolean out) {
        ...
        if (iBestProvinceID >= 0 && BuildingsManager.constructLibrary(iBestProvinceID, nCivID)) {
            out = true;
            if (this.getMoney(nCivID) > (long)this.lBuildCost.get(0).intValue() && BuildingsManager.getLibrary_BuildMovementCost(1) <= CFG.core.getCiv(nCivID).getMovemPoints()) {
                int tSize = 0;
                for (int i = this.lProvincesToBuild.size() - 1; i >= 0; --i) {
                    for (int j = ((List)this.lProvincesToBuild.get(i)).size() - 1; j >= 0; --j) {
