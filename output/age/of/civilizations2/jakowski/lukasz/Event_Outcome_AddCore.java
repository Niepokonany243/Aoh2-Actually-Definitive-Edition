...
extends Event_Outcome {
    private static final long serialVersionUID = 146076810864068222L;
    public int iCivID = -1;
        ...
        if (this.getCivID() >= 0 && this.getCivID() < CFG.core.getCivsSize()) {
            for (int i = 0; i < this.lProvinces.size(); ++i) {
                try {
                    ...
                    CFG.core.getProv(this.lProvinces.get(i)).getCores().addNewCore(this.getCivID(), GameCalendar.TURNID);
                    continue;
                }
        ...
        try {
            ...
            if (this.getProvinces().size() < 9) {
                for (int i = 0; i < this.getProvinces().size(); ++i) {
                    ...
                    tData.add(new ME_Hover_2Type_Text(CFG.lang.get("FabricateClaim") + ": ", CFG.COLOR_HOVER_TITLE));
    ...
    public String getConditionText() {
        try {
            return CFG.lang.get("AddCore") + ": " + CFG.core.getCiv(this.getCivID()).getCivName();
        }
        catch (IndexOutOfBoundsException ex) {
            return CFG.lang.get("AddCore");
        }
    }
