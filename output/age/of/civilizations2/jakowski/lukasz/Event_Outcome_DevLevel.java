...
public class Event_Outcome_DevLevel
extends Event_Outcome {
    private static final long serialVersionUID = -9209808499658420265L;
    public int iCivID = -1;
    ...
    public void outcomeAction() {
        if (this.getCivID() >= 0 && this.getCivID() < CFG.core.getCivsSize()) {
            for (int i = 0; i < this.getProvinces().size(); ++i) {
                ...
                CFG.core.getProv(this.getProvinces().get(i)).setDevLvl(CFG.core.getProv(this.getProvinces().get(i)).getDeveLvl() + (float)this.getValue() / 100.0f);
            }
        }
    }
    ...
    public boolean canMakeAction(int i) {
        try {
            return !CFG.core.getProv(this.getProvinces().get(i)).getSeaProv() && CFG.core.getProv(this.getProvinces().get(i)).getWastelandLvl() < 0 && CFG.core.getProv(this.getProvinces().get(i)).getCivId() == this.getCivID();
        }
    ...
    public String getConditionText() {
        try {
            return CFG.lang.get("UpdateDevelopmentLevel") + ": " + CFG.core.getCiv(this.getCivID()).getCivName() + ", " + (this.getValue() > 0 ? "+" : "") + (float)this.getValue() / 100.0f;
        }
        catch (IndexOutOfBoundsException ex) {
            return CFG.lang.get("UpdateDevelopmentLevel");
        }
    }
        ...
        try {
            ...
            if (this.getProvinces().size() < 5) {
                for (int i = 0; i < this.getProvinces().size(); ++i) {
                    ...
                    tData.add(new ME_Hover_2Type_Text(CFG.lang.get("Development") + ": ", CFG.COLOR_HOVER_TITLE));
                    ...
                    tData.add(new ME_Hover_2Type_Text(" " + (this.getValue() > 0 ? "+" : "") + (float)this.getValue() / 100.0f, this.getValue() > 0 ? CFG.COLOR_POSITIVE : (this.getValue() == 0 ? CFG.COLOR_NEUTRAL2 : CFG.COLOR_NEGATIVE_2)));
                    tData.add(new ME_Hover_2Type_Image(Images.development, CFG.PADD, 0));
