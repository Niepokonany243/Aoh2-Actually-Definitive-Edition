...
public class Event_Outcome_Occupy
extends Event_Outcome {
    ...
    public List<MEHover_2E> getHoverText() {
        ...
        if (this.getProvinces().size() < 9) {
            for (int i = 0; i < this.getProvinces().size(); ++i) {
                ...
                tData.add(new ME_Hover_2Type_Text(CFG.lang.get("Occupy") + ": ", CFG.COLOR_HOVER_TITLE));
    ...
    public String getConditionText() {
        try {
            return CFG.lang.get("Occupy") + ": " + CFG.core.getCiv(this.getCivID()).getCivName();
        }
        catch (IndexOutOfBoundsException ex) {
            return CFG.lang.get("Occupy");
        }
    }
