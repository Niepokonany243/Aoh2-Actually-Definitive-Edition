...
public class Event_Conditions_AveDevLow
extends Event_Conditions {
    ...
    public boolean outCondition() {
        try {
            return CFG.core.getCiv((int)this.getCivID()).fAverageDevelopment < (float)this.getValue() / 100.0f;
        }
        catch (Exception ex) {
            return false;
        }
    ...
    public String getConditionText() {
        try {
            return CFG.lang.get("AverageDevelopment") + " < " + (float)this.getValue() / 100.0f + ", " + CFG.core.getCiv(this.getCivID()).getCivName();
        }
        catch (Exception ex) {
            return CFG.lang.get("AverageDevelopment");
        }
    }
