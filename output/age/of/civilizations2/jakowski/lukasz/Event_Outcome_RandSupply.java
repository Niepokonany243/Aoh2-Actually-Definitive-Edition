...
public class Event_Outcome_RandSupply
extends Event_Outcome {
            ...
            try {
                ...
                if (this.getValue() < 0) {
                    ...
                    for (i = 0; i < CFG.core.getCiv(this.getCivID()).getNumOfProvs(); ++i) {
                        if (CFG.core.getProv(CFG.core.getCiv(this.getCivID()).getProvID(i)).getLvlOfSupply() <= 0) continue;
                    ...
                    if (!lProvinces.isEmpty()) {
                        for (i = 0; i < Math.abs(this.getValue()); ++i) {
                            ...
                            if (CFG.core.getProv((Integer)lProvinces.get(randID)).getLvlOfSupply() <= 0) continue;
                            CFG.core.getProv((Integer)lProvinces.get(randID)).setLvlOfSupply(CFG.core.getProv((Integer)lProvinces.get(randID)).getLvlOfSupply() - 1);
                        }
                    }
                } else {
                    int i;
                    ArrayList<Integer> lProvinces = new ArrayList<Integer>();
                    for (i = 0; i < CFG.core.getCiv(this.getCivID()).getNumOfProvs(); ++i) {
                        if (CFG.core.getProv(CFG.core.getCiv(this.getCivID()).getProvID(i)).getLvlOfSupply() >= BuildingsManager.getSupply_MaxLevel()) continue;
                        lProvinces.add(CFG.core.getCiv(this.getCivID()).getProvID(i));
                    }
                    if (!lProvinces.isEmpty()) {
                        for (i = 0; i < this.getValue(); ++i) {
                            int randID = CFG.oR.nextInt(lProvinces.size());
                            if (CFG.core.getProv((Integer)lProvinces.get(randID)).getLvlOfSupply() >= BuildingsManager.getSupply_MaxLevel()) continue;
                            CFG.core.getProv((Integer)lProvinces.get(randID)).setLvlOfSupply(CFG.core.getProv((Integer)lProvinces.get(randID)).getLvlOfSupply() + 1);
                        }
                    }
                }
    ...
    public String getConditionText() {
        try {
            if (this.getValue() < 0) {
                return CFG.lang.get("DestroyBuilding") + ": " + CFG.lang.get("SupplyCamp") + " -> " + CFG.lang.get("Random") + ": " + CFG.core.getCiv(this.getCivID()).getCivName() + ", " + (this.getValue() > 0 ? "+" : "") + this.getValue();
            }
            return CFG.lang.get("Build") + " " + CFG.lang.get("SupplyCamp") + " -> " + CFG.lang.get("Random") + ": " + CFG.core.getCiv(this.getCivID()).getCivName() + ", " + (this.getValue() > 0 ? "+" : "") + this.getValue();
        }
        catch (IndexOutOfBoundsException ex) {
            return CFG.lang.get("Build") + " " + CFG.lang.get("SupplyCamp") + " -> " + CFG.lang.get("Random");
        }
    }
        ...
        try {
            ...
            if (this.canMakeAction()) {
                ...
                if (this.getValue() < 0) {
                    ...
                    tData.add(new ME_Hover_2Type_Text(CFG.lang.get("SupplyCamp") + ": ", CFG.COLOR_HOVER_TITLE));
                } else {
                ...
                tData.add(new ME_Hover_2Type_Image(Images.bSupply, CFG.PADD, 0));
