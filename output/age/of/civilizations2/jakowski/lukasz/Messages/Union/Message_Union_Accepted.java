...
public class Message_Union_Accepted
extends Message {
    public Message_Union_Accepted(int fromCivID, int iValue) {
        super(fromCivID, iValue);
        this.messageType = MessageType.UNION_ACCEPTED;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            lMess.add(CFG.lang.get("FormUnion") + " - " + CFG.lang.get("OurProposalWasAccepted"));
            lColors.add(CFG.COLOR_POSITIVE);
            CFG.toastM.addM(lMess, lColors);
            CFG.toastM.setTimeInView(4500);
            CFG.menus.rebuildMenu_InGame_InfoboxSmallFlags(CFG.lang.get("FormUnion") + ": " + CFG.lang.get("OurProposalWasAccepted"), iCivID, this.fromCivID, Images.infoDiplomacy);
        }
    }
