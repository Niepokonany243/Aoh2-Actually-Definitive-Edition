...
package age.of.civilizations2.jakowski.lukasz.Messages.Union;
...
public class Message_Union
extends Message {
    public Message_Union(int fromCivID, int iValue) {
        super(fromCivID, iValue);
        this.messageType = MessageType.UNION;
        this.requestsResponse = GameValues.gvDipUnion.UNION_REQUESTS_RESPONSE;
    }

    @Override
    public void onAction(int iMessageID) {
        CFG.menus.rebuildInGame_Message_Union(this.fromCivID, iMessageID, (int)this.iValue);
    }

    @Override
    public void onAccept(int iCivID) {
        GameManager.acceptUnionProposal(iCivID, this.fromCivID);
    }

    @Override
    public void onDecline(int iCivID) {
        GameManager.declineUnionProposal(iCivID, this.fromCivID);
    }

    @Override
    public int getImageID() {
        return Images.diploUnion;
    }
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("FormUnionWithCivA", CFG.core.getCiv(this.fromCivID).getCivName()), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
        ...
        nData.add(new ME_Hover_2Type_Text(GameManager.getUnionMessage(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() + this.fromCivID)));
        nElements.add(new MEHover_2E(nData));
        nData.clear();
