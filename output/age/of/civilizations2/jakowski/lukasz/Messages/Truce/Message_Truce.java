...
public class Message_Truce
extends Message {
    public Message_Truce(int fromCivID) {
        super(fromCivID, 0);
        this.messageType = MessageType.TRUCE;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            lMess.add(CFG.lang.get("TruceWith") + ": " + CFG.core.getCiv(this.fromCivID).getCivName());
    ...
    public void onDecline(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            CFG.menus.rebuildMenu_InGame_TruceInfoBox(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), this.fromCivID);
            ArrayList<String> lMess = new ArrayList<String>();
            ArrayList<Color> lColors = new ArrayList<Color>();
            lMess.add(CFG.lang.get("TruceWith") + ": " + CFG.core.getCiv(this.fromCivID).getCivName());
            lColors.add(CFG.COLOR_POSITIVE);
