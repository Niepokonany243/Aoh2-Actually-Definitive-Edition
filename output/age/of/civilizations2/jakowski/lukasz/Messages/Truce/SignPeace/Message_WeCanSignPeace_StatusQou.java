 ...
 */
package age.of.civilizations2.jakowski.lukasz.Messages.Truce.SignPeace;
...
public class Message_WeCanSignPeace_StatusQou
extends Message {
    public Message_WeCanSignPeace_StatusQou(int fromCivID) {
        super(fromCivID, 0);
        this.messageType = MessageType.WE_CAN_SIGN_PEACE_STATUS_QUO;
        this.numOfTurnsLeft = 2;
    }

    @Override
    public void onAction(int iMessageID) {
        int tWarID = CFG.core.getWarID(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), this.fromCivID);
        if (tWarID >= 0 && tWarID < CFG.core.getWarsSize()) {
            Menu_InGame_WarDetails.WAR_ID = tWarID;
            CFG.menus.rebuildInGame_WarDetails();
        }
        CFG.toastM.addM(CFG.lang.get("PeaceNegotiations"), CFG.COLOR_TEXT_NUM_OF_PROVINCES);
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("OfferPeaceToThisCivilization"), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
        nData.add(new ME_Hover_2Type_Image_Big(Images.peace, CFG.PADD, 0));
        ...
        nData.add(new ME_Hover_2Type_Text("" + CFG.core.getCiv(this.fromCivID).getNumOfProvs(), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
