...
public class Message_BecomeVassalRejected
extends Message {
    public Message_BecomeVassalRejected(int fromCivID, int iValue) {
        super(fromCivID, iValue);
        this.messageType = MessageType.BECOME_VASSAL_REJECTED;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            lMess.add(CFG.lang.get("OfferOfVassalageRejected"));
            lColors.add(CFG.COLOR_NEGATIVE_2);
            CFG.toastM.addM(lMess, lColors);
            CFG.toastM.setTimeInView(4500);
            CFG.menus.rebuildMenu_InGame_InfoboxSmallFlags(CFG.lang.get("OfferOfVassalageRejected"), GameCalendar.getCurrDate(), Images.infoDiplomacy, iCivID, this.fromCivID);
        }
    }
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("OfferOfVassalageRejected") + ": "));
