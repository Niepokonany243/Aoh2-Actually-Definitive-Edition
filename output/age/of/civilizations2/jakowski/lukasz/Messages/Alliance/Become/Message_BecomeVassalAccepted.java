...
public class Message_BecomeVassalAccepted
extends Message {
    public Message_BecomeVassalAccepted(int fromCivID, int iValue) {
        super(fromCivID, iValue);
        this.messageType = MessageType.BECOME_VASSAL_ACCEPTED;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            lMess.add(CFG.lang.get("VassalageAccepted"));
            lColors.add(CFG.COLOR_POSITIVE);
            CFG.toastM.addM(lMess, lColors);
            CFG.toastM.setTimeInView(4500);
            CFG.menus.rebuildMenu_InGame_InfoboxSmallFlags(CFG.lang.get("VassalageAccepted"), GameCalendar.getCurrDate(), Images.infoDiplomacy, iCivID, this.fromCivID);
        }
    }
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("VassalageAccepted") + ": "));
