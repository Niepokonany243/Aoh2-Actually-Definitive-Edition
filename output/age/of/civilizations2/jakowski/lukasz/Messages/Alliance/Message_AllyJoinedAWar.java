...
public class Message_AllyJoinedAWar
extends Message {
    private int iCivID;

    public Message_AllyJoinedAWar(int fromCivID, int iValue, int iCivID) {
        super(fromCivID, iValue);
        this.iCivID = iCivID;
        this.messageType = MessageType.WAR_DECLARED_ON_ALLY_JOINED;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            CFG.menus.rebuildMenu_InGame_InfoboxDate(CFG.lang.get("XJoinedTheWar", CFG.core.getCiv(this.fromCivID).getCivName()), iCivID, this.fromCivID, Images.infoWar);
        }
    }
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("Alliance"), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
        nElements.add(new MEHover_2E(nData));
        nData.clear();
        nData.add(new ME_Hover_2Type_Flag(this.fromCivID));
        nData.add(new ME_Hover_2Type_Text(CFG.lang.get("XJoinedTheWar", CFG.core.getCiv(this.fromCivID).getCivName()) + ": "));
        nData.add(new ME_Hover_2Type_Flag(this.iCivID, 0, CFG.PADD));
        nData.add(new ME_Hover_2Type_Image(Images.diploRivals, 0, 0));
        nData.add(new ME_Hover_2Type_Flag((int)this.iValue, CFG.PADD, 0));
        nElements.add(new MEHover_2E(nData));
        nData.clear();
