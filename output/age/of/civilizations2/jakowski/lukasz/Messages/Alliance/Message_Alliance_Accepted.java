...
public class Message_Alliance_Accepted
extends Message {
    public Message_Alliance_Accepted(int fromCivID) {
        super(fromCivID, 0);
        this.messageType = MessageType.ALLIANCE_ACCEPTED;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            lMess.add(CFG.lang.get("AllianceProposalAccepted"));
            lColors.add(CFG.COLOR_POSITIVE);
            CFG.toastM.addM(lMess, lColors);
            CFG.toastM.setTimeInView(4500);
            CFG.menus.rebuildMenu_InGame_AllianceInfo(iCivID, this.fromCivID);
        }
    }
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("AllianceProposalAccepted"), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
