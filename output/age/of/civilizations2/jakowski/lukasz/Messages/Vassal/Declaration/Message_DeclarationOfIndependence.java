...
public class Message_DeclarationOfIndependence
extends Message {
    public Message_DeclarationOfIndependence(int byCivID, int nProvinceID) {
        super(byCivID, nProvinceID);
        this.messageType = MessageType.DECLARATION_OF_INDEPENDENCE;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            CFG.toastM.addM(CFG.lang.get("DeclarationOfIndependence"), CFG.COLOR_TEXT_NUM_OF_PROVINCES);
            CFG.menus.rebuildMenu_InGame_Infobox(CFG.lang.get("DeclarationOfIndependence"), iCivID, this.fromCivID, Images.infoDiplomacy);
        }
    }
