...
public class Message_Liberation
extends Message {
    public Message_Liberation(int byCivID) {
        super(byCivID, 0);
        this.messageType = MessageType.LIBERATION_OF_VASSAL;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            CFG.toastM.addM(CFG.lang.get("Liberation"), CFG.COLOR_TEXT_NUM_OF_PROVINCES);
            CFG.menus.rebuildMenu_InGame_Infobox(CFG.lang.get("Liberation"), iCivID, this.fromCivID, Images.infoDiplomacy);
        }
    }
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("Liberation"), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
        nElements.add(new MEHover_2E(nData));
        nData.clear();
        nData.add(new ME_Hover_2Type_Text(CFG.lang.get("WeAreNotLongerAVassal")));
