 ...
 */
package age.of.civilizations2.jakowski.lukasz.Messages.Relations.Sanctions;
...
public class Message_Sanctioned
extends Message {
    public Message_Sanctioned(int fromCivID) {
        super(fromCivID, 0);
        this.messageType = MessageType.SANCTIONED;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID && !CFG.SPECTATOR_MODE) {
            ArrayList<String> lMess = new ArrayList<String>();
            ArrayList<Color> lColors = new ArrayList<Color>();
            lMess.add(CFG.lang.get("SanctionsImposed"));
            lColors.add(CFG.COLOR_NEGATIVE_1);
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("SanctionsImposed")));
        nData.add(new ME_Hover_2Type_Image_Big(Images.sanctions, CFG.PADD, 0));
        nElements.add(new MEHover_2E(nData));
        nData.clear();
        float impact = GameManager.getSanctionsImpact(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), this.fromCivID);
        if (impact > 0.0f) {
            nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("SanctionsImpact") + ": "));
            nData.add(new ME_Hover_2Type_Text_Big("-" + CFG.getPrecision2(impact * 100.0f, 100) + "%", CFG.COLOR_NEGATIVE_2));
            nData.add(new ME_Hover_2Type_Image_Big(Images.sanctions, CFG.PADD, 0));
