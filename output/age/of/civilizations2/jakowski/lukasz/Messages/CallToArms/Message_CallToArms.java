...
public class Message_CallToArms
extends Message {
    public Message_CallToArms(int fromCivID, int iValue) {
        super(fromCivID, iValue);
        this.messageType = MessageType.WAR_DECLARED_ON_ALLY;
    ...
    public void onAction(int iMessageID) {
        CFG.menus.rebuildInGame_Message_CallToArms(this.fromCivID, iMessageID, (int)this.iValue);
    }

    @Override
    public void onAccept(int iCivID) {
        GameManager.acceptCallToArms(iCivID, this.fromCivID, (int)this.iValue);
    }

    @Override
    public void onDecline(int iCivID) {
        GameManager.declineCallToArms(iCivID, this.fromCivID, (int)this.iValue);
    }
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Flag_Big((int)this.iValue, 0, CFG.PADD));
        nData.add(new ME_Hover_2Type_Text_Big(CFG.core.getCiv((int)this.iValue).getCivName(), CFG.COLOR_HOVER_TITLE));
        nElements.add(new MEHover_2E(nData));
        nData.clear();
        nData.add(new ME_Hover_2Type_Space());
        nElements.add(new MEHover_2E(nData));
