...
public class Message_LoanRequest_Refused
extends Message {
    public Message_LoanRequest_Refused(int fromCivID, int iValue) {
        super(fromCivID, iValue);
        this.messageType = MessageType.LOAN_REQUEST_REJECTED;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            lMess.add(CFG.lang.get("LoanProposalRejected"));
            lColors.add(CFG.COLOR_NEGATIVE_2);
            CFG.menus.rebuildMenu_InGame_Infobox(CFG.lang.get("LoanProposalRejected"), this.fromCivID, CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), Images.infoDiplomacy);
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("LoanProposalRejected") + ": ", CFG.COLOR_TEXT_NUM_OF_PROVINCES));
