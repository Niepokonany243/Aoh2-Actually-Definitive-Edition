...
extends Message {
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            lMess.add(CFG.core.getCiv(this.fromCivID).getCivName() + ": " + CFG.lang.get("RefusedToJoinWarPreparationsAgainst") + ": " + CFG.core.getCiv((int)this.iValue).getCivName());
            lColors.add(CFG.COLOR_NEGATIVE_2);
            CFG.toastM.addM(lMess, lColors);
            CFG.toastM.setTimeInView(4500);
            CFG.menus.rebuildMenu_InGame_InfoboxDate(CFG.core.getCiv(this.fromCivID).getCivName() + ": " + CFG.lang.get("RefusedToJoinWarPreparationsAgainst") + ": " + CFG.core.getCiv((int)this.iValue).getCivName(), -1, -1, Images.infoWar);
        }
    }
