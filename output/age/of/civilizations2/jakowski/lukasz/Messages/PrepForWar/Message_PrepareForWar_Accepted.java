...
public class Message_PrepareForWar_Accepted
extends Message {
    public Message_PrepareForWar_Accepted(int fromCivID, int iValue) {
        super(fromCivID, iValue);
        this.messageType = MessageType.PREPARE_FOR_WAR_ACCEPTED;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            lMess.add(CFG.core.getCiv(this.fromCivID).getCivName() + ": " + CFG.lang.get("AcceptedToJoinWarPreparationsAgainst") + ": " + CFG.core.getCiv((int)this.iValue).getCivName());
            lColors.add(CFG.COLOR_POSITIVE);
            CFG.toastM.addM(lMess, lColors);
            CFG.toastM.setTimeInView(4500);
            CFG.menus.rebuildMenu_InGame_InfoboxDate(CFG.core.getCiv(this.fromCivID).getCivName() + ": " + CFG.lang.get("AcceptedToJoinWarPreparationsAgainst") + ": " + CFG.core.getCiv((int)this.iValue).getCivName(), -1, -1, Images.infoWar);
        }
    }
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("AcceptedToJoinWarPreparationsAgainst") + ":", CFG.COLOR_POSITIVE));
        nData.add(new ME_Hover_2Type_Flag_Big((int)this.iValue, CFG.PADD, CFG.PADD));
        nData.add(new ME_Hover_2Type_Text_Big(CFG.core.getCiv((int)this.iValue).getCivName()));
