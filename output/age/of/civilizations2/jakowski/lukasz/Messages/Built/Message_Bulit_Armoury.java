...
public class Message_Bulit_Armoury
extends Message {
    public Message_Bulit_Armoury(int fromCivID, int iValue) {
        super(fromCivID, iValue);
        this.messageType = MessageType.BUILT_ARMOURY;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            lMess.add((CFG.core.getProv((int)this.iValue).getName().length() > 0 ? CFG.core.getProv((int)this.iValue).getName() + ": " : "") + CFG.lang.get(BuildingsManager.getArmoury_Name(CFG.core.getProv((int)this.iValue).getLvlOfArmoury())));
    ...
    public ME_Hover_v2 getHover() {
        ...
        nElements.add(new MEHover_2E(nData));
        nData.clear();
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get(BuildingsManager.getArmoury_Name(CFG.core.getProv((int)this.iValue).getLvlOfArmoury())), CFG.COLOR_HOVER_TITLE));
