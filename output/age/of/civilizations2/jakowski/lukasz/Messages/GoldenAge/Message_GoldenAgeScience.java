...
public class Message_GoldenAgeScience
extends Message {
    public Message_GoldenAgeScience(int fromCivID, int iNumOfTurns) {
        super(fromCivID, 0);
        this.messageType = MessageType.GOLDEN_AGE_SCIENCE;
        this.numOfTurnsLeft = iNumOfTurns;
    }

    @Override
    public void onAction(int iMessageID) {
        CFG.toastM.addM(CFG.lang.get("OurCivilizationIsInAGoldenAgeOf") + ": " + CFG.lang.get("GAScience"), CFG.COLOR_TEXT_GOLDEN_AGE);
        CFG.menus.rebuildMenu_InGame_InfoboxDate(CFG.lang.get("OurCivilizationIsInAGoldenAgeOf") + ": " + CFG.lang.get("GAScience"), this.fromCivID, -1, Images.infoDiplomacy);
        try {
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("GAScience")));
        ...
        for (int i = 0; i < CFG.core.getCiv(this.fromCivID).getBonusesSize(); ++i) {
            if (CFG.core.getCiv((int)this.fromCivID).getBonus((int)i).BONUS_TYPE != CivBonus_Type.GOLDEN_AGE_SCIENCE) continue;
            nID = i;
            break;
        }
        if (nID >= 0) {
