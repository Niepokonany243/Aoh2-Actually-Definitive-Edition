...
package age.of.civilizations2.jakowski.lukasz.Messages.Trade;
...
public class Message_TradeRequest
extends Message {
    public Message_TradeRequest(int fromCivID, TradeRequest_GameData nTradeRequest) {
        super(fromCivID, 0);
        this.tradeRequest = nTradeRequest;
        this.messageType = MessageType.TRADE_REQUEST;
        this.requestsResponse = GameValues.gvTrade.TRADE_REQUESTS_RESPONSE;
    }

    @Override
    public void onAction(int iMessageID) {
        CFG.menus.rebuildInGame_Message_TradeRequest(this.fromCivID, iMessageID, this.tradeRequest);
    }

    @Override
    public void onAccept(int iCivID) {
        GameManager.acceptTradeRequest(iCivID, this.fromCivID, this.tradeRequest);
    }

    @Override
    public void onDecline(int iCivID) {
        GameManager.declineTradeRequest(iCivID, this.fromCivID, this.tradeRequest);
    }

    @Override
    public int getImageID() {
        return Images.diploTrade;
    }
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text(CFG.lang.get("TradeRequest"), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
        nData.add(new ME_Hover_2Type_Image(Images.diploTrade, CFG.PADD, 0));
