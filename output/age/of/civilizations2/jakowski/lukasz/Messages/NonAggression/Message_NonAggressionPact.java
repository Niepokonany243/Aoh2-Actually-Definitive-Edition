...
package age.of.civilizations2.jakowski.lukasz.Messages.NonAggression;
...
public class Message_NonAggressionPact
extends Message {
    public Message_NonAggressionPact(int fromCivID, int iValue) {
        super(fromCivID, iValue);
        this.messageType = MessageType.NONAGGRESSIONPACT;
        this.requestsResponse = GameValues.gvDipNonAggression.NON_AGGRESSION_REQUESTS_RESPONSE;
    }

    @Override
    public void onAction(int iMessageID) {
        CFG.menus.rebuildInGame_Message_NonAggression(this.fromCivID, iMessageID, (int)this.iValue);
    }

    @Override
    public void onAccept(int iCivID) {
        GameManager.acceptNonAggressionPact(iCivID, this.fromCivID, (int)this.iValue);
    }

    @Override
    public void onDecline(int iCivID) {
        GameManager.declineNonAggressionPact(iCivID, this.fromCivID, (int)this.iValue);
    }

    @Override
    public int getImageID() {
        return Images.diploNonAggression;
    }
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("SignNonAggressionPactWithCivA", CFG.core.getCiv(this.fromCivID).getCivName())));
        nElements.add(new MEHover_2E(nData));
        nData.clear();
