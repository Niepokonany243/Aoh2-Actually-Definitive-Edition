...
public class Message_NonAggressionPact_Expired
extends Message {
    public Message_NonAggressionPact_Expired(int fromCivID) {
        super(fromCivID, 0);
        this.messageType = MessageType.NONAGGRESSIONPACT_EXPIRED;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            lMess.add(CFG.lang.get("NonAggressionPactHasExpiredWith") + ": " + CFG.core.getCiv(this.fromCivID).getCivName());
            lColors.add(CFG.COLOR_TEXT_NUM_OF_PROVINCES);
            CFG.toastM.addM(lMess, lColors);
            CFG.toastM.setTimeInView(4500);
            CFG.menus.rebuildMenu_InGame_InfoboxSmallFlags(CFG.lang.get("NonAggressionPactHasExpiredWith") + ": " + CFG.core.getCiv(this.fromCivID).getCivName(), iCivID, this.fromCivID, Images.infoDiplomacy);
        }
    }
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("NonAggressionPactHasExpiredWith") + ": ", CFG.COLOR_TEXT_NUM_OF_PROVINCES));
