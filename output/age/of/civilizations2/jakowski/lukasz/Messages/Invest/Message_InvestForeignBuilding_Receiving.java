...
public class Message_InvestForeignBuilding_Receiving
extends Message {
    public long gold = 0L;
    public long profit = 0L;

    public Message_InvestForeignBuilding_Receiving(int fromCivID, int iValue, long gold, long profit) {
        super(fromCivID, iValue);
        this.gold = gold;
        this.profit = profit;
        this.messageType = MessageType.RECEIVING_FOREIGN_INVEST_BUILD;
        this.numOfTurnsLeft = 2;
    }

    @Override
    public void onAction(int iMessageID) {
        CFG.map.getMpC().centerToProvID((int)this.iValue);
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            lMess.add(CFG.lang.get("ReceivingForeignConstruction") + ": " + CFG.lang.get("Gold") + ": " + CFG.getNumberWthSpaces("" + this.gold) + ", " + CFG.lang.get("Profit") + ": " + CFG.getNumberWthSpaces("" + this.profit));
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Flag_Big(CFG.core.getProv((int)this.iValue).getCivId()));
        nData.add(new ME_Hover_2Type_Text_Big((CFG.core.getProv((int)this.iValue).getName().length() > 0 ? CFG.core.getProv((int)this.iValue).getName() : CFG.lang.get("Province")) + ": ", CFG.COLOR_TEXT_NUM_OF_PROVINCES));
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("ReceivingForeignConstruction")));
