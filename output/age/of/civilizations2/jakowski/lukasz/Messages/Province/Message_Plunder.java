...
public class Message_Plunder
extends Message {
    private int iTreasury;
    private int iEconomy;
    private long iPopulation;
    private float fHappiness;
    private float fDevelopment;

    public Message_Plunder(int fromCivID, int iValue, int iTreasury, int iEconomy, float fDevelopment, float fHappiness, long iPopulation) {
        super(fromCivID, iValue);
        this.iTreasury = iTreasury;
        this.iEconomy = iEconomy;
        this.fHappiness = fHappiness;
        this.fDevelopment = fDevelopment;
        this.iPopulation = iPopulation;
        this.messageType = MessageType.PLUNDER_REPORT;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            lMess.add(CFG.lang.get("Treasury") + ": +" + this.iTreasury);
            lColors.add(CFG.COLOR_GOLD);
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("Plunder") + (CFG.core.getProv((int)this.iValue).getName().length() > 0 ? ": " + CFG.core.getProv((int)this.iValue).getName() : ""), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
        nElements.add(new MEHover_2E(nData));
        nData.clear();
        nData.add(new ME_Hover_2Type_Text(CFG.lang.get("Treasury") + ": "));
        nData.add(new ME_Hover_2Type_Text("+" + this.iTreasury, CFG.COLOR_GOLD));
