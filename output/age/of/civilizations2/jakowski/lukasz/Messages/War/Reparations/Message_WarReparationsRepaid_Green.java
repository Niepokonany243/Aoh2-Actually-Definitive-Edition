 ...
 */
package age.of.civilizations2.jakowski.lukasz.Messages.War.Reparations;
...
public class Message_WarReparationsRepaid_Green
extends Message {
    public Message_WarReparationsRepaid_Green(int fromCivID) {
        super(fromCivID, 0);
        this.messageType = MessageType.WAR_REPARATIONS_REPAID;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            lMess.add(CFG.lang.get("WarReparationsRepaid") + ": " + CFG.core.getCiv(this.fromCivID).getCivName());
            lColors.add(CFG.COLOR_TEXT_NUM_OF_PROVINCES);
            CFG.toastM.addM(lMess, lColors);
            CFG.toastM.setTimeInView(4500);
            CFG.menus.rebuildMenu_InGame_InfoboxDate(CFG.lang.get("WarReparationsRepaid") + ": " + CFG.core.getCiv(this.fromCivID).getCivName(), iCivID, -1, Images.infoDiplomacy);
        }
    }
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("WarReparationsRepaid") + ": ", CFG.COLOR_TEXT_NUM_OF_PROVINCES));
