...
public class Message_TransferControl_Refused
extends Message {
    private int iCivID;

    public Message_TransferControl_Refused(int fromCivID, int iValue, int iCivID) {
        super(fromCivID, iValue);
        this.iCivID = iCivID;
        this.messageType = MessageType.TRANSFER_CONTROL_REFUSED;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            CFG.toastM.addM(CFG.lang.get("ProposalWasRejected") + "!", CFG.COLOR_NEGATIVE_2);
            CFG.menus.rebuildMenu_InGame_InfoboxSmallFlags(CFG.lang.get("TransferControl") + ": " + CFG.lang.get("ProposalWasRejected"), iCivID, this.fromCivID, Images.infoDiplomacy);
        }
    }
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.clear();
        nData.add(new ME_Hover_2Type_Text(CFG.core.getProv((int)this.iValue).getName().length() > 0 ? CFG.core.getProv((int)this.iValue).getName() : CFG.lang.get("Province")));
