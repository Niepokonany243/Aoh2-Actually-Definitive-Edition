...
public class Message_TransferControl_Accepted
extends Message {
    private int iCivID;

    public Message_TransferControl_Accepted(int fromCivID, int iValue, int iCivID) {
        super(fromCivID, iValue);
        this.iCivID = iCivID;
        this.messageType = MessageType.TRANSFER_CONTROL_ACCEPTED;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            CFG.toastM.addM(CFG.lang.get("Great") + "!", CFG.COLOR_POSITIVE);
            CFG.menus.rebuildMenu_InGame_InfoboxSmallFlags(CFG.lang.get("TransferControl") + ": " + CFG.lang.get("OurProposalWasAccepted"), iCivID, this.fromCivID, Images.infoDiplomacy);
        }
    }
