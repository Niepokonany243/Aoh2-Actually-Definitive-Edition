...
public class Message_Age
extends Message {
    public Message_Age(int fromCivID) {
        super(fromCivID, 0);
        this.messageType = MessageType.NEW_AGE;
        this.numOfTurnsLeft = 4;
    }

    @Override
    public void onAction(int iMessageID) {
        CFG.menus.rebuildMenu_InGame_Infobox("" + CFG.gameAges.getAge(GameCalendar.CURRENT_AGEID).getName(), GameCalendar.getCurrDate(), Images.infoDiplomacy);
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("WelcomeInTheNewAge"), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
        nElements.add(new MEHover_2E(nData));
        nData.clear();
        nData.add(new ME_Hover_2Type_Space());
        nElements.add(new MEHover_2E(nData));
        nData.clear();
        nData.add(new ME_Hover_2Type_Text("" + CFG.gameAges.getAge(GameCalendar.CURRENT_AGEID).getName() + " "));
        nData.add(new ME_Hover_2Type_Text("[" + CFG.gameAges.getYear(CFG.gameAges.getAge(GameCalendar.CURRENT_AGEID).getBeginningYear()), CFG.COLOR_NEUTRAL));
        nData.add(new ME_Hover_2Type_Text(" - "));
        nData.add(new ME_Hover_2Type_Text("" + CFG.gameAges.getYear(CFG.gameAges.getAge(GameCalendar.CURRENT_AGEID).getEndYear()) + "]", CFG.COLOR_NEUTRAL));
