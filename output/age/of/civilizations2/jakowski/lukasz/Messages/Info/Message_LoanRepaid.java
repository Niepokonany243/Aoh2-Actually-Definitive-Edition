...
public class Message_LoanRepaid
extends Message {
    public Message_LoanRepaid(int fromCivID, int iValue) {
        super(fromCivID, iValue);
        this.messageType = MessageType.LOAN_REPAID;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            lMess.add(CFG.lang.get("LoanRepaid"));
            lColors.add(CFG.COLOR_TEXT_NUM_OF_PROVINCES);
            CFG.toastM.addM(lMess, lColors);
            CFG.toastM.setTimeInView(4500);
            CFG.menus.rebuildMenu_InGame_InfoboxDate(CFG.lang.get("LoanRepaid"), iCivID, -1, Images.infoDiplomacy);
        }
    }
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("LoanRepaid"), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
