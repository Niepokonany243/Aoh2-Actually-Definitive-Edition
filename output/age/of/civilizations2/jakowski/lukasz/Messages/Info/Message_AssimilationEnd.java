...
public class Message_AssimilationEnd
extends Message {
    public Message_AssimilationEnd(int fromCivID, int iValue) {
        super(fromCivID, iValue);
        this.messageType = MessageType.ASSIMILATION_IS_OVER;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            lMess.add(CFG.lang.get("TheProcessOfAssimilationHasComeToAnEnd"));
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("TheProcessOfAssimilationHasComeToAnEnd")));
        ...
        nData.add(new ME_Hover_2Type_Text("" + (int)(CFG.core.getProv((int)this.iValue).getProviStability() * 100.0f) + "%", CFG.COLOR_PROVINCE_STABILITY_MAX));
