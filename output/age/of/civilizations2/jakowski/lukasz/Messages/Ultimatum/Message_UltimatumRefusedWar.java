...
public class Message_UltimatumRefusedWar
extends Message {
    public Message_UltimatumRefusedWar(int fromCivID) {
        super(fromCivID, 0);
        this.messageType = MessageType.ULTIMATUM_REFUSED_WAR;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            lMess.add(CFG.core.getCiv(this.fromCivID).getCivName() + ": " + CFG.lang.get("RefusedToAcceptOurUltimatum"));
            lColors.add(CFG.COLOR_NEGATIVE_2);
            CFG.toastM.addM(lMess, lColors);
            CFG.toastM.setTimeInView(4500);
            CFG.menus.rebuildMenu_InGame_InfoboxSmallFlags(CFG.core.getCiv(this.fromCivID).getCivName() + ": " + CFG.lang.get("RefusedToAcceptOurUltimatum"), iCivID, this.fromCivID, Images.infoDiplomacy);
        }
    }
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("RefusedToAcceptOurUltimatum"), CFG.COLOR_NEGATIVE_2));
        nElements.add(new MEHover_2E(nData));
        nData.clear();
        nData.add(new ME_Hover_2Type_Text(CFG.lang.get("WeAreAtWar"), CFG.COLOR_NEGATIVE_2));
