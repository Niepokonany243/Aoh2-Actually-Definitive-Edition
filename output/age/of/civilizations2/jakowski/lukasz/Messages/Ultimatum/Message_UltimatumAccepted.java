...
public class Message_UltimatumAccepted
extends Message {
    public Message_UltimatumAccepted(int fromCivID) {
        super(fromCivID, 0);
        this.messageType = MessageType.ULTIMATUM_ACCEPTED;
    ...
    public void onAccept(int iCivID) {
        if (CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() == iCivID) {
            ...
            lMess.add(CFG.core.getCiv(this.fromCivID).getCivName() + ": " + CFG.lang.get("AcceptedOurUltimatum"));
            lColors.add(CFG.COLOR_POSITIVE);
            CFG.toastM.addM(lMess, lColors);
            CFG.toastM.setTimeInView(4500);
            CFG.menus.rebuildMenu_InGame_InfoBox_Ultimatum(iCivID, this.fromCivID);
        }
    }
    ...
    public ME_Hover_v2 getHover() {
        ...
        nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("AcceptedOurUltimatum"), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
