...
public class ME_Hover_2Type_Wonder
implements ME_Hover_2Type {
    ...
    public ME_Hover_2Type_Wonder(int nProvinceID) {
        this.iProvinceID = nProvinceID;
        this.offsetLeft = 0;
        this.offsetRight = CFG.PADD;
    }

    public ME_Hover_2Type_Wonder(int nProvinceID, int offsetLeft) {
        this.iProvinceID = nProvinceID;
        this.offsetLeft = offsetLeft;
        this.offsetRight = CFG.PADD;
    }

    public ME_Hover_2Type_Wonder(int nProvinceID, int iWonderID, int offsetLeft, int offsetRight) {
        this.iProvinceID = nProvinceID;
    ...
    private final float getImageScale() {
        try {
            return (float)CFG.TEXT_HEIGHT_DEFAULT / (float)CFG.core.getProv((int)this.iProvinceID).getWonder((int)this.iWonderID).nImage.getHeight();
        }
        catch (Exception ex) {
