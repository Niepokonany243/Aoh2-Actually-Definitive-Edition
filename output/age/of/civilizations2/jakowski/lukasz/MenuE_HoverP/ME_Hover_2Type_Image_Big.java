...
public class ME_Hover_2Type_Image_Big
implements ME_Hover_2Type {
    private int iImageID;
    private int offsetLeft;
    private int offsetRight;

    public ME_Hover_2Type_Image_Big(int iImageID) {
        this.iImageID = iImageID;
        this.offsetLeft = 0;
        this.offsetRight = CFG.PADD;
    }

    public ME_Hover_2Type_Image_Big(int iImageID, int offsetLeft) {
        this.iImageID = iImageID;
        this.offsetLeft = offsetLeft;
        this.offsetRight = CFG.PADD;
    }

    public ME_Hover_2Type_Image_Big(int iImageID, int offsetLeft, int offsetRight) {
        this.iImageID = iImageID;
        this.offsetLeft = offsetLeft;
    ...
    private float getImageScale() {
        return (float)Math.max(IMGManager.getIMG(Images.flagRect2Mask).getHeight(), CFG.TEXT_HEIGHT_DEFAULT) / (float)IMGManager.getIMG(this.iImageID).getHeight();
    }
}
