...
public class ME_Hover_2Type_Image
implements ME_Hover_2Type {
    private int iImageID;
    private int offsetLeft;
    private int offsetRight;

    public ME_Hover_2Type_Image(int iImageID) {
        this.iImageID = iImageID;
        this.offsetLeft = 0;
        this.offsetRight = CFG.PADD;
    }

    public ME_Hover_2Type_Image(int iImageID, int offsetLeft) {
        this.iImageID = iImageID;
        this.offsetLeft = offsetLeft;
        this.offsetRight = CFG.PADD;
    }

    public ME_Hover_2Type_Image(int iImageID, int offsetLeft, int offsetRight) {
        this.iImageID = iImageID;
        this.offsetLeft = offsetLeft;
    ...
    public void draw(SpriteBatch oSB, int nPosX, int nPosY, float nAlpha) {
        ...
        IMGManager.getIMG(this.iImageID).draw(oSB, nPosX + this.offsetLeft, nPosY + CFG.PADD + CFG.TEXT_HEIGHT_DEFAULT / 2 - (int)((float)IMGManager.getIMG(this.iImageID).getHeight() * this.getImageScale() / 2.0f), (int)((float)IMGManager.getIMG(this.iImageID).getWidth() * this.getImageScale()), (int)((float)IMGManager.getIMG(this.iImageID).getHeight() * this.getImageScale()));
    }
