...
public class Event_Outcome_DiplomacyPoints
extends Event_Outcome {
    private static final long serialVersionUID = -1030200749092877332L;
    public int iCivID = -1;
    ...
    public void outcomeAction() {
        if (this.canMakeAction()) {
            CFG.core.getCiv(this.getCivID()).setDiploPoints(CFG.core.getCiv(this.getCivID()).getDiploPoints() + (int)this.getValue());
    ...
    public String getConditionText() {
        try {
            return CFG.lang.get("UpdateDiplomacyPoints") + ": " + CFG.core.getCiv(this.getCivID()).getCivName() + ", " + (this.getValue() > 0 ? "+" : "") + (float)this.getValue() / 10.0f;
        }
        catch (IndexOutOfBoundsException ex) {
            return CFG.lang.get("UpdateDiplomacyPoints");
        }
    }
    ...
    public List<MEHover_2E> getHoverText() {
        try {
            ...
            if (this.canMakeAction()) {
                ...
                tData.add(new ME_Hover_2Type_Text(CFG.lang.get("DiplomacyPoints") + ": ", CFG.COLOR_HOVER_TITLE));
