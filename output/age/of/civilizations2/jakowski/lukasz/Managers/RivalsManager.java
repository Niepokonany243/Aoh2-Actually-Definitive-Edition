...
public class RivalsManager {
    public static List<Integer> buildRivals(int civID, int rivalsLimit) {
        ...
        List<AI_CivsInRange> inRange;
        try {
            inRange = new ArrayList<AI_CivsInRange>(CFG.core.getCiv(civID).civsInRange);
        } catch (Exception ex) {
            inRange = Collections.emptyList();
        }
        for (int i = inRange.size() - 1; i >= 0; --i) {
            AI_CivsInRange civInRange = inRange.get(i);
            if (civInRange == null || civInRange.iCivID < 0 || CFG.core.getCiv(civInRange.iCivID).getNumOfProvs() <= 0) continue;
            rivals.add(civInRange.iCivID);
            distance.add(Float.valueOf(civInRange.fDistance));
        }
        ...
        while (out.size() <= rivalsLimit && !rivals.isEmpty() && !distance.isEmpty()) {
            int bestID = 0;
            int iSize2 = rivals.size();
            for (int i = 1; i < iSize2 && i < distance.size(); ++i) {
