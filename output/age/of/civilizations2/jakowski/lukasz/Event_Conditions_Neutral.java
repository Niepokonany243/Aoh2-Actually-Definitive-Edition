...
public class Event_Conditions_Neutral
extends Event_Conditions {
        ...
        try {
            if (this.getValue() == 0) {
                for (int i = 0; i < this.getProvinces().size(); ++i) {
                    if (CFG.core.getProv(this.getProvinces().get(i)).getCivId() == 0) continue;
                    return false;
                }
            } else {
                for (int i = 0; i < this.getProvinces().size(); ++i) {
    ...
    public String getConditionText() {
        try {
            return CFG.lang.get("NeutralProvince") + ": " + (this.getValue() == 0);
        }
        catch (IndexOutOfBoundsException ex) {
            return CFG.lang.get("NeutralProvince");
        }
    }
