...
public class Slider_InGame_Army
extends Slider {
    public static final Color bgColor = new Color(0.0f, 0.0f, 0.0f, 0.3f);

    public Slider_InGame_Army(String sText, int iPosX, int iPosY, int iWidth, int iHeight, long iMin, long iMax, long iCurrent) {
        this.initSlider(sText, iPosX, iPosY, iWidth, iHeight, iMin, iMax, iCurrent);
    }

    public Slider_InGame_Army(String sText, int iPosX, int iPosY, int iWidth, int iHeight, int iMin, int iMax, int iCurrent) {
        this.initSlider(sText, iPosX, iPosY, iWidth, iHeight, (long)iMin, (long)iMax, (long)iCurrent);
    }

    public Slider_InGame_Army(String sText, int iPosX, int iPosY, int iWidth, int iHeight, long iMin, long iMax, long iCurrent, float FONT_SCALE) {
        this.initSlider(sText, iPosX, iPosY, iWidth, iHeight, iMin, iMax, iCurrent);
    }

    public Slider_InGame_Army(String sText, int iPosX, int iPosY, int iWidth, int iHeight, int iMin, int iMax, int iCurrent, float FONT_SCALE) {
        this.initSlider(sText, iPosX, iPosY, iWidth, iHeight, (long)iMin, (long)iMax, (long)iCurrent);
    }
    ...
    public void drawE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive, boolean scrollableY) {
        if (CFG.settingsGD.BETTER_UI) {
            ...
            BetterUI_Manager.drawBetterSlider(oSB, this.getPosXE() + iTranslateX, this.getPosY() + this.getHeightE() - CFG.PADD - this.getSliderHeight() + iTranslateY, this.getWidthE(), this.getSliderHeight(), this.iCurrentPosX, this.iDifference_CurrentPosX, this.getIsHovered(), isActive, this.getColorLEFT());
            
            oSB.setColor(primary.r, primary.g, primary.b, 0.8f);
            IMGManager.getIMG(Images.diploArmy).drawO(oSB, this.getPosXE() + this.getWidthE() - CFG.PADD * 2 - this.getTextWidthU() - (int)((float)IMGManager.getIMG(Images.diploArmy).getWidth() * this.getImageScale(1.0f, Images.diploArmy)) + iTranslateX, this.getPosY() + this.getHeightE() - CFG.PADD * 2 - this.getSliderHeight() - (int)((float)IMGManager.getIMG(Images.diploArmy).getHeight() * this.getImageScale(1.0f, Images.diploArmy)) - IMGManager.getIMG(Images.diploArmy).getHeight() + iTranslateY, (int)((float)IMGManager.getIMG(Images.diploArmy).getWidth() * this.getImageScale(1.0f, Images.diploArmy)), (int)((float)IMGManager.getIMG(Images.diploArmy).getHeight() * this.getImageScale(1.0f, Images.diploArmy)));
            
            Renderer.drawText(oSB, this.fontID, this.getTextE(), this.getPosXE() + CFG.PADD + iTranslateX, this.getPosY() + this.getHeightE() - CFG.PADD * 2 - this.getSliderHeight() - this.getTextHeight() + iTranslateY, this.getColor(isActive));
            Renderer.drawText(oSB, this.fontID, this.getDrawText(), this.getPosXE() + this.getWidthE() - CFG.PADD - this.getTextWidthU() + iTranslateX, this.getPosY() + this.getHeightE() - CFG.PADD * 2 - this.getSliderHeight() - this.getTextHeight() + iTranslateY, primary);
        ...
        Renderer.drawTextWithShadow(oSB, this.fontID, this.getDrawText(), this.getPosXE() + this.getWidthE() - CFG.PADD - this.getTextWidthU() + iTranslateX, this.getPosY() + this.getHeightE() - CFG.PADD * 2 - this.getSliderHeight() - this.getTextHeight() + iTranslateY, this.getCurr() == 0 ? CFG.COLOR_NEUTRAL : CFG.COLOR_TEXT_NUM_OF_PROVINCES);
    }
