 ...
 */
package age.of.civilizations2.jakowski.lukasz.Menus.Send.Army;
...
public class Menu_InGame_SendArmy
extends Menu {
    public static int fromProvinceID = -1;
    public static int toProvinceID = -1;
    public static long armyToSend = 0;

    public final int getElementW2() {
        return this.getWidthM();
    }

    public Menu_InGame_SendArmy(int nFromProvinceID) {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        fromProvinceID = nFromProvinceID;
        long maxArmy = CFG.core.getProv(fromProvinceID).getArmyCivID1(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId());
        if (maxArmy <= 0) {
            fromProvinceID = -1;
        }
        ...
        menuElements.add(new TextBuildTitle(CFG.lang.get("FromProvince"), -1, 0, tY, CFG.BUTTON_W, CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 4){
            ...
            public int getWidthE() {
                return Menu_InGame_SendArmy.this.getElementW2();
            }
        });
        ...
        if (fromProvinceID < 0) {
            ...
            menuElements.add(new ButtonN_Pop(new Color((float)CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getR() / 255.0f, (float)CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getG() / 255.0f, (float)CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getB() / 255.0f, 1.0f), CFG.core.getProv(fromProvinceID).getProvName(), CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), CFG.lang.get("Army") + ": ", CFG.getNumberWthSpaces("" + CFG.core.getProv(fromProvinceID).getArmyCivID1(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId())), Images.diploArmy, CFG.COLOR_TEXT_NUM_OF_PROVINCES, 0, tY, CFG.BUTTON_W){
                ...
                public void buildElemHover() {
                    ...
                    nData.add(new ME_Hover_2Type_Text("" + CFG.core.getProv(fromProvinceID).getArmyCivID1(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()), CFG.COLOR_HOVER_TITLE));
        ...
        menuElements.add(new TextBuildTitle(CFG.lang.get("ToProvince"), -1, 0, tY, CFG.BUTTON_W, CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 4){
        ...
        if (toProvinceID < 0) {
            ...
            tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
            menuElements.add(new Slider_InGame_Army(CFG.lang.get("SupportWithTroops"), CFG.PADD * 2, ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE(), tempWidth - CFG.PADD * 3 - CFG.BUTTON_W, Math.max(CFG.BUTTON_H * 3 / 4, CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 6), 0, maxArmy, armyToSend, 0.65f){

                @Override
                public void actionElem(int iID) {
                    armyToSend = this.getCurr();
                }

                @Override
                public int getWidthE() {
                    return Menu_InGame_SendArmy.this.getElementW2() - CFG.PADD * 4;
                }
        ...
        menuElements.add(new Text_Desc(CFG.lang.get("SendVolunteerArmyText"), CFG.PADD, tY, tempWidth - CFG.PADD * 2){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_SendArmy.this.getElementW2() - CFG.PADD * 2;
            }
        });
        tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
        menuElements.add(new Button_InGameAction(CFG.lang.get("Cancel"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_SendArmy.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }

            @Override
            public void actionElem(int iID) {
                Menu_InGame_SendArmy.this.setVisibleM(false);
            }
        });
        menuElements.add(new Button_InGameAction(CFG.lang.get("Confirm"), -1, 2, tY, CFG.BUTTON_W, true){
            ...
            @Override
            public int getPosXE() {
                return Menu_InGame_SendArmy.this.getElementW() + CFG.PADD / 2;
            }
            ...
            @Override
            public void actionElem(int iID) {
                Menu_InGame_SendArmy.this.sendArmy();
                Menu_InGame_SendArmy.this.setVisibleM(false);
            }

            @Override
            public void drawTextE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
                IMGManager.getIMG(Images.diploArmySend).drawO(oSB, this.getPosXE() + this.getWidthE() / 2 - (this.getTextWidthU() + IMGManager.getIMG(Images.diploArmySend).getWidth() + CFG.PADD) / 2 + iTranslateX, this.getPosY() + this.getHeightE() / 2 - IMGManager.getIMG(Images.diploArmySend).getHeight() / 2 + iTranslateY);
                Renderer.drawText(oSB, this.fontID, this.getTextE(), this.getPosXE() + (this.getTextPosElem() < 0 ? this.getWidthE() / 2 - (this.getTextWidthU() + IMGManager.getIMG(Images.diploArmySend).getWidth() + CFG.PADD) / 2 + IMGManager.getIMG(Images.diploArmySend).getWidth() + CFG.PADD : this.getTextPosElem()) + iTranslateX, this.getPosY() + this.getHeightE() / 2 - this.getTextHeight() / 2 + iTranslateY, this.getColorE(isActive));
            }
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("SendVolunteerArmy"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                oSB.setColor(new Color(0.23529412f, 0.4117647f, 0.6666667f, 0.165f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, nPosX + iTranslateX, nPosY - this.getHeightT() + 2 - IMGManager.getIMG(Images.line32Off1).getHeight(), nWidth, this.getHeightT() - 2, false, true);
                oSB.setColor(new Color(0.23529412f, 0.4117647f, 0.6666667f, 0.375f));
                ...
                CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getFlagC().drawO(oSB, Menu_InGame_SendArmy.this.getPosX() + CFG.PADD * 2 + iTranslateX, Menu_InGame_SendArmy.this.getPosY() - this.getHeightT() / 2 - CFG.CIV_FLAG_HEIGHT / 2 - CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getFlagC().getHeight(), CFG.CIV_FLAG_WIDTH, CFG.CIV_FLAG_HEIGHT);
                IMGManager.getIMG(Images.flagRectSmall).drawO(oSB, Menu_InGame_SendArmy.this.getPosX() + CFG.PADD * 2 + iTranslateX, Menu_InGame_SendArmy.this.getPosY() - this.getHeightT() / 2 - CFG.CIV_FLAG_HEIGHT / 2);
                int imgID = Images.diploArmySend;
                IMGManager.getIMG(imgID).drawO(oSB, nPosX + (nWidth - (this.getTextWidth() + IMGManager.getIMG(imgID).getWidth() + CFG.PADD)) / 2 + iTranslateX, Menu_InGame_SendArmy.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(imgID).getHeight() / 2);
        ...
        if (sliderButtonID >= 0) {
            ((MenuElemUI)menuElements.get(sliderButtonID)).setCurr(armyToSend);
        }
    }
    ...
    public void draw(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean sliderMenuIsActive) {
        ...
        try {
            if (fromProvinceID < 0) {
                if (CFG.core.getActiveProvID() >= 0 && CFG.core.getProv(CFG.core.getActiveProvID()).getArmyCivID1(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()) > 0) {
                    fromProvinceID = CFG.core.getActiveProvID();
                    CFG.menus.rebuildInGame_SendArmy(fromProvinceID);
                }
            } else if (CFG.core.getActiveProvID() >= 0 && CFG.core.getActiveProvID() != toProvinceID && CFG.core.getActiveProvID() != fromProvinceID && CFG.core.getProv(CFG.core.getActiveProvID()).getCivId() != CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId() && CFG.core.getProv(CFG.core.getActiveProvID()).getCivId() > 0) {
    ...
    public void sendArmy() {
        ...
        } else if (armyToSend <= 0) {
            CFG.toastM.addM(CFG.lang.get("SendVolunteerArmy") + ": " + armyToSend, CFG.COLOR_NEGATIVE_2);
        } else {
            long army = Math.min((long)armyToSend, CFG.core.getProv(fromProvinceID).getArmyCivID1(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()));
            GameManager.sendVolunteerArmy(armyToSend, CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), fromProvinceID, toProvinceID);
            CFG.menus.updateInGameTopAll(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId());
            CFG.menus.setVisible_Menu_InGame_CurrentWars(true);
            CFG.menus.rebuildMenu_InGame_Infobox(CFG.lang.get("VolunteerArmyHasBeenSent"), CFG.lang.get("Army") + ": " + CFG.getNumberWthSpaces("" + army), Images.infoDiplomacy);
        }
    }
}
