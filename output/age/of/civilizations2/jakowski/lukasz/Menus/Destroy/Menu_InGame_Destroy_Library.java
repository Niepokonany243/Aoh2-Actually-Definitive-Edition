...
public class Menu_InGame_Destroy_Library
extends Menu {
    ...
    public Menu_InGame_Destroy_Library(int nProvinceID) {
        ...
        menuElements.add(new Button_Build_Building_Destroy(CFG.core.getProv(this.iProvinceID).getName().length() > 0 ? CFG.core.getProv(this.iProvinceID).getName() : CFG.lang.get("Province"), CFG.lang.get("Destroy") + ": " + CFG.lang.get(BuildingsManager.getLibrary_Name(CFG.core.getProv(this.iProvinceID).getLvlOfLibrary())), Images.bLibrary, 0, tY, CFG.BUTTON_W * 2, true){
            ...
            public void buildElemHover() {
                ...
                nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get(BuildingsManager.getLibrary_Name(CFG.core.getProv(Menu_InGame_Destroy_Library.this.iProvinceID).getLvlOfLibrary())) + ": ", CFG.COLOR_TEXT_NUM_OF_PROVINCES));
                nData.add(new ME_Hover_2Type_Text_Big(CFG.core.getProv(Menu_InGame_Destroy_Library.this.iProvinceID).getName().length() > 0 ? CFG.core.getProv(Menu_InGame_Destroy_Library.this.iProvinceID).getName() : CFG.lang.get("Province")));
                nData.add(new ME_Hover_2Type_Flag_Big(CFG.core.getProv(Menu_InGame_Destroy_Library.this.iProvinceID).getCivId(), CFG.PADD, 0));
                ...
                nData.add(new ME_Hover_2Type_Text(CFG.lang.get("ResearchPerTurnForEveryXPeopleInProvince", BuildingsManager.getLibrary_ResearchPerPopulation(CFG.core.getProv(Menu_InGame_Destroy_Library.this.iProvinceID).getLvlOfLibrary())), CFG.COLOR_HOVER_TITLE));
                nElements.add(new MEHover_2E(nData));
                nData.clear();
        ...
        menuElements.add(new Button_InGameAction(CFG.lang.get("Cancel"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_Destroy_Library.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }
        });
        menuElements.add(new Button_InGameAction(CFG.lang.get("Destroy"), -1, 2, tY, CFG.BUTTON_W, true){
            ...
            @Override
            public int getPosXE() {
                return Menu_InGame_Destroy_Library.this.getElementW() + CFG.PADD / 2;
            }
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get(BuildingsManager.getLibrary_Name(CFG.core.getProv(this.iProvinceID).getLvlOfLibrary())), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getFlagC().drawO(oSB, Menu_InGame_Destroy_Library.this.getPosX() + CFG.PADD * 2 + iTranslateX, Menu_InGame_Destroy_Library.this.getPosY() - this.getHeightT() / 2 - CFG.CIV_FLAG_HEIGHT / 2 - CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getFlagC().getHeight(), CFG.CIV_FLAG_WIDTH, CFG.CIV_FLAG_HEIGHT);
                IMGManager.getIMG(Images.flagRectSmall).drawO(oSB, Menu_InGame_Destroy_Library.this.getPosX() + CFG.PADD * 2 + iTranslateX, Menu_InGame_Destroy_Library.this.getPosY() - this.getHeightT() / 2 - CFG.CIV_FLAG_HEIGHT / 2);
    ...
    public final void actionEL(int iID) {
        if (iID == this.getMenuElemsSize() - 1) {
            if (BuildingsManager.destroyLibrary(this.iProvinceID, CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId())) {
