...
import age.of.civilizations2.jakowski.lukasz.Button.Diplomacy.Button_Diplomacy_Plunder;
...
public class Menu_InGame_Plunder
extends Menu {
    public static int iProvinceID = -1;

    public Menu_InGame_Plunder() {
    ...
    public Menu_InGame_Plunder(final int nProvinceID) {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        iProvinceID = nProvinceID;
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        menuElements.add(new Button_Diplomacy_Plunder("" + (float)CFG.ideologiesMgr.getIdeologyID((int)CFG.core.getCiv((int)CFG.core.getPlayer((int)CFG.PLAYER_TURN_ID).getCivId()).getIdeology()).COST_OF_PLUNDER / 10.0f, CFG.core.getProv(nProvinceID).getPop().getPops(), Plunder.plunderProvinceIncome(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), nProvinceID, 0), 0, tY, CFG.BUTTON_W * 2){

            @Override
            public int getWidthE() {
                return Menu_InGame_Plunder.this.getElementW2();
            }
        ...
        tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
        long currArmyPlunder = 0L;
        for (int i = 0; i < CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getMoveUnitsPlunderSize(); ++i) {
            if (CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getMoveUnitsPlunder(i).getFromProvinceID() != nProvinceID) continue;
            currArmyPlunder = CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getMoveUnitsPlunder(i).getNumOfUnits();
            break;
        }
        menuElements.add(new Slider_InGame_Clear(CFG.lang.get("Army"), CFG.PADD * 2, ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, tempWidth - CFG.PADD * 3 - CFG.BUTTON_W, CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 2 + CFG.PADD * 4, 0, CFG.core.getProv(nProvinceID).getArmyCivID1(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()) + currArmyPlunder, currArmyPlunder == 0 ? CFG.core.getProv(nProvinceID).getArmyCivID1(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()) : currArmyPlunder, 0.65f){

            @Override
            public int getWidthE() {
                return Menu_InGame_Plunder.this.getElementW() * 2 - CFG.PADD * 4;
            }
            ...
            @Override
            public void actionElem(int iID) {
                Menu_InGame_Plunder.this.getMenuElem(0).setCurr((int)(Plunder.plunderEfficiency(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), nProvinceID, this.getCurr()) * 100.0f));
            }
        });
        ...
        menuElements.add(new Button_InGameAction(CFG.lang.get("Cancel"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_Plunder.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }
        });
        menuElements.add(new Button_InGameAction(CFG.lang.get("Confirm"), -1, 2, tY, CFG.BUTTON_W, true){
            ...
            @Override
            public int getPosXE() {
                return Menu_InGame_Plunder.this.getElementW() + CFG.PADD / 2;
            }
            ...
            public void buildElemHover() {
                ...
                nData.add(new ME_Hover_2Type_Flag_Big(CFG.core.getProv(iProvinceID).getTrueOwnerOfProv()));
                nData.add(new ME_Hover_2Type_Text_Big(Menu_InGame_Plunder.this.getTitleM().getText(), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
                nData.add(new ME_Hover_2Type_Image_Big(Images.diploPlunder, CFG.PADD, 0));
                nElements.add(new MEHover_2E(nData));
                nData.clear();
                nData.add(new ME_Hover_2Type_Text(CFG.lang.get("Army") + ": "));
                nData.add(new ME_Hover_2Type_Text("" + Menu_InGame_Plunder.this.getMenuElem(1).getCurr(), CFG.COLOR_HOVER_TITLE));
                ...
                nData.add(new ME_Hover_2Type_Text("+" + Plunder.plunderTreasuryIncome(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), iProvinceID, Menu_InGame_Plunder.this.getMenuElem(1).getCurr()), CFG.COLOR_GOLD));
                nData.add(new ME_Hover_2Type_Image(Images.topGold(), CFG.PADD, 0));
                nElements.add(new MEHover_2E(nData));
                nData.clear();
                nData.add(new ME_Hover_2Type_Text(CFG.lang.get("Economy") + ": "));
                nData.add(new ME_Hover_2Type_Text("-" + (float)((int)(Plunder.plunder_LossesEconomy_Perc(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), iProvinceID, Menu_InGame_Plunder.this.getMenuElem(1).getCurr()) * 10000.0f)) / 100.0f + "%", CFG.COLOR_NEGATIVE_2));
                nData.add(new ME_Hover_2Type_Image(Images.economy, CFG.PADD, 0));
                nElements.add(new MEHover_2E(nData));
                nData.clear();
                nData.add(new ME_Hover_2Type_Text(CFG.lang.get("Happiness") + ": "));
                nData.add(new ME_Hover_2Type_Text("-" + (float)((int)(Plunder.plunder_Happiness(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), iProvinceID, Menu_InGame_Plunder.this.getMenuElem(1).getCurr()) * 10000.0f)) / 100.0f + "%", CFG.COLOR_NEGATIVE_2));
            ...
            public void drawTextE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
                IMGManager.getIMG(Images.diploPlunder).drawO(oSB, this.getPosXE() + this.getWidthE() / 2 - (this.getTextWidthU() + IMGManager.getIMG(Images.diploPlunder).getWidth() + CFG.PADD) / 2 + iTranslateX, this.getPosY() + this.getHeightE() / 2 - IMGManager.getIMG(Images.diploPlunder).getHeight() / 2 + iTranslateY);
                Renderer.drawText(oSB, this.fontID, this.getTextE(), this.getPosXE() + (this.getTextPosElem() < 0 ? this.getWidthE() / 2 - (this.getTextWidthU() + IMGManager.getIMG(Images.diploPlunder).getWidth() + CFG.PADD) / 2 + IMGManager.getIMG(Images.diploPlunder).getWidth() + CFG.PADD : this.getTextPosElem()) + iTranslateX, this.getPosY() + this.getHeightE() / 2 - this.getTextHeight() / 2 + iTranslateY, this.getColorE(isActive));
            }
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("Plunder") + (CFG.core.getProv(nProvinceID).getName().length() > 0 ? ": " + CFG.core.getProv(nProvinceID).getName() : ""), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                CFG.core.getCiv(CFG.core.getProv(iProvinceID).getTrueOwnerOfProv()).getFlagC().drawO(oSB, nPosX + CFG.PADD * 2 + iTranslateX, nPosY - this.getHeightT() + this.getHeightT() / 2 + 1 - CFG.CIV_FLAG_HEIGHT / 2 - CFG.core.getCiv(CFG.core.getProv(iProvinceID).getTrueOwnerOfProv()).getFlagC().getHeight(), CFG.CIV_FLAG_WIDTH, CFG.CIV_FLAG_HEIGHT);
        ...
        this.updateLang();
        this.getMenuElem(1).setCurr(this.getMenuElem(1).getCurr());
        this.getMenuElem(0).setCurr((int)(Plunder.plunderEfficiency(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), nProvinceID, this.getMenuElem(1).getCurr()) * 100.0f));
