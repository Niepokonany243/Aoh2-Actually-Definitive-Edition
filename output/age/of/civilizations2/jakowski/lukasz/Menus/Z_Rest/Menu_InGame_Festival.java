...
import age.of.civilizations2.jakowski.lukasz.Button.Build.Button_Build_Festival;
...
public class Menu_InGame_Festival
extends Menu {
    private int iProvinceID = -1;

    public Menu_InGame_Festival() {
    ...
    public Menu_InGame_Festival(int nProvinceID) {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        this.iProvinceID = nProvinceID;
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        menuElements.add(new Button_Build_Festival(CFG.lang.get("OrganizeAFestivalIn") + ": ", CFG.core.getProv(this.iProvinceID).getName().length() > 0 ? CFG.core.getProv(this.iProvinceID).getName() : CFG.lang.get("Province"), GameCalendar.getCurrDate() + " - " + GameCalendar.getDate_ByTurnID(GameCalendar.TURNID + GameValues.gvFestival.FESTIVAL_NUM_OF_TURNS), Images.diploFestival, Festival.festivalCost(BuildingsManager.iBuildInProvinceID), GameValues.gvFestival.COST_FESTIVAL_MOVEMENT_POINTS, 0, tY, CFG.BUTTON_W * 2){

            @Override
            public int getWidthE() {
                return Menu_InGame_Festival.this.getElementW2();
            }
            ...
            public void buildElemHover() {
                ...
                nData.add(new ME_Hover_2Type_Text("+" + (float)((int)(Festival.festivalHappinessPerTurn(Menu_InGame_Festival.this.iProvinceID) * 10000.0f)) / 100.0f, CFG.COLOR_POSITIVE));
        ...
        menuElements.add(new Text_Desc(CFG.lang.get("FestivalDesc"), CFG.PADD, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE(), tempWidth - CFG.PADD * 2){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_Festival.this.getElementW2() - CFG.PADD * 2;
            }
        });
        ...
        menuElements.add(new Button_InGameAction(CFG.lang.get("Cancel"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_Festival.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }
        });
        menuElements.add(new Button_InGameAction(CFG.lang.get("Confirm"), -1, 2, tY, CFG.BUTTON_W, true){
            ...
            @Override
            public int getPosXE() {
                return Menu_InGame_Festival.this.getElementW() + CFG.PADD / 2;
            }
            ...
            @Override
            public void buildElemHover() {
                this.menuElemHover = Menu_InGame_Festival.getHoverFestival(Menu_InGame_Festival.this.iProvinceID);
            }

            @Override
            public boolean getIsClickable() {
                return CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getMovemPoints() >= GameValues.gvFestival.COST_FESTIVAL_MOVEMENT_POINTS;
            }
        });
        int tempMenuPosY = IMGManager.getIMG(Images.topBar).getHeight() + CFG.PADD * 2 + CFG.BUTTON_H * 3 / 4;
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("Festival"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getFlagC().drawO(oSB, Menu_InGame_Festival.this.getPosX() + CFG.PADD * 2 + iTranslateX, Menu_InGame_Festival.this.getPosY() - this.getHeightT() / 2 - CFG.CIV_FLAG_HEIGHT / 2 - CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getFlagC().getHeight(), CFG.CIV_FLAG_WIDTH, CFG.CIV_FLAG_HEIGHT);
                IMGManager.getIMG(Images.flagRectSmall).drawO(oSB, Menu_InGame_Festival.this.getPosX() + CFG.PADD * 2 + iTranslateX, Menu_InGame_Festival.this.getPosY() - this.getHeightT() / 2 - CFG.CIV_FLAG_HEIGHT / 2);
    ...
    public final void actionEL(int iID) {
        if (iID == this.getMenuElemsSize() - 1) {
            if (Festival.addFestival(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), this.iProvinceID)) {
    ...
    public static ME_Hover_v2 getHoverFestival(int provinceID) {
        ArrayList<MEHover_2E> nElements = new ArrayList<MEHover_2E>();
        ArrayList<ME_Hover_2Type> nData = new ArrayList<ME_Hover_2Type>();
        ...
        nData.clear();
        nData.add(new ME_Hover_2Type_Text(CFG.lang.get("Happiness") + ": "));
        nData.add(new ME_Hover_2Type_Text("+" + (float)((int)(Festival.festivalHappinessPerTurn(provinceID) * 10000.0f)) / 100.0f, CFG.COLOR_POSITIVE));
        nData.add(new ME_Hover_2Type_Image(Images.happiness, CFG.PADD, CFG.PADD));
        nData.add(new ME_Hover_2Type_Text(CFG.lang.get("PerTurn"), CFG.COLOR_NEUTRAL));
        nElements.add(new MEHover_2E(nData));
        nData.clear();
        ...
        nData.add(new ME_Hover_2Type_Text("" + Festival.festivalCost(provinceID), CFG.COLOR_GOLD));
