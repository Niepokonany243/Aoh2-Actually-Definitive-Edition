...
extends Menu {
    ...
    public Menu_InGame_RecruitedArmy(int tInit) {
        ...
        this.initMenu(null, Menu_InGame_Stats.getMenuX() - tempWidth - CFG.PADD * 2, Menu_InGame_Stats.getMenuY(), tempWidth, CFG.GAMEHEIGHT * 3 / 5, menuElements, false, false);
    }

    public Menu_InGame_RecruitedArmy() {
        int i;
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        ...
        menuElements.add(new Button_Stats_Title(CFG.lang.get("Civilizations"), CFG.PADD * 2, 2, 0, CFG.BUTTON_W * 2, tElemHeight){
            ...
            public int getWidthE() {
                return Menu_InGame_RecruitedArmy.this.getElementW() * 2 + CFG.PADD * 2 - 2;
            }
        ...
        menuElements.add(new Button_Stats_Title(CFG.lang.get("RecruitedArmy"), CFG.PADD, CFG.PADD * 2 + CFG.BUTTON_W * 5, 0, CFG.BUTTON_W, tElemHeight){

            @Override
            public int getPosXE() {
                return Menu_InGame_RecruitedArmy.this.getElementW() * 2 + CFG.PADD * 2;
            }

            @Override
            public int getWidthE() {
                return Menu_InGame_RecruitedArmy.this.getW() - Menu_InGame_RecruitedArmy.this.getElementW() * 2 + CFG.PADD * 2 - 2;
            }
            ...
            public void buildElemHover() {
                ...
                ArrayList<ME_Hover_2Type> nData = new ArrayList<ME_Hover_2Type>();
                nData.add(new ME_Hover_2Type_Text(CFG.lang.get("SortBy") + ": "));
                nData.add(new ME_Hover_2Type_Text(CFG.lang.get("RecruitedArmy"), CFG.COLOR_HOVER_TITLE));
                nElements.add(new MEHover_2E(nData));
                nData.clear();
                this.menuElemHover = new ME_Hover_v2(nElements);
            }
        ...
        if (iSort == 0) {
            ...
            ArrayList<Long> tempScore = new ArrayList<Long>();
            ...
            while (tempIDS.size() > 0) {
                ...
                for (int i3 = 1; i3 < tempIDS.size(); ++i3) {
                    if (tempScore.get(tAddID).longValue() >= tempScore.get(i3).longValue()) continue;
                    tAddID = i3;
                }
        ...
        for (i = 0; i < iSize; ++i) {
            if (CFG.FOG_OF_WAR == 2 && !CFG.SANDBOX_MODE) {
    ...
    public void updateLang() {
        try {
            this.getTitleM().setText(CFG.lang.get("RecruitedArmy"));
        }
