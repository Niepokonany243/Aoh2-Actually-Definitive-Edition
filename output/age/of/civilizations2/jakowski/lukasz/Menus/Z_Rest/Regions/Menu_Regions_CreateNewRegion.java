...
public class Menu_Regions_CreateNewRegion
extends Menu {
    private String sName;
    private int iNameWidth;

    public Menu_Regions_CreateNewRegion() {
        ...
        menuElements.add(new Button_Classic("", -1, CFG.BUTTON_W + CFG.PADD * 2, 0, CFG.GAMEWIDTH - (CFG.BUTTON_W + CFG.PADD * 2) * 2, CFG.BUTTON_H + CFG.PADD * 2, true){
            ...
            public String getTextToDrawElem() {
                return Menu_Regions_CreateNewRegion.this.sName + ": " + super.getTextE();
            }

            @Override
            public int getTextWidthU() {
                return super.getTextWidthU() + Menu_Regions_CreateNewRegion.this.iNameWidth;
            }
        ...
        menuElements.add(new Button_Game(null, -1, CFG.PADD, CFG.BUTTON_H + CFG.PADD * 3, CFG.BUTTON_W * 2){
            ...
            @Override
            public Color getColorE(boolean isActive) {
                return isActive ? new Color(CFG.editor_Region_GameData.getR(), CFG.editor_Region_GameData.getG(), CFG.editor_Region_GameData.getB(), 1.0f) : (this.getIsClickable() ? new Color(0.38f, 0.38f, 0.38f, 1.0f) : new Color(0.49f, 0.49f, 0.49f, 0.5f));
            }
        });
        ...
        switch (iID) {
            ...
            case 2: {
                if (this.getMenuElem(1).getTextE().length() > 0) {
                    CFG.editor_Region_GameData.setName(this.getMenuElem(1).getTextE());
                    CFG.core.saveRegionPackagesData();
                    this.onBackPressed();
                } else {
                    CFG.showKeyboard(1);
