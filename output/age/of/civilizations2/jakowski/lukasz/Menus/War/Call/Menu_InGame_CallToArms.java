...
public class Menu_InGame_CallToArms
extends Menu {
    public static long lTime = 0L;
    private int iOnCivID = -1;
    private int warAgainstCivID;

    public Menu_InGame_CallToArms() {
        ...
        this.initMenu(new TitleM(CFG.lang.get("CallToArms"), CFG.BUTTON_H * 3 / 5, true, true), CFG.GAMEWIDTH / 2 - tempWidth / 2, tempMenuPosY, tempWidth, ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD + tempMenuPosY > CFG.GAMEHEIGHT - CFG.BUTTON_H - CFG.PADD * 2 ? Math.max(CFG.GAMEHEIGHT - CFG.BUTTON_H - CFG.PADD * 2 - tempMenuPosY, (CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 2) * 6) : ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, menuElements, false, true);
        this.updateLang();
    }

    public Menu_InGame_CallToArms(int onCivID) {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        this.iOnCivID = onCivID;
        this.warAgainstCivID = this.warAgainstCivID;
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        menuElements.add(new Button_Diplomacy_CallToArms(Images.diploRivals, CFG.lang.get("RequestHelpInTheWarAgainst") + ": ", onCivID, 2, tY, CFG.BUTTON_W * 2, GameValues.gvDipCallToArms.COST_CALL_TO_ARMS_DIPLOMACY_POINTS){

            @Override
            public int getWidthE() {
                return Menu_InGame_CallToArms.this.getElementW() * 2;
            }
        });
        tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
        boolean canSend = true;
        List<Integer> alliesToCall = GameManager.callToArmsListOfCivs(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), onCivID);
        if (alliesToCall.size() > 0) {
            for (int i = 0; i < alliesToCall.size(); ++i) {
                menuElements.add(new Button_Diplomacy_CallAllies(i, (int)alliesToCall.get(i), onCivID, 2, tY, CFG.BUTTON_W * 2){
        ...
        menuElements.add(new Button_InGameAction(CFG.lang.get("Cancel"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_CallToArms.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }
        });
        menuElements.add(new Button_InGameAction(CFG.lang.get("Confirm"), -1, 2, tY, CFG.BUTTON_W, canSend){
            ...
            @Override
            public int getPosXE() {
                return Menu_InGame_CallToArms.this.getElementW() + CFG.PADD / 2;
            }
            ...
            public void buildElemHover() {
                ...
                nData.add(new ME_Hover_2Type_Text("-" + (float)GameValues.gvDipCallToArms.COST_CALL_TO_ARMS_DIPLOMACY_POINTS / 10.0f, CFG.COLOR_NEGATIVE_2));
            ...
            public boolean getIsClickable() {
                return CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getDiploPoints() >= GameValues.gvDipCallToArms.COST_CALL_TO_ARMS_DIPLOMACY_POINTS && super.getIsClickable();
            }
    ...
    public final void actionEL(int iID) {
        if (iID == this.getMenuElemsSize() - 1) {
            ...
            for (i = 1; i < this.getMenuElemsSize() - 2; ++i) {
