...
import age.of.civilizations2.jakowski.lukasz.Button.View.Button_View_PopulationPerc;
...
public class Menu_InGame_View_PopulationChange
extends Menu {
    public static long lTime = 0L;
    public static boolean hideAnimation = true;

    public Menu_InGame_View_PopulationChange() {
        int tempW = CFG.CIV_INFO_MENU_WIDTH;
        int tY = 0;
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        menuElements.add(new Button_DiplomacyAction(Images.pop, CFG.lang.get("WorstProvinces"), 0, 0, tY, tempW, Menu_InGame_Civ_Decisions.getButtonH(), true){

            @Override
            public void actionElem(int iID) {
                CFG.menus.setVisible_InGame_ViewPopulationChangeWorst(true);
            }
            ...
            public void buildElemHover() {
                ...
                nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("WorstProvinces"), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
        ...
        menuElements.add(new Button_DiplomacyAction(Images.pop, CFG.lang.get("TopCivilizations"), 0, 0, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE(), tempW, Menu_InGame_Civ_Decisions.getButtonH(), true){

            @Override
            public void actionElem(int iID) {
                CFG.menus.setVisible_InGame_ViewPopulationChangeCivs(true);
            }
        ...
        menuElements.add(new TextBuildTitle(CFG.lang.get("TopProvinces"), -1, 0, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE(), tempW, CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 4){
        ...
        for (int i = 0; i < CFG.core.getProvinSize(); ++i) {
            CFG.core.getProv((int)i).freeValue = (int)(CFG.core.getProv(i).getCivId() == 0 ? 0L : CFG.core.getProv(i).getPop().getPops() - CFG.core.getProv((int)i).provGD.startingPopulation);
        }
        for (int a = 0; a < GameValues.gvInGame.POP_CHANGE_VIEW_LIMIT_PROVINCES; ++a) {
            int bestID = 0;
            for (int i = 0; i < CFG.core.getProvinSize(); ++i) {
                if (CFG.core.getProv((int)bestID).freeValue >= CFG.core.getProv((int)i).freeValue) continue;
                bestID = i;
            }
            if (CFG.core.getProv((int)bestID).freeValue == 0) break;
            menuElements.add(new Button_View_PopulationPerc(a, a + 1 + ". " + (CFG.core.getProv(bestID).getName().length() > 0 ? CFG.core.getProv(bestID).getName() : CFG.core.getCiv(CFG.core.getProv(bestID).getCivId()).getCivName()), bestID, 0, 0, tY, tempW, false){
            ...
            tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
            CFG.core.getProv((int)bestID).freeValue = 0;
        }
        if (elemBefore == menuElements.size()) {
            menuElements.add(new TextScale(CFG.lang.get("NoData"), -1, 0, tY, tempW, CFG.BUTTON_H * 3 / 4, 0.75f));
            tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
        }
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("PopulationChange"), CFG.BUTTON_H * 3 / 5, false, false){

            @Override
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                IMGManager.getIMG(Images.gameTopEdgeTitle).draw2O(oSB, Menu_InGame_View_PopulationChange.this.getPosX() + iTranslateX, Menu_InGame_View_PopulationChange.this.getPosY() - Core.PADDING - IMGManager.getIMG(Images.gameTopEdgeTitle).getHeight() - this.getHeightT(), Menu_InGame_View_PopulationChange.this.getWidthM() + 2 + Core.PADDING, this.getHeightT() + Core.PADDING, true, false);
                ...
                IMGManager.getIMG(Images.pix255).drawO(oSB, Menu_InGame_View_PopulationChange.this.getPosX() + iTranslateX, Menu_InGame_View_PopulationChange.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight(), Menu_InGame_View_PopulationChange.this.getWidthM());
                oSB.setColor(new Color(CFG.COLOR_FLAG_FRAME.r, CFG.COLOR_FLAG_FRAME.g, CFG.COLOR_FLAG_FRAME.b, 0.4f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, Menu_InGame_View_PopulationChange.this.getPosX() + iTranslateX, Menu_InGame_View_PopulationChange.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.line32Off1).getHeight(), Menu_InGame_View_PopulationChange.this.getWidthM(), 1);
                oSB.setColor(new Color(0.0f, 0.0f, 0.0f, 0.6f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, Menu_InGame_View_PopulationChange.this.getPosX() + iTranslateX, Menu_InGame_View_PopulationChange.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.line32Off1).getHeight() - 1, Menu_InGame_View_PopulationChange.this.getWidthM(), 1);
                oSB.setColor(new Color(0.0f, 0.0f, 0.0f, 0.8f));
                IMGManager.getIMG(Images.sliderGradient).drawO(oSB, Menu_InGame_View_PopulationChange.this.getPosX() + iTranslateX, Menu_InGame_View_PopulationChange.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.sliderGradient).getHeight(), Menu_InGame_View_PopulationChange.this.getWidthM() / 4, 1);
                IMGManager.getIMG(Images.sliderGradient).drawO(oSB, Menu_InGame_View_PopulationChange.this.getPosX() + Menu_InGame_View_PopulationChange.this.getWidthM() - Menu_InGame_View_PopulationChange.this.getWidthM() / 4 + iTranslateX, Menu_InGame_View_PopulationChange.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.sliderGradient).getHeight(), Menu_InGame_View_PopulationChange.this.getWidthM() / 4, 1, true, false);
                oSB.setColor(Color.WHITE);
                IMGManager.getIMG(Images.pop).drawO(oSB, nPosX + CFG.PADD * 2 + iTranslateX, Menu_InGame_View_PopulationChange.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(Images.pop).getHeight() / 2);
