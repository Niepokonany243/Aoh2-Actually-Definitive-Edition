...
public class Menu_InGame_View_PopulationProvinces
extends Menu {
    ...
    public Menu_InGame_View_PopulationProvinces() {
        int tempW = CFG.CIV_INFO_MENU_WIDTH;
        int tY = 0;
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        menuElements.add(new Button_DiplomacyAction(Images.pop, CFG.lang.get("Population") + ": " + CFG.lang.get("Civilization"), 0, 0, tY, tempW, Menu_InGame_Civ_Decisions.getButtonH(), true){
        ...
        menuElements.add(new Button_DiplomacyAction(Images.pop, CFG.map.getMapName_Just(CFG.map.getActiveMapIDN()) + ": " + CFG.lang.get("WorstProvinces"), 0, 0, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE(), tempW, Menu_InGame_Civ_Decisions.getButtonH(), true){

            @Override
            public void actionElem(int iID) {
                CFG.menus.setVisible_InGame_ViewPopulationProvincesWorst(true);
            }
        ...
        ArrayList<Long> topScore = new ArrayList<Long>();
        for (int i = 0; i < CFG.core.getProvinSize(); ++i) {
            if (CFG.core.getProv(i).getWastelandLvl() >= 0 || CFG.core.getProv(i).getSeaProv()) {
                topScore.add(-100L);
                continue;
            }
            topScore.add(CFG.core.getProv(i).getPop().getPops());
        }
        for (int a = 0; a < GameValues.gvInGame.TOP_PROVINCES_LIMIT; ++a) {
            int bestID = 0;
            for (int i = 0; i < CFG.core.getProvinSize(); ++i) {
                if ((Long)topScore.get(i) <= (Long)topScore.get(bestID)) continue;
                bestID = i;
            }
            if ((Long)topScore.get(bestID) < 0) break;
            menuElements.add(new Button_View_Population(a % 2, a + 1 + ". " + (CFG.core.getProv(bestID).getName().length() > 0 ? CFG.core.getProv(bestID).getName() : CFG.core.getCiv(CFG.core.getProv(bestID).getCivId()).getCivName()), bestID, 0, 0, tY, tempW, (CFG.SPECTATOR_MODE || CFG.core.isAlly(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), CFG.core.getProv(bestID).getCivId())) && CFG.core.getCiv(CFG.core.getProv(bestID).getCivId()).isInConstruction(bestID, ConstructionType.LIBRARY) > 0){
        ...
        this.initMenu(new TitleM_TextSmall(CFG.map.getMapName_Just(CFG.map.getActiveMapIDN()) + ": " + CFG.lang.get("TopProvinces"), CFG.BUTTON_H * 3 / 5, false, false){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                IMGManager.getIMG(Images.gameTopEdgeTitle).draw2O(oSB, Menu_InGame_View_PopulationProvinces.this.getPosX() + iTranslateX, Menu_InGame_View_PopulationProvinces.this.getPosY() - Core.PADDING - IMGManager.getIMG(Images.gameTopEdgeTitle).getHeight() - this.getHeightT(), Menu_InGame_View_PopulationProvinces.this.getWidthM() + 2 + Core.PADDING, this.getHeightT() + Core.PADDING, true, false);
                ...
                IMGManager.getIMG(Images.pix255).drawO(oSB, Menu_InGame_View_PopulationProvinces.this.getPosX() + iTranslateX, Menu_InGame_View_PopulationProvinces.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight(), Menu_InGame_View_PopulationProvinces.this.getWidthM());
                oSB.setColor(new Color(CFG.COLOR_FLAG_FRAME.r, CFG.COLOR_FLAG_FRAME.g, CFG.COLOR_FLAG_FRAME.b, 0.4f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, Menu_InGame_View_PopulationProvinces.this.getPosX() + iTranslateX, Menu_InGame_View_PopulationProvinces.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.line32Off1).getHeight(), Menu_InGame_View_PopulationProvinces.this.getWidthM(), 1);
                oSB.setColor(new Color(0.0f, 0.0f, 0.0f, 0.6f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, Menu_InGame_View_PopulationProvinces.this.getPosX() + iTranslateX, Menu_InGame_View_PopulationProvinces.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.line32Off1).getHeight() - 1, Menu_InGame_View_PopulationProvinces.this.getWidthM(), 1);
                oSB.setColor(new Color(0.0f, 0.0f, 0.0f, 0.8f));
                IMGManager.getIMG(Images.sliderGradient).drawO(oSB, Menu_InGame_View_PopulationProvinces.this.getPosX() + iTranslateX, Menu_InGame_View_PopulationProvinces.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.sliderGradient).getHeight(), Menu_InGame_View_PopulationProvinces.this.getWidthM() / 4, 1);
                IMGManager.getIMG(Images.sliderGradient).drawO(oSB, Menu_InGame_View_PopulationProvinces.this.getPosX() + Menu_InGame_View_PopulationProvinces.this.getWidthM() - Menu_InGame_View_PopulationProvinces.this.getWidthM() / 4 + iTranslateX, Menu_InGame_View_PopulationProvinces.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.sliderGradient).getHeight(), Menu_InGame_View_PopulationProvinces.this.getWidthM() / 4, 1, true, false);
                oSB.setColor(Color.WHITE);
                IMGManager.getIMG(Images.pop).drawO(oSB, nPosX + CFG.PADD * 2 + iTranslateX, Menu_InGame_View_PopulationProvinces.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(Images.pop).getHeight() / 2);
