...
public class Menu_Leader_Edit_SelectCiv_List
extends Menu {
    ...
    public Menu_Leader_Edit_SelectCiv_List() {
        ...
        if (CFG.sSearch != null && CFG.sSearch.length() > 0) {
            ...
            for (i = 0; i < iSize; ++i) {
                if (CFG.lang.getCiv(tagsSPLITED[i]).toLowerCase().indexOf(CFG.sSearch.toLowerCase()) < 0 || CFG.isInLeaderCivs(tagsSPLITED[i])) continue;
                lTempNames.add(CFG.lang.getCiv(tagsSPLITED[i]));
                lTempTags.add(tagsSPLITED[i]);
            }
            iSize = tagsSPLITED_ED.length;
            for (i = 0; i < iSize; ++i) {
                if (CFG.lang.getCiv(tagsSPLITED_ED[i]).toLowerCase().indexOf(CFG.sSearch.toLowerCase()) < 0 || CFG.isInLeaderCivs(tagsSPLITED_ED[i])) continue;
            ...
            int iSize = tagsSPLITED.length;
            for (i = 0; i < iSize; ++i) {
                if (CFG.isInLeaderCivs(tagsSPLITED[i])) continue;
                lTempNames.add(CFG.lang.getCiv(tagsSPLITED[i]));
                lTempTags.add(tagsSPLITED[i]);
            }
            iSize = tagsSPLITED_ED.length;
            for (i = 0; i < iSize; ++i) {
                if (CFG.isInLeaderCivs(tagsSPLITED_ED[i])) continue;
                lTempNames.add(CFG.lang.getCiv(tagsSPLITED_ED[i]));
                lTempTags.add(tagsSPLITED_ED[i]);
            }
            int nPosY = 0;
            ...
            int iSize = tagsSPLITED.length;
            for (i = 0; i < iSize; ++i) {
                if (CFG.lang.getCiv(tagsSPLITED[i]).charAt(0) != CFG.chosenAlphabetCharachter.charAt(0) || CFG.isInLeaderCivs(tagsSPLITED[i])) continue;
                lTempNames.add(CFG.lang.getCiv(tagsSPLITED[i]));
                lTempTags.add(tagsSPLITED[i]);
            }
            iSize = tagsSPLITED_ED.length;
            for (i = 0; i < iSize; ++i) {
                if (CFG.lang.getCiv(tagsSPLITED_ED[i]).charAt(0) != CFG.chosenAlphabetCharachter.charAt(0) || CFG.isInLeaderCivs(tagsSPLITED_ED[i])) continue;
                lTempNames.add(CFG.lang.getCiv(tagsSPLITED_ED[i]));
                lTempTags.add(tagsSPLITED_ED[i]);
            }
            int nPosY = 0;
    ...
    public final void actionEL(int iID) {
        if (iID % 2 == 0) {
            CFG.leaderGameData.addCiv(this.lCivsTags.get(iID / 2));
            this.onBack();
    ...
    private final void onBack() {
        CFG.menus.setMenuID(View.eGAME_LEADERS_EDIT);
    }
