...
public class Menu_Editor_Unions_AddCiv_List
extends Menu {
    ...
    public Menu_Editor_Unions_AddCiv_List() {
        ...
        if (CFG.sSearch != null && CFG.sSearch.length() > 0) {
            ...
            for (i = 0; i < iSize; ++i) {
                if (CFG.lang.getCiv(tagsSPLITED_ED[i]).toLowerCase().indexOf(CFG.sSearch.toLowerCase()) < 0) continue;
                ...
                menuElements.add(new Button_Classic_Classic_Wiki(tID++, CFG.GAMEWIDTH - (CFG.BUTTON_W + CFG.BUTTON_W / 2), CFG.BUTTON_H * nPosY + CFG.PADD * (nPosY + 1), CFG.BUTTON_W + CFG.BUTTON_W / 2, CFG.BUTTON_H, true){
                    ...
                    public void buildElemHover() {
                        try {
                            ...
                            nData.add(new ME_Hover_2Type_Text(CFG.getWikiInforLinkClear((String)Menu_Editor_Unions_AddCiv_List.this.lCivsTags.get(this.getTextPosElem())), CFG.COLOR_HOVER_TITLE));
            ...
            iSize = tagsSPLITED_ED.length;
            for (i = 0; i < iSize; ++i) {
                if (CFG.lang.getCiv(tagsSPLITED_ED[i]).charAt(0) != CFG.chosenAlphabetCharachter.charAt(0)) continue;
                lTempNames.add(CFG.lang.getCiv(tagsSPLITED_ED[i]));
                lTempTags.add(tagsSPLITED_ED[i]);
            }
            int nPosY = 0;
    ...
    public void draw(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean sliderMenuIsActive) {
        ...
        try {
            for (int i = tempRandomButton; i < this.getMenuElemsSize(); i += 2) {
                ...
                this.lFlags.get(this.getFlagID((i - tempRandomButton) / 2)).drawO(oSB, this.getMenuElem(i).getPosXE() + this.getMenuElem(i).getTextPosElem() / 2 - CFG.CIV_FLAG_WIDTH / 2 + iTranslateX, this.getMenuElem(i).getPosY() + this.getMenuPosY() + this.getMenuElem(i).getHeightE() / 2 - CFG.CIV_FLAG_HEIGHT / 2 + iTranslateY);
    ...
    public final void actionEL(int iID) {
        if (CFG.MANAGE_DIPLOMACY_ADD_NEW_PACT_CIV1 == -2) {
            ...
            CFG.unionsManager.createUnion_Data.lCreateCivTag = this.lCivsTags.get(iID / 2);
        } else if (CFG.MANAGE_DIPLOMACY_ADD_NEW_PACT_CIV1 == -1) {
            if (iID % 2 != 0) {
                CFG.EDITOR_ACTIVE_GAMEDATA_TAG = this.lCivsTags.get(iID / 2);
                CFG.setDialogType(DialogType.GO_TO_WIKI);
                return;
            }
            CFG.unionsManager.createUnion_Data.lCivsTags.add(CFG.ideologiesMgr.getRealTag(this.lCivsTags.get(iID / 2)));
        } else {
            if (iID % 2 != 0) {
                CFG.EDITOR_ACTIVE_GAMEDATA_TAG = this.lCivsTags.get(iID / 2);
                CFG.setDialogType(DialogType.GO_TO_WIKI);
            ...
            try {
                CFG.unionsManager.createUnion_Data.lCivsTags.set(CFG.MANAGE_DIPLOMACY_ADD_NEW_PACT_CIV1, CFG.ideologiesMgr.getRealTag(this.lCivsTags.get(iID / 2)));
            }
            catch (IndexOutOfBoundsException indexOutOfBoundsException) {}
        }
        this.onBackPressed();
