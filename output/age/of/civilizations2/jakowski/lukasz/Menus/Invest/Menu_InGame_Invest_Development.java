...
import age.of.civilizations2.jakowski.lukasz.Button.Build.Button_Build_InvestOverPenalty;
import age.of.civilizations2.jakowski.lukasz.Button.Build.Button_Build_Invest_Development;
...
public class Menu_InGame_Invest_Development
extends Menu {
    ...
    public Menu_InGame_Invest_Development() {
        ...
        this.initMenu(new TitleM(CFG.lang.get("Invest"), CFG.BUTTON_H * 3 / 5, true, true), CFG.GAMEWIDTH / 2 - tempWidth / 2, tempMenuPosY, tempWidth, ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD + tempMenuPosY > CFG.GAMEHEIGHT - CFG.BUTTON_H - CFG.PADD * 2 ? Math.max(CFG.GAMEHEIGHT - CFG.BUTTON_H - CFG.PADD * 2 - tempMenuPosY, (CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 2) * 6) : ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, menuElements, false, true);
        this.updateLang();
    }

    public Menu_InGame_Invest_Development(int nProvinceID) {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        iProvinceID = nProvinceID;
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        menuElements.add(new Button_Build_Invest_Development(CFG.lang.get("InvestIn") + ": ", CFG.core.getProv(iProvinceID).getName().length() > 0 ? CFG.core.getProv(iProvinceID).getName() : CFG.lang.get("Province"), Images.investDev, 0, GameValues.gvInvestDevelopment.INVEST_DEVELOPMENT_MOVEMENT_POINTS, 0, tY, CFG.BUTTON_W * 2){

            @Override
            public int getWidthE() {
                return Menu_InGame_Invest_Development.this.getElementW2();
            }
        });
        menuElements.add(new Button_Build_InvestOverPenalty(CFG.lang.get("OverinvestmentPenalty") + ": ", 0, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE(), CFG.BUTTON_W * 2, "+" + CFG.getPrecision2(Core.getOverInvestmentsPenalty(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()) * 100.0f, 100) + "%", CFG.getPrecision2(CFG.core.getCiv((int)CFG.core.getPlayer((int)CFG.PLAYER_TURN_ID).getCivId()).civGD.numberOfInvestments, 10)){
        ...
        menuElements.add(new Text_Desc(CFG.lang.get("EconomyDevelopmentDesc") + " " + CFG.lang.get("Development2") + " " + CFG.lang.get("TechnologyLevel") + ": " + CFG.getPrecision2(CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getTechLevel(), 100), CFG.PADD, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, tempWidth - CFG.PADD * 2){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_Invest_Development.this.getElementW2() - CFG.PADD * 2;
            }
        });
        tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD;
        menuElements.add(new Slider_InGame_Gold(CFG.lang.get("Gold"), CFG.PADD * 2, ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, tempWidth - CFG.PADD * 3 - CFG.BUTTON_W, CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 2 + CFG.PADD * 4, 0, GameManager.investMaxDevGold(nProvinceID, CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()), 0, 0.65f){

            @Override
            public int getWidthE() {
                return Menu_InGame_Invest_Development.this.getElementW() * 2 - CFG.PADD * 4;
            }
        ...
        menuElements.add(new Button_InGameAction(CFG.lang.get("Cancel"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_Invest_Development.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }
        });
        menuElements.add(new Button_InGameAction(CFG.lang.get("Confirm"), -1, 2, tY, CFG.BUTTON_W, true){
            ...
            @Override
            public int getPosXE() {
                return Menu_InGame_Invest_Development.this.getElementW() + CFG.PADD / 2;
            }
            ...
            @Override
            public void buildElemHover() {
                this.menuElemHover = Menu_InGame_Invest_Development.getHoverInvestDev(iProvinceID);
            }
            ...
            public boolean getIsClickable() {
                return CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getMovemPoints() >= GameValues.gvInvestEconomy.INVEST_ECO_COST_MOVEMENT_POINTS;
            }
        });
        int tempMenuPosY = IMGManager.getIMG(Images.topBar).getHeight() + CFG.PADD * 2 + CFG.BUTTON_H * 3 / 4;
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("Invest"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getFlagC().drawO(oSB, Menu_InGame_Invest_Development.this.getPosX() + CFG.PADD * 2 + iTranslateX, Menu_InGame_Invest_Development.this.getPosY() - this.getHeightT() / 2 - CFG.CIV_FLAG_HEIGHT / 2 - CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getFlagC().getHeight(), CFG.CIV_FLAG_WIDTH, CFG.CIV_FLAG_HEIGHT);
                IMGManager.getIMG(Images.flagRectSmall).drawO(oSB, Menu_InGame_Invest_Development.this.getPosX() + CFG.PADD * 2 + iTranslateX, Menu_InGame_Invest_Development.this.getPosY() - this.getHeightT() / 2 - CFG.CIV_FLAG_HEIGHT / 2);
        ...
        try {
            this.getMenuElem(3).setCurr(GameManager.investMaxDevGold(nProvinceID, CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()));
            this.getMenuElem(0).setMin((int)(GameManager.invest_DevelopmentByGold(iProvinceID, this.getMenuElem(this.getMenuElemsSize() - 3).getCurr()) * 10000.0f));
        }
        catch (Exception ex) {
    ...
    public final void actionEL(int iID) {
        if (iID == this.getMenuElemsSize() - 1) {
            if (GameManager.investDevelopment(iProvinceID, CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), this.getMenuElem(3).getCurr())) {
                ...
                if (CFG.mapModesManager.getActiveMapModeID() == MapModesManager.VIEW_ECONOMY_MODE) {
                    CFG.core.getProv((int)Menu_InGame_Invest_Development.iProvinceID).viewBool = true;
        ...
        if (iID == this.getMenuElemsSize() - 3) {
            this.getMenuElem(0).setMin((int)(GameManager.invest_DevelopmentByGold(iProvinceID, this.getMenuElem(iID).getCurr()) * 10000.0f));
            return;
        }
    ...
    public static ME_Hover_v2 getHoverInvestDev(int provinceID) {
        ArrayList<MEHover_2E> nElements = new ArrayList<MEHover_2E>();
        ArrayList<ME_Hover_2Type> nData = new ArrayList<ME_Hover_2Type>();
        ...
        nElements.add(new MEHover_2E(nData));
        nData.clear();
        String sPerc = "" + (float)((int)(CFG.core.getProv(provinceID).getDeveLvl() / CFG.core.getCiv(CFG.core.getProv(provinceID).getCivId()).getTechLevel() * 10000.0f)) / 100.0f + "%";
        nData.add(new ME_Hover_2Type_Text(CFG.lang.get("Development") + ": "));
        nData.add(new ME_Hover_2Type_Text("" + String.format("%.2f", Float.valueOf(CFG.core.getProv(provinceID).getDeveLvl())).replace(',', '.'), CFG.COLOR_NEUTRAL2));
        ...
        nData.add(new ME_Hover_2Type_Text(" [" + sPerc + "]", CFG.COLOR_NEUTRAL));
        nElements.add(new MEHover_2E(nData));
        nData.clear();
        nData.add(new ME_Hover_2Type_Space());
        nElements.add(new MEHover_2E(nData));
        nData.clear();
        if (CFG.core.getProv(provinceID).getDeveLvl() >= CFG.core.getCiv(CFG.core.getProv(provinceID).getCivId()).getTechLevel()) {
            nData.add(new ME_Hover_2Type_TextDesc(CFG.lang.get("Development2"), CFG.COLOR_NEGATIVE_1));
            nElements.add(new MEHover_2E(nData));
            nData.clear();
            nData.add(new ME_Hover_2Type_Space());
            nElements.add(new MEHover_2E(nData));
