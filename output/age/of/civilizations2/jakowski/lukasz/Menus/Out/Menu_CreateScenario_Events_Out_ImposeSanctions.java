...
public class Menu_CreateScenario_Events_Out_ImposeSanctions
extends Menu {
    public Menu_CreateScenario_Events_Out_ImposeSanctions() {
    ...
    public void updateLang() {
        ...
        try {
            this.getMenuElem(1).setTextE(CFG.eventsManager.createScenarioEvents.lDecisions.get((int)CFG.eventsManager.createEvent_EditTriggerID).lOutcomes.get(CFG.eventsManager.createEvent_EditConditionID).getCivID() >= 0 ? CFG.lang.get("ImposeSanctions") + ": " + CFG.core.getCiv(CFG.eventsManager.createScenarioEvents.lDecisions.get((int)CFG.eventsManager.createEvent_EditTriggerID).lOutcomes.get(CFG.eventsManager.createEvent_EditConditionID).getCivID()).getCivName() : CFG.lang.get("ImposeSanctions"));
        }
        catch (IndexOutOfBoundsException ex) {
            this.getMenuElem(1).setTextE(CFG.lang.get("ImposeSanctions"));
        }
        try {
            this.getMenuElem(2).setTextE(CFG.eventsManager.createScenarioEvents.lDecisions.get((int)CFG.eventsManager.createEvent_EditTriggerID).lOutcomes.get(CFG.eventsManager.createEvent_EditConditionID).getCivID2() >= 0 ? CFG.lang.get("Recipient") + ": " + CFG.core.getCiv(CFG.eventsManager.createScenarioEvents.lDecisions.get((int)CFG.eventsManager.createEvent_EditTriggerID).lOutcomes.get(CFG.eventsManager.createEvent_EditConditionID).getCivID2()).getCivName() : CFG.lang.get("Recipient"));
        }
        catch (IndexOutOfBoundsException ex) {
            this.getMenuElem(2).setTextE(CFG.lang.get("Recipient"));
        }
        this.getTitleM().setText(CFG.lang.get("ImposeSanctions"));
    }
    ...
    public final void actionEL(int iID) {
        switch (iID) {
            ...
            case 1: {
                CFG.eventsManager.eSelectCivAction = Event_SelectCivAction.OUT_SELECTCIV_IMPOSE_SANCTIONS;
                CFG.menus.setMenuID(View.eCREATE_SCENARIO_EVENTS_SELECT_CIV);
                break;
            }
            case 2: {
                CFG.eventsManager.eSelectCivAction = Event_SelectCivAction.OUT_SELECTCIV_IMPOSE_SANCTIONS2;
                CFG.menus.setMenuID(View.eCREATE_SCENARIO_EVENTS_SELECT_CIV);
            }
        }
    }
