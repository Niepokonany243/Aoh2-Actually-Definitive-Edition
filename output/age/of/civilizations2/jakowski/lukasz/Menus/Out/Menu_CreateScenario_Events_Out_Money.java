...
public class Menu_CreateScenario_Events_Out_Money
extends Menu {
    public Menu_CreateScenario_Events_Out_Money() {
        ...
        menuElements.add(new Slider_BG(0, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD * 2, CFG.GAMEWIDTH, CFG.BUTTON_H - CFG.PADD * 2, -1000000, 1000000, CFG.eventsManager.createScenarioEvents.lDecisions.get((int)CFG.eventsManager.createEvent_EditTriggerID).lOutcomes.get(CFG.eventsManager.createEvent_EditConditionID).getValue()){
    ...
    public void updateLang() {
        ...
        try {
            this.getMenuElem(1).setTextE(CFG.eventsManager.createScenarioEvents.lDecisions.get((int)CFG.eventsManager.createEvent_EditTriggerID).lOutcomes.get(CFG.eventsManager.createEvent_EditConditionID).getCivID() > 0 ? CFG.lang.get("Civilization") + ": " + CFG.core.getCiv(CFG.eventsManager.createScenarioEvents.lDecisions.get((int)CFG.eventsManager.createEvent_EditTriggerID).lOutcomes.get(CFG.eventsManager.createEvent_EditConditionID).getCivID()).getCivName() : CFG.lang.get("SelectCivilization") + ": " + (CFG.eventsManager.createScenarioEvents.lDecisions.get((int)CFG.eventsManager.createEvent_EditTriggerID).lOutcomes.get(CFG.eventsManager.createEvent_EditConditionID).getCivID() == 0 ? CFG.lang.get("AllCivilizations") : CFG.lang.get("RandomCivilization") + " [-1] #If " + CFG.lang.get("Neutral") + " Civ -> " + CFG.lang.get("AllCivilizations")));
        }
        catch (IndexOutOfBoundsException ex) {
            this.getMenuElem(1).setTextE(CFG.lang.get("SelectCivilization"));
        }
        this.getMenuElem(2).setTextE(CFG.lang.get("Treasury") + ": ");
        this.getTitleM().setText(CFG.lang.get("UpdateMoney"));
    }
    ...
    public final void actionEL(int iID) {
        switch (iID) {
            ...
            case 1: {
                CFG.eventsManager.eSelectCivAction = Event_SelectCivAction.OUT_SELECTCIV_MONEY;
                CFG.menus.setMenuID(View.eCREATE_SCENARIO_EVENTS_SELECT_CIV);
                break;
            }
            case 2: {
                CFG.eventsManager.createScenarioEvents.lDecisions.get((int)CFG.eventsManager.createEvent_EditTriggerID).lOutcomes.get(CFG.eventsManager.createEvent_EditConditionID).setValue(this.getMenuElem(iID).getCurrLong());
            }
        }
    }
