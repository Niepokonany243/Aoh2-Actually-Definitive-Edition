...
public class Menu_CreateScenario_Events_Out_Defensive
extends Menu {
    public Menu_CreateScenario_Events_Out_Defensive() {
        ...
        menuElements.add(new Slider_BG(0, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD * 2, CFG.GAMEWIDTH, CFG.BUTTON_H - CFG.PADD * 2, 0, GameValues.gvDipDefensivePact.DIPLOMACY_MAX_NUMBER_OF_TURNS_FOR_DEFENSIVE_PACT, CFG.eventsManager.createScenarioEvents.lDecisions.get((int)CFG.eventsManager.createEvent_EditTriggerID).lOutcomes.get(CFG.eventsManager.createEvent_EditConditionID).getValue()));
