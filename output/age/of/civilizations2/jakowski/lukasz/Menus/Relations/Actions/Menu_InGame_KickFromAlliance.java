...
public class Menu_InGame_KickFromAlliance
extends Menu {
    private int iOnCivID = 0;

    public Menu_InGame_KickFromAlliance(int onCivID) {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        this.iOnCivID = onCivID;
        menuElements.add(new Button_Diplomacy_CallToArms(Images.diploAlliance, CFG.lang.get("KickFromAlliance") + ": ", this.iOnCivID, 2, tY, CFG.BUTTON_W * 2, GameValues.gvAllianceOffer.COST_ALLIANCE_LEAVE){

            @Override
            public int getWidthE() {
                return Menu_InGame_KickFromAlliance.this.getElementW() * 2;
            }

            @Override
            public void actionElem(int iID) {
                CFG.menus.rebuildInGame_Alliance(CFG.core.getCiv(Menu_InGame_KickFromAlliance.this.iOnCivID).getAlliance());
            }
        });
        ...
        menuElements.add(new Button_InGameAction(CFG.lang.get("Cancel"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_KickFromAlliance.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }
        });
        menuElements.add(new Button_InGameAction(CFG.lang.get("Confirm"), -1, 2, tY, CFG.BUTTON_W, true){
            ...
            @Override
            public int getPosXE() {
                return Menu_InGame_KickFromAlliance.this.getElementW() + CFG.PADD / 2;
            }
            ...
            public void buildElemHover() {
                ...
                nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("KickFromAlliance") + ": ", CFG.COLOR_HOVER_TITLE));
                nData.add(new ME_Hover_2Type_Flag_Big(Menu_InGame_KickFromAlliance.this.iOnCivID, CFG.PADD, CFG.PADD));
                nData.add(new ME_Hover_2Type_Text_Big(CFG.core.getCiv(Menu_InGame_KickFromAlliance.this.iOnCivID).getCivName(), CFG.COLOR_HOVER_TITLE));
                nElements.add(new MEHover_2E(nData));
                nData.clear();
                nData.add(new ME_Hover_2Type_Text(CFG.lang.get("DiplomacyPoints") + ": ", CFG.COLOR_HOVER_TITLE));
                nData.add(new ME_Hover_2Type_Text("-" + (float)GameValues.gvAllianceOffer.COST_ALLIANCE_LEAVE / 10.0f, CFG.COLOR_NEGATIVE_2));
            ...
            public boolean getIsClickable() {
                return CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getDiploPoints() >= GameValues.gvAllianceOffer.COST_ALLIANCE_LEAVE;
            }
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("KickFromAlliance"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                oSB.setColor(new Color(0.8f, 0.24313726f, 0.28235295f, 0.165f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, nPosX + iTranslateX, nPosY - this.getHeightT() + 2 - IMGManager.getIMG(Images.line32Off1).getHeight(), nWidth, this.getHeightT() - 2, false, true);
                oSB.setColor(new Color(0.8f, 0.24313726f, 0.28235295f, 0.375f));
                ...
                IMGManager.getIMG(imgID).drawO(oSB, nPosX + (nWidth - (this.getTextWidth() + IMGManager.getIMG(imgID).getWidth() + CFG.PADD)) / 2 + iTranslateX, Menu_InGame_KickFromAlliance.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(imgID).getHeight() / 2);
    ...
    public final void actionEL(int iID) {
        if (iID == this.getMenuElemsSize() - 1) {
            GameManager.kickFromAlliance(this.iOnCivID, CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId());
