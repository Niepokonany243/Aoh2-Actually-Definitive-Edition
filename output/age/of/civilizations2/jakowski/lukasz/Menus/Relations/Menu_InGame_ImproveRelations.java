...
public class Menu_InGame_ImproveRelations
extends Menu {
    private int iOnCivID = -1;

    public Menu_InGame_ImproveRelations() {
        ...
        this.initMenu(new TitleM(CFG.lang.get("Relations"), CFG.BUTTON_H * 3 / 5, true, true), CFG.GAMEWIDTH / 2 - tempWidth / 2, tempMenuPosY, tempWidth, ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD + tempMenuPosY > CFG.GAMEHEIGHT - CFG.BUTTON_H - CFG.PADD * 2 ? Math.max(CFG.GAMEHEIGHT - CFG.BUTTON_H - CFG.PADD * 2 - tempMenuPosY, (CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 2) * 6) : ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, menuElements, false, true);
        this.updateLang();
    }

    public Menu_InGame_ImproveRelations(int onCivID) {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        ...
        menuElements.add(new ButtonN_Opinion(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), CFG.getActiveCivInfoId(), Images.diploRelationsInc, 0, GameValues.gvRelationImprove.COST_OFFER_IMPROVE_RELATIONS_DIPLOMACY_POINTS, 2, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE(), CFG.BUTTON_W * 2){
        ...
        menuElements.add(new Text_Desc(GameManager.getImproveRelationsMessage(), 2, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, tempWidth - 4){
        ...
        menuElements.add(new Slider_InGame_Date(CFG.lang.get("NumberOfTurns"), CFG.PADD * 2, ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, tempWidth - CFG.PADD * 3 - CFG.BUTTON_W, Math.max(CFG.BUTTON_H * 4 / 5, CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 2 + CFG.PADD * 4), 1, GameValues.gvRelationImprove.IMPROVE_RELATIONS_MAX_NUM_OF_TURNS, GameValues.gvRelationImprove.IMPROVE_RELATIONS_MAX_NUM_OF_TURNS, 0.65f){

            @Override
            public int getWidthE() {
                return Menu_InGame_ImproveRelations.this.getElementW() * 2 - CFG.PADD * 4;
            }
        ...
        menuElements.add(new Button_InGameAction(CFG.lang.get("Cancel"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_ImproveRelations.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }
        });
        menuElements.add(new Button_InGameAction(CFG.lang.get("ImproveRelation"), -1, 2, tY, CFG.BUTTON_W, true){

            @Override
            public int getPosXE() {
                return Menu_InGame_ImproveRelations.this.getElementW() + CFG.PADD / 2;
            }
            ...
            public void buildElemHover() {
                ...
                nData.add(new ME_Hover_2Type_Flag_Big(Menu_InGame_ImproveRelations.this.iOnCivID, CFG.PADD, CFG.PADD));
                nData.add(new ME_Hover_2Type_Text_Big(CFG.core.getCiv(Menu_InGame_ImproveRelations.this.iOnCivID).getCivName()));
                nData.add(new ME_Hover_2Type_Image_Big(Images.diploRelations, CFG.PADD, CFG.PADD));
                ...
                nData.add(new ME_Hover_2Type_Text("+" + CFG.getPrecision2(GameManager.getImproveRelation(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), Menu_InGame_ImproveRelations.this.iOnCivID) * GameValues.gvRelationImprove.IMPROVE_RELATIONS_RECIPROCITY_MODIFIER, 100), CFG.COLOR_POSITIVE));
                nData.add(new ME_Hover_2Type_Image(Images.diploRelationsInc, CFG.PADD, 0));
                nElements.add(new MEHover_2E(nData));
                nData.clear();
                nData.add(new ME_Hover_2Type_Text(GameCalendar.getDate_ByTurnID(GameCalendar.TURNID + Menu_InGame_ImproveRelations.this.getMenuElem(3).getCurr())));
                nData.add(new ME_Hover_2Type_Text(" [" + CFG.lang.get("TurnsX", Menu_InGame_ImproveRelations.this.getMenuElem(3).getCurr()) + "]", CFG.COLOR_NEUTRAL));
            ...
            public void drawTextE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
                IMGManager.getIMG(Images.diploRelationsInc).drawO(oSB, this.getPosXE() + this.getWidthE() / 2 - (this.getTextWidthU() + IMGManager.getIMG(Images.diploRelationsInc).getWidth() + CFG.PADD) / 2 + iTranslateX, this.getPosY() + this.getHeightE() / 2 - IMGManager.getIMG(Images.diploRelationsInc).getHeight() / 2 + iTranslateY);
                Renderer.drawText(oSB, this.fontID, this.getTextE(), this.getPosXE() + (this.getTextPosElem() < 0 ? this.getWidthE() / 2 - (this.getTextWidthU() + IMGManager.getIMG(Images.diploRelationsInc).getWidth() + CFG.PADD) / 2 + IMGManager.getIMG(Images.diploRelationsInc).getWidth() + CFG.PADD : this.getTextPosElem()) + iTranslateX, this.getPosY() + this.getHeightE() / 2 - this.getTextHeight() / 2 + iTranslateY, this.getColorE(isActive));
            }

            @Override
            public boolean getIsClickable() {
                return CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getDiploPoints() >= GameValues.gvRelationImprove.COST_OFFER_IMPROVE_RELATIONS_DIPLOMACY_POINTS;
            }
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("Relations"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                oSB.setColor(new Color(CFG.COLOR_POSITIVE.r, CFG.COLOR_POSITIVE.g, CFG.COLOR_POSITIVE.b, 0.165f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, nPosX + iTranslateX, nPosY - this.getHeightT() + 2 - IMGManager.getIMG(Images.line32Off1).getHeight(), nWidth, this.getHeightT() - 2, false, true);
                oSB.setColor(new Color(CFG.COLOR_POSITIVE.r, CFG.COLOR_POSITIVE.g, CFG.COLOR_POSITIVE.b, 0.375f));
                ...
                IMGManager.getIMG(Images.diploRelationsInc).drawO(oSB, nPosX + (nWidth - this.getTextWidth()) / 2 - CFG.PADD - IMGManager.getIMG(Images.diploRelationsInc).getWidth() + iTranslateX, 2 + nPosY - this.getHeightT() + this.getHeightT() / 2 - IMGManager.getIMG(Images.diploRelationsInc).getHeight() / 2);
    ...
    public final void actionEL(int iID) {
        if (iID == this.getMenuElemsSize() - 1) {
            if (this.getMenuElem(3).getCurr() > 1) {
                CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getCivDiploGD().addImproveRelations(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), this.iOnCivID, this.getMenuElem(3).getCurr());
            }
            CFG.toastM.addM(CFG.lang.get("DiplomatSent") + "!", CFG.COLOR_POSITIVE);
            CFG.toastM.setTimeInView(3500);
            CFG.menus.rebuildMenu_InGame_Infobox(CFG.lang.get("DiplomatSent"), CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), this.iOnCivID, Images.infoRelations);
