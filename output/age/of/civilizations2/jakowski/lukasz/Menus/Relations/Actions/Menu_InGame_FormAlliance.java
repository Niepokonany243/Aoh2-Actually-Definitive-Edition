...
public class Menu_InGame_FormAlliance
extends Menu {
    public static long lTime = 0L;

    public Menu_InGame_FormAlliance() {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        List<Integer> civs = CFG.core.getPossibleAlliances(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId());
        if (!civs.isEmpty()) {
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("FormAlliance"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                IMGManager.getIMG(imgID).drawO(oSB, nPosX + (nWidth - (this.getTextWidth() + IMGManager.getIMG(imgID).getWidth() + CFG.PADD)) / 2 + iTranslateX, Menu_InGame_FormAlliance.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(imgID).getHeight() / 2);
