...
public class Menu_CreateScenario_Cores_AddCiv_List
extends Menu {
    ...
    public Menu_CreateScenario_Cores_AddCiv_List() {
        ...
        menuElements.add(new Slider_BG(CFG.lang.get("PercentageOfPopulation"), 0, CFG.PADD * 2, CFG.GAMEWIDTH, CFG.BUTTON_H, 1, 99, CFG.MANAGE_DIPLOMACY_CUSTOMIZE_ALLIANCE_ID){
        ...
        if (CFG.sSearch != null && CFG.sSearch.length() > 0) {
            ...
            CFG.menus.menuCreateScenario_Cores_AddCiv_UpdateTitle(lTempNames.size());
            int nPosY = 0;
            int tID = 0;
            while (lTempNames.size() > 0) {
                ...
                menuElements.add(new Button_Classic(CFG.lang.getCiv((String)lTempTags.get(toAddID)) + " [" + (String)lTempTags.get(toAddID) + "]", (int)(50.0f * CFG.GUI_SCALE), 0, CFG.BUTTON_H * (nPosY + 1) + CFG.PADD * (nPosY + 3), CFG.GAMEWIDTH - (CFG.BUTTON_W + CFG.BUTTON_W / 2), CFG.BUTTON_H, true));
                menuElements.add(new Button_Classic_Classic_Wiki(tID++, CFG.GAMEWIDTH - (CFG.BUTTON_W + CFG.BUTTON_W / 2), CFG.BUTTON_H * (nPosY + 1) + CFG.PADD * (nPosY + 3), CFG.BUTTON_W + CFG.BUTTON_W / 2, CFG.BUTTON_H, true){
                    ...
                    public void buildElemHover() {
                        try {
                            ...
                            nData.add(new ME_Hover_2Type_Text(CFG.getWikiInforLinkClear((String)Menu_CreateScenario_Cores_AddCiv_List.this.lCivsTags.get(this.getTextPosElem())), CFG.COLOR_HOVER_TITLE));
    ...
    public final void actionEL(int iID) {
        switch (iID) {
            case 0: {
                CFG.MANAGE_DIPLOMACY_CUSTOMIZE_ALLIANCE_ID = this.getMenuElem(iID).getCurr();
                return;
            }
        }
        if (--iID % 2 == 0) {
            ...
            for (int i = 0; i < CFG.core.getProvSelected().getProvSize(); ++i) {
                CFG.province_CoresGD.addCore(CFG.core.getProvSelected().getProv(i), CFG.core.getProv(CFG.core.getProvSelected().getProv(i)).getCivId(), 100);
                int tID = CFG.core.getCivsSize() - 1;
                CFG.core.getProv(CFG.core.getProvSelected().getProv(i)).getCores().addNewCore(tID, 1);
                CFG.province_CoresGD.addCore(CFG.core.getProvSelected().getProv(i), tID, CFG.MANAGE_DIPLOMACY_CUSTOMIZE_ALLIANCE_ID);
            }
