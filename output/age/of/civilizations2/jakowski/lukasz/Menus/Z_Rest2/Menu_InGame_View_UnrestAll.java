...
public class Menu_InGame_View_UnrestAll
extends Menu {
    public static long lTime = 0L;
    public static boolean hideAnimation = true;

    public static float getRevRisk(int civID) {
        float out = 0.0f;
        for (int i = 0; i < CFG.core.getCiv(civID).getNumOfProvs(); ++i) {
            out += (float)((int)(CFG.core.getProv(CFG.core.getCiv(civID).getProvID(i)).getRevRisk() * 100.0f));
        }
        return out / (float)CFG.core.getCiv(civID).getNumOfProvs();
    }

    public Menu_InGame_View_UnrestAll() {
        int tempW = CFG.CIV_INFO_MENU_WIDTH + CFG.BUTTON_W * 3 / 4;
        int tY = 0;
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        ArrayList<Integer> tCivs = new ArrayList<Integer>();
        ArrayList<Float> tRevRisk = new ArrayList<Float>();
        float totalRevRisk = 0.0f;
        for (int i = 1; i < CFG.core.getCivsSize(); ++i) {
            if (CFG.core.getCiv(i).getNumOfProvs() <= 0) continue;
            tCivs.add(i);
            float countedRevRisk = Menu_InGame_View_UnrestAll.getRevRisk(i);
            tRevRisk.add(Float.valueOf(countedRevRisk));
            totalRevRisk += countedRevRisk;
        }
        totalRevRisk /= (float)tCivs.size();
        menuElements.add(new Button_DiplomacyAction(Images.diploRevolution, CFG.lang.get("RevolutionaryRisk"), 0, 0, tY, tempW, Menu_InGame_Civ_Decisions.getButtonH(), true){
        ...
        if (!tCivs.isEmpty()) {
            ...
            while (!tCivs.isEmpty()) {
                ...
                for (int o = 1; o < tCivs.size(); ++o) {
                    if (!(((Float)tRevRisk.get(toAddID)).floatValue() < ((Float)tRevRisk.get(o)).floatValue())) continue;
                    toAddID = o;
                }
                ...
                menuElements.add(new ButtonN_Pop2(new Color((float)CFG.core.getCiv(civID).getR() / 255.0f, (float)CFG.core.getCiv(civID).getG() / 255.0f, (float)CFG.core.getCiv(civID).getB() / 255.0f, 1.0f), rankID++ + ". " + (metCiv ? CFG.core.getCiv(civID).getCivName() : CFG.lang.get("Undiscovered")), metCiv ? civID : -1, "", CFG.getPrecision2(((Float)tRevRisk.get(toAddID)).floatValue(), 1000) + "%", Images.diploRevolution, CFG.COLOR_TEXT_NUM_OF_PROVINCES, 0, tY, tempW){
                    ...
                    public void buildElemHover() {
                        try {
                            ...
                            if (this.iCivID > 0) {
                                ...
                                nData.add(new ME_Hover_2Type_Text(CFG.getPrecision2(Menu_InGame_View_UnrestAll.getRevRisk(this.iCivID), 100) + "%", CFG.COLOR_TEXT_NUM_OF_PROVINCES));
                ...
                tRevRisk.remove(toAddID);
            }
        } else {
            menuElements.add(new TextScale(CFG.lang.get("NoData"), -1, 0, tY, tempW, CFG.BUTTON_H * 3 / 4, 0.75f));
            tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
        }
        this.initMenu(new TitleM_TextSmall(CFG.map.getMapName_Just(CFG.map.getActiveMapIDN()) + ": " + CFG.lang.get("RevolutionaryRisk") + (playerRankID > 0 ? " #" + playerRankID : ""), CFG.BUTTON_H * 3 / 5, false, false){

            @Override
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                IMGManager.getIMG(Images.gameTopEdgeTitle).draw2O(oSB, Menu_InGame_View_UnrestAll.this.getPosX() + iTranslateX, Menu_InGame_View_UnrestAll.this.getPosY() - Core.PADDING - IMGManager.getIMG(Images.gameTopEdgeTitle).getHeight() - this.getHeightT(), Menu_InGame_View_UnrestAll.this.getWidthM() + 2 + Core.PADDING, this.getHeightT() + Core.PADDING, true, false);
                oSB.setColor(new Color(CFG.COLOR_REVOLUTION_MAX.r, CFG.COLOR_REVOLUTION_MAX.g, CFG.COLOR_REVOLUTION_MAX.b, 0.165f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, nPosX + iTranslateX, nPosY - this.getHeightT() + 2 - IMGManager.getIMG(Images.line32Off1).getHeight(), nWidth, this.getHeightT() - 2, false, true);
                oSB.setColor(new Color(CFG.COLOR_REVOLUTION_MAX.r, CFG.COLOR_REVOLUTION_MAX.g, CFG.COLOR_REVOLUTION_MAX.b, 0.375f));
                IMGManager.getIMG(Images.gradient).drawO(oSB, nPosX + iTranslateX, nPosY - this.getHeightT() * 2 / 3 - IMGManager.getIMG(Images.gradient).getHeight(), nWidth, this.getHeightT() * 2 / 3, false, true);
                oSB.setColor(CFG.COLOR_NEW_GAME_EDGE_LINE);
                IMGManager.getIMG(Images.pix255).drawO(oSB, Menu_InGame_View_UnrestAll.this.getPosX() + iTranslateX, Menu_InGame_View_UnrestAll.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight(), Menu_InGame_View_UnrestAll.this.getWidthM());
                oSB.setColor(new Color(CFG.COLOR_FLAG_FRAME.r, CFG.COLOR_FLAG_FRAME.g, CFG.COLOR_FLAG_FRAME.b, 0.4f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, Menu_InGame_View_UnrestAll.this.getPosX() + iTranslateX, Menu_InGame_View_UnrestAll.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.line32Off1).getHeight(), Menu_InGame_View_UnrestAll.this.getWidthM(), 1);
                oSB.setColor(new Color(0.0f, 0.0f, 0.0f, 0.6f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, Menu_InGame_View_UnrestAll.this.getPosX() + iTranslateX, Menu_InGame_View_UnrestAll.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.line32Off1).getHeight() - 1, Menu_InGame_View_UnrestAll.this.getWidthM(), 1);
                oSB.setColor(new Color(0.0f, 0.0f, 0.0f, 0.8f));
                IMGManager.getIMG(Images.sliderGradient).drawO(oSB, Menu_InGame_View_UnrestAll.this.getPosX() + iTranslateX, Menu_InGame_View_UnrestAll.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.sliderGradient).getHeight(), Menu_InGame_View_UnrestAll.this.getWidthM() / 4, 1);
                IMGManager.getIMG(Images.sliderGradient).drawO(oSB, Menu_InGame_View_UnrestAll.this.getPosX() + Menu_InGame_View_UnrestAll.this.getWidthM() - Menu_InGame_View_UnrestAll.this.getWidthM() / 4 + iTranslateX, Menu_InGame_View_UnrestAll.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.sliderGradient).getHeight(), Menu_InGame_View_UnrestAll.this.getWidthM() / 4, 1, true, false);
                oSB.setColor(Color.WHITE);
                IMGManager.getIMG(Images.diploRevolution).drawO(oSB, nPosX + CFG.PADD * 2 + iTranslateX, Menu_InGame_View_UnrestAll.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(Images.diploRevolution).getHeight() / 2);
