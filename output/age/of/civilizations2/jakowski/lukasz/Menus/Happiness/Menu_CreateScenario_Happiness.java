...
package age.of.civilizations2.jakowski.lukasz.Menus.Happiness;
...
public class Menu_CreateScenario_Happiness
extends Menu_CreateScenario {
    ...
    public Menu_CreateScenario_Happiness() {
        ...
        menuElements.add(new Slider(CFG.BUTTON_W + CFG.PADD * 2, CFG.GAMEHEIGHT - CFG.BUTTON_H - CFG.PADD, CFG.GAMEWIDTH - (CFG.BUTTON_W + CFG.PADD * 2) * 2, CFG.BUTTON_H, 0, 100, 100){
            ...
            public void drawSliderText(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive, boolean scrollableY) {
                if (CFG.createScenarioAssignProvsCiv >= 0) {
                    ...
                    CFG.core.getCiv(CFG.createScenarioAssignProvsCiv).getFlagC().draw(oSB, this.getPosXE() + this.getWidthE() / 2 - this.getTextWidthU() / 2 + iTranslateX, this.getPosY() + this.getHeightE() / 2 - CFG.CIV_FLAG_HEIGHT / 2 + iTranslateY, IMGManager.getIMG(Images.flagRectSmall).getWidth(), IMGManager.getIMG(Images.flagRectSmall).getHeight());
    ...
    public void updateLang() {
        this.sTopText = CFG.lang.get("Happiness");
    ...
    public final void actionEL(int iID) {
        switch (iID) {
            ...
            case 3: {
                ...
                if (CFG.createScenarioAssignProvsCiv > 0) {
                    CFG.core.getCiv(CFG.createScenarioAssignProvsCiv).setHappiness(this.getMenuElem(iID + 1).getCurr());
                    CFG.core.getProv(CFG.core.getCiv(CFG.createScenarioAssignProvsCiv).getCapitalProvID()).getArmyObject(0).updateArmyWidth(CFG.core.getCiv(CFG.createScenarioAssignProvsCiv).getHappiness() + "%");
                }
                return;
            }
            case 4: {
                if (CFG.createScenarioAssignProvsCiv > 0) {
                    CFG.core.getCiv(CFG.createScenarioAssignProvsCiv).setHappiness(this.getMenuElem(iID).getCurr());
                    CFG.core.getProv(CFG.core.getCiv(CFG.createScenarioAssignProvsCiv).getCapitalProvID()).getArmyObject(0).updateArmyWidth(CFG.core.getCiv(CFG.createScenarioAssignProvsCiv).getHappiness() + "%");
                }
                return;
            }
