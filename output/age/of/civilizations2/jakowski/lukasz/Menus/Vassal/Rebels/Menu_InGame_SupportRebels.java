...
public class Menu_InGame_SupportRebels
extends Menu {
    public static int iOnCivID = -1;

    public Menu_InGame_SupportRebels() {
        ...
        this.initMenu(new TitleM(CFG.lang.get("SupportRebels"), CFG.BUTTON_H * 3 / 5, true, true), CFG.GAMEWIDTH / 2 - tempWidth / 2, tempMenuPosY, tempWidth, ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD + tempMenuPosY > CFG.GAMEHEIGHT - CFG.BUTTON_H - CFG.PADD * 2 ? Math.max(CFG.GAMEHEIGHT - CFG.BUTTON_H - CFG.PADD * 2 - tempMenuPosY, (CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 2) * 6) : ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, menuElements, false, true);
        this.updateLang();
    }

    public Menu_InGame_SupportRebels(int onCivID, int iRebelsID) {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        iOnCivID = onCivID;
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        ...
        tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
        SupportRebels_List supportData = GameManager.supportRebels(iOnCivID);
        if (supportData.lMovementsCivID.size() > 0) {
            menuElements.add(new TextBuildTitle(CFG.lang.get("SelectMovement"), -1, 0, tY, CFG.BUTTON_W, CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 4){
            ...
            for (int i = 0; i < supportData.lMovementsCivID.size(); ++i) {
                menuElements.add(new Button_Diplomacy_SupportRebels(i, supportData.lMovementsCivID.get(i), supportData.lPopulation.get(i), (int)((float)supportData.lUnrest.get(i).intValue() / (float)supportData.lProvinces.get(i).intValue()), supportData.lProvinces.get(i), 2, tY, CFG.BUTTON_W * 2){
                ...
                tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
                if (((MenuElemUI)menuElements.get(menuElements.size() - 1)).getCurr() != iRebelsID) continue;
                ((MenuElemUI)menuElements.get(menuElements.size() - 1)).setCheckboxSt(true);
            }
            menuElements.add(new Slider_InGame_Gold(CFG.lang.get("SelectTheAmountOfMoneyToSend"), CFG.PADD * 2, ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, tempWidth - CFG.PADD * 3 - CFG.BUTTON_W, CFG.BUTTON_H * 3 / 4, 0L, CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getGold() > 0L ? CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getGold() : 0L, 0L, 0.65f){

                @Override
                public int getWidthE() {
                    return Menu_InGame_SupportRebels.this.getElementW() * 2 - CFG.PADD * 4;
                }
        ...
        menuElements.add(new Button_InGameAction(CFG.lang.get("Cancel"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_SupportRebels.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }
        });
        menuElements.add(new Button_InGameAction(CFG.lang.get("SupportRebels"), -1, 2, tY, CFG.BUTTON_W, supportData.lMovementsCivID.size() > 0){

            @Override
            public int getPosXE() {
                return Menu_InGame_SupportRebels.this.getElementW() + CFG.PADD / 2;
            }
            ...
            public boolean getIsClickable() {
                return !CFG.SPECTATOR_MODE && CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getDiploPoints() >= GameValues.gvRebelsSupport.COST_SUPPORT_REBELS_DIPLOMACY_POINTS && super.getIsClickable();
            }
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("RevolutionaryMovements"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                IMGManager.getIMG(imgID).drawO(oSB, nPosX + (nWidth - (this.getTextWidth() + IMGManager.getIMG(imgID).getWidth() + CFG.PADD)) / 2 + iTranslateX, Menu_InGame_SupportRebels.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(imgID).getHeight() / 2);
    ...
    public final void actionEL(int iID) {
        if (iID == this.getMenuElemsSize() - 1) {
            ...
            for (int i = 0; i < this.getMenuElemsSize(); ++i) {
                if (!this.getMenuElem(i).getCheckboxSt()) continue;
                iSelectedCivID = (int)this.getMenuElem(i).getCurrLong();
            }
            if (iSelectedCivID < 0) {
            ...
            if (this.getMenuElem(this.getMenuElemsSize() - 3).getCurrLong() <= 0L) {
                CFG.toastM.addM(CFG.lang.get("SelectTheAmountOfMoneyToSend") + "!", CFG.COLOR_NEGATIVE_2);
                CFG.toastM.setTimeInView(3500);
                return;
            }
            if (GameManager.supportRebels(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), iOnCivID, iSelectedCivID, this.getMenuElem(this.getMenuElemsSize() - 3).getCurrLong())) {
                CFG.toastM.addM(CFG.lang.get("Sent") + "!", CFG.COLOR_POSITIVE);
                CFG.toastM.setTimeInView(3500);
                int tOldActive = CFG.core.getActiveProvID();
                CFG.core.setActiveProvID(-1);
                CFG.core.setActiveProvID(tOldActive);
                CFG.SFXManager.playSound(SFXManager.SFX_PLUNDER);
            }
        ...
        if (iID == this.getMenuElemsSize() - 3 || iID <= 1) {
            return;
        }
        boolean centerToProv = this.getMenuElem(iID).getCheckboxSt();
        for (int i = 0; i < this.getMenuElemsSize(); ++i) {
            this.getMenuElem(i).setCheckboxSt(false);
        }
        this.getMenuElem(iID).setCheckboxSt(true);
        List<Integer> rebelsProvinces = GameManager.supportRebels_Provinces(iOnCivID, (int)this.getMenuElem(iID).getCurrLong());
        this.getMenuElem(this.getMenuElemsSize() - 3).setMax(Math.max(0L, Math.min(CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getGold(), GameManager.supportRebels_MaxGold(rebelsProvinces))));
        this.getMenuElem(this.getMenuElemsSize() - 3).setCurr(this.getMenuElem(this.getMenuElemsSize() - 3).getCurrLong());
        if (centerToProv && rebelsProvinces.size() > 0) {
            int tBest = 0;
            for (int i = 1; i < rebelsProvinces.size(); ++i) {
                if (!CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getMetProv(rebelsProvinces.get(i)) || CFG.core.getProv(rebelsProvinces.get(tBest)).getPop().getPopulationOfCivID((int)this.getMenuElem(iID).getCurrLong()) >= CFG.core.getProv(rebelsProvinces.get(i)).getPop().getPopulationOfCivID((int)this.getMenuElem(iID).getCurrLong())) continue;
                tBest = i;
            }
