...
public class Menu_InGame_TransferVassal2
extends Menu {
    public static long lTime = 0L;
    public static int toCivID = 0;
    public static int vassalCivID = 0;

    public Menu_InGame_TransferVassal2(int nToCivID, int nVassalID) {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        toCivID = nToCivID;
        vassalCivID = nVassalID;
        menuElements.add(new ButtonN_Civs2(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), toCivID, 0, tY, tempWidth){

            @Override
            public int getWidthE() {
                return Menu_InGame_TransferVassal2.this.getElementW() * 2;
            }
        });
        ...
        menuElements.add(new TextBuildTitle(CFG.lang.get("Vassal"), -1, 0, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, CFG.BUTTON_W, CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 4){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_TransferVassal2.this.getW();
            }
        });
        tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
        if (vassalCivID > 0) {
            menuElements.add(new ButtonN_FlagIcon_3_TextRight(vassalCivID, Images.diploVassal, 0, tY, CFG.BUTTON_W * 2, CFG.getNumberWthSpaces("" + CFG.core.getCiv(vassalCivID).getNumOfProvs()), Images.provinces, CFG.COLOR_TEXT_NUM_OF_PROVINCES, CFG.getNumberWthSpaces("" + CFG.core.getCiv(vassalCivID).countPop()), Images.pop, CFG.COLOR_POPULATION){
            ...
            menuElements.add(new Button_InGameAction(CFG.lang.get("Cancel"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
                ...
                @Override
                public int getWidthE() {
                    return Menu_InGame_TransferVassal2.this.getElementW() - CFG.PADD - CFG.PADD / 2;
                }
            ...
            menuElements.add(new Button_InGameAction(CFG.lang.get("Confirm"), -1, 2, tY, CFG.BUTTON_W, true){
                ...
                public void actionElem(int iID) {
                    try {
                        if (CFG.core.getCiv(vassalCivID).getPuppetOfCiv() == CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()) {
                            CFG.core.getCiv(vassalCivID).setPuppetOfCivId(toCivID);
                            CFG.core.getCiv(toCivID).setRelationD(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), CFG.core.getCiv(toCivID).getRelationD(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()) + GameValues.gvVassal.TRANSFER_VASSAL_RELATION);
                            CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).setRelationD(toCivID, CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getRelationD(toCivID) + GameValues.gvVassal.TRANSFER_VASSAL_RELATION);
                            CFG.menus.rebuildMenu_InGame_InfoboxSmallFlags(CFG.lang.get("VassalTransferredSuccessfully") + ": " + CFG.core.getCiv(vassalCivID).getCivName(), GameCalendar.getCurrDate(), Images.infoDiplomacy, toCivID, vassalCivID);
                        }
                    }
                ...
                @Override
                public int getPosXE() {
                    return Menu_InGame_TransferVassal2.this.getElementW() + CFG.PADD / 2;
                }
                ...
                public void buildElemHover() {
                    ...
                    ArrayList<ME_Hover_2Type> nData = new ArrayList<ME_Hover_2Type>();
                    nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("TransferVassal"), CFG.COLOR_HOVER_TITLE));
                    nData.add(new ME_Hover_2Type_Flag_Big(vassalCivID, CFG.PADD, 0));
                    nData.add(new ME_Hover_2Type_Image_Big(Images.diploVassal, CFG.PADD, 0));
                    nElements.add(new MEHover_2E(nData));
                    nData.clear();
                    nData.add(new ME_Hover_2Type_Space());
                    ...
                    nData.add(new ME_Hover_2Type_Flag_Big(toCivID, CFG.PADD, 0));
                    ...
                    nElements.add(new MEHover_2E(nData));
                    nData.clear();
                    nData.add(new ME_Hover_2Type_Flag_Big(vassalCivID, 0, CFG.PADD));
                    nData.add(new ME_Hover_2Type_Text_Big(CFG.core.getCiv(vassalCivID).getCivName(), CFG.COLOR_HOVER_TITLE));
                    nElements.add(new MEHover_2E(nData));
                    nData.clear();
                    nData.add(new ME_Hover_2Type_Space());
                    nElements.add(new MEHover_2E(nData));
                    nData.clear();
                    nData.add(new ME_Hover_2Type_Text(CFG.lang.get("Provinces") + ": "));
                    nData.add(new ME_Hover_2Type_Text(CFG.getNumberWthSpaces("" + CFG.core.getCiv(vassalCivID).getNumOfProvs()), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
                    nData.add(new ME_Hover_2Type_Image(Images.provinces, CFG.PADD, 0));
                    nElements.add(new MEHover_2E(nData));
                    nData.clear();
                    nData.add(new ME_Hover_2Type_Text(CFG.lang.get("Population") + ": "));
                    nData.add(new ME_Hover_2Type_Text(CFG.getNumberWthSpaces("" + CFG.core.getCiv(vassalCivID).countPop()), CFG.COLOR_POPULATION));
                    nData.add(new ME_Hover_2Type_Image(Images.pop, CFG.PADD, 0));
                    nElements.add(new MEHover_2E(nData));
                    nData.clear();
                    nData.add(new ME_Hover_2Type_Text(CFG.lang.get("Economy") + ": "));
                    nData.add(new ME_Hover_2Type_Text(CFG.getNumberWthSpaces("" + CFG.core.getCiv(vassalCivID).countEco()), CFG.COLOR_ECONOMY));
                    nData.add(new ME_Hover_2Type_Image(Images.economy, CFG.PADD, 0));
                    nElements.add(new MEHover_2E(nData));
                    nData.clear();
                    this.menuElemHover = new ME_Hover_v2(nElements);
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("TransferVassal"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                IMGManager.getIMG(imgID).drawO(oSB, nPosX + (nWidth - (this.getTextWidth() + IMGManager.getIMG(imgID).getWidth() + CFG.PADD)) / 2 + iTranslateX, Menu_InGame_TransferVassal2.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(imgID).getHeight() / 2);
