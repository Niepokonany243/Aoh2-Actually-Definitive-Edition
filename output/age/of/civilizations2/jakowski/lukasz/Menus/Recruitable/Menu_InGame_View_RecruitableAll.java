...
import age.of.civilizations2.jakowski.lukasz.Menus.Recruitable.Menu_InGame_View_Recruitable;
...
public class Menu_InGame_View_RecruitableAll
extends Menu {
    public static long lTime = 0L;
    public static boolean hideAnimation = true;

    public Menu_InGame_View_RecruitableAll() {
        int tempW = CFG.CIV_INFO_MENU_WIDTH + CFG.BUTTON_W * 3 / 4;
        int tY = 0;
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        ArrayList<Integer> tCivs = new ArrayList<Integer>();
        ArrayList<Long> tRecruitable = new ArrayList<Long>();
        long totalRecruitable = 0L;
        for (int i = 1; i < CFG.core.getCivsSize(); ++i) {
            if (CFG.core.getCiv(i).getNumOfProvs() <= 0) continue;
            tCivs.add(i);
            long countedRecruitable = Menu_InGame_View_Recruitable.getRecruitable(i);
            tRecruitable.add(countedRecruitable);
            totalRecruitable += countedRecruitable;
        }
        menuElements.add(new Button_DiplomacyAction(Images.diploArmy, CFG.lang.get("RecruitablePopulation") + ": " + CFG.getNumberWthSpaces("" + totalRecruitable), 0, 0, tY, tempW, Menu_InGame_Civ_Decisions.getButtonH(), true){
        ...
        menuElements.add(new Graph_Vertical(Graph_Vertical_Data_Type.ARMY_PER_CAPITA, CFG.lang.get("Civilizations"), CFG.lang.get("Armies"), CFG.PADD, tY, tempW - CFG.PADD * 2, tempW * 2 / 5, true, tempData));
        tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
        int playerRankID = 0;
        if (!tCivs.isEmpty()) {
            int rankID = 1;
            while (!tCivs.isEmpty()) {
                ...
                for (int o = 1; o < tCivs.size(); ++o) {
                    if ((Long)tRecruitable.get(toAddID) >= (Long)tRecruitable.get(o)) continue;
                    toAddID = o;
                }
                    ...
                    public void buildElemHover() {
                        try {
                            ...
                            if (this.iCivID > 0) {
                                ...
                                nData.add(new ME_Hover_2Type_Text(CFG.getNumberWthSpaces("" + Menu_InGame_View_Recruitable.getRecruitable(this.iCivID)), CFG.COLOR_POSITIVE));
                ...
                tRecruitable.remove(toAddID);
            }
        } else {
            menuElements.add(new TextScale(CFG.lang.get("NoData"), -1, 0, tY, tempW, CFG.BUTTON_H * 3 / 4, 0.75f));
            tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
        }
        this.initMenu(new TitleM_TextSmall(CFG.map.getMapName_Just(CFG.map.getActiveMapIDN()) + ": " + CFG.lang.get("RecruitablePopulation") + (playerRankID > 0 ? " #" + playerRankID : ""), CFG.BUTTON_H * 3 / 5, false, false){

            @Override
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                IMGManager.getIMG(Images.gameTopEdgeTitle).draw2O(oSB, Menu_InGame_View_RecruitableAll.this.getPosX() + iTranslateX, Menu_InGame_View_RecruitableAll.this.getPosY() - Core.PADDING - IMGManager.getIMG(Images.gameTopEdgeTitle).getHeight() - this.getHeightT(), Menu_InGame_View_RecruitableAll.this.getWidthM() + 2 + Core.PADDING, this.getHeightT() + Core.PADDING, true, false);
                ...
                IMGManager.getIMG(Images.pix255).drawO(oSB, Menu_InGame_View_RecruitableAll.this.getPosX() + iTranslateX, Menu_InGame_View_RecruitableAll.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight(), Menu_InGame_View_RecruitableAll.this.getWidthM());
                oSB.setColor(new Color(CFG.COLOR_FLAG_FRAME.r, CFG.COLOR_FLAG_FRAME.g, CFG.COLOR_FLAG_FRAME.b, 0.4f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, Menu_InGame_View_RecruitableAll.this.getPosX() + iTranslateX, Menu_InGame_View_RecruitableAll.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.line32Off1).getHeight(), Menu_InGame_View_RecruitableAll.this.getWidthM(), 1);
                oSB.setColor(new Color(0.0f, 0.0f, 0.0f, 0.6f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, Menu_InGame_View_RecruitableAll.this.getPosX() + iTranslateX, Menu_InGame_View_RecruitableAll.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.line32Off1).getHeight() - 1, Menu_InGame_View_RecruitableAll.this.getWidthM(), 1);
                oSB.setColor(new Color(0.0f, 0.0f, 0.0f, 0.8f));
                IMGManager.getIMG(Images.sliderGradient).drawO(oSB, Menu_InGame_View_RecruitableAll.this.getPosX() + iTranslateX, Menu_InGame_View_RecruitableAll.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.sliderGradient).getHeight(), Menu_InGame_View_RecruitableAll.this.getWidthM() / 4, 1);
                IMGManager.getIMG(Images.sliderGradient).drawO(oSB, Menu_InGame_View_RecruitableAll.this.getPosX() + Menu_InGame_View_RecruitableAll.this.getWidthM() - Menu_InGame_View_RecruitableAll.this.getWidthM() / 4 + iTranslateX, Menu_InGame_View_RecruitableAll.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.sliderGradient).getHeight(), Menu_InGame_View_RecruitableAll.this.getWidthM() / 4, 1, true, false);
                oSB.setColor(Color.WHITE);
                IMGManager.getIMG(Images.diploArmy).drawO(oSB, nPosX + CFG.PADD * 2 + iTranslateX, Menu_InGame_View_RecruitableAll.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(Images.diploArmy).getHeight() / 2);
