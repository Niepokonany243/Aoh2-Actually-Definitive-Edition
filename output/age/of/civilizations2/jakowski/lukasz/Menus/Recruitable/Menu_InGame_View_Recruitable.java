 ...
 */
package age.of.civilizations2.jakowski.lukasz.Menus.Recruitable;
...
import age.of.civilizations2.jakowski.lukasz.Button.View.Button_View_Recruitable;
...
public class Menu_InGame_View_Recruitable
extends Menu {
    public static long lTime = 0L;
    public static boolean hideAnimation = true;
    private int iCivID = 0;

    public static int getRecruitable(int civID) {
        int out = 0;
        for (int i = 0; i < CFG.core.getCiv(civID).getNumOfProvs(); ++i) {
            out += CFG.gameAction.gMARY(CFG.core.getCiv(civID).getProvID(i));
        }
        return out;
    }

    public Menu_InGame_View_Recruitable() {
        int i;
        ...
        for (i = 0; i < CFG.core.getCiv(this.iCivID).getNumOfProvs(); ++i) {
            ...
            tempRecruitableTotal += CFG.gameAction.gMARY(CFG.core.getCiv(this.iCivID).getProvID(i));
        }
        while (!tempProvs.isEmpty()) {
            int tBest = 0;
            for (int i2 = 1; i2 < tempProvs.size(); ++i2) {
                if (CFG.gameAction.gMARY((Integer)tempProvs.get(tBest), this.iCivID) >= CFG.gameAction.gMARY((Integer)tempProvs.get(i2), this.iCivID)) continue;
                tBest = i2;
            }
            tempProvincesSorted.add((Integer)tempProvs.get(tBest));
            tempProvs.remove(tBest);
        }
        menuElements.add(new Button_DiplomacyAction(Images.diploArmy, CFG.map.getMapName_Just(CFG.map.getActiveMapIDN()) + ": " + CFG.lang.get("RecruitablePopulation"), 0, 0, tY, tempW, Menu_InGame_Civ_Decisions.getButtonH(), true){

            @Override
            public void actionElem(int iID) {
                CFG.menus.setVisible_InGame_ViewRecruitableAll(true);
            }
        ...
        menuElements.add(new Button_DiplomacyAction(Images.diploArmy, CFG.map.getMapName_Just(CFG.map.getActiveMapIDN()) + ": " + CFG.lang.get("TopProvinces"), 0, 0, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE(), tempW, Menu_InGame_Civ_Decisions.getButtonH(), true){

            @Override
            public void actionElem(int iID) {
                CFG.menus.setVisible_InGame_ViewRecruitableProvinces(true);
            }
        ...
        if (!tempProvincesSorted.isEmpty()) {
            menuElements.add(new ButtonN_Pop(new Color((float)CFG.core.getCiv(this.iCivID).getR() / 255.0f, (float)CFG.core.getCiv(this.iCivID).getG() / 255.0f, (float)CFG.core.getCiv(this.iCivID).getB() / 255.0f, 1.0f), CFG.core.getCiv(this.iCivID).getCivName(), this.iCivID, CFG.lang.get("RecruitablePopulation") + ": ", CFG.getNumberWthSpaces("" + tempRecruitableTotal), Images.diploArmy, CFG.COLOR_POSITIVE, 0, tY, tempW){
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("RecruitablePopulation"), CFG.BUTTON_H * 3 / 5, false, false){

            @Override
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                IMGManager.getIMG(Images.gameTopEdgeTitle).draw2O(oSB, Menu_InGame_View_Recruitable.this.getPosX() + iTranslateX, Menu_InGame_View_Recruitable.this.getPosY() - Core.PADDING - IMGManager.getIMG(Images.gameTopEdgeTitle).getHeight() - this.getHeightT(), Menu_InGame_View_Recruitable.this.getWidthM() + 2 + Core.PADDING, this.getHeightT() + Core.PADDING, true, false);
                oSB.setColor(new Color(0.13333334f, 0.30588236f, 0.3882353f, 0.165f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, nPosX + iTranslateX, nPosY - this.getHeightT() + 2 - IMGManager.getIMG(Images.line32Off1).getHeight(), nWidth, this.getHeightT() - 2, false, true);
                oSB.setColor(new Color(0.13333334f, 0.30588236f, 0.3882353f, 0.375f));
                IMGManager.getIMG(Images.gradient).drawO(oSB, nPosX + iTranslateX, nPosY - this.getHeightT() * 2 / 3 - IMGManager.getIMG(Images.gradient).getHeight(), nWidth, this.getHeightT() * 2 / 3, false, true);
                oSB.setColor(CFG.COLOR_NEW_GAME_EDGE_LINE);
                IMGManager.getIMG(Images.pix255).drawO(oSB, Menu_InGame_View_Recruitable.this.getPosX() + iTranslateX, Menu_InGame_View_Recruitable.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight(), Menu_InGame_View_Recruitable.this.getWidthM());
                oSB.setColor(new Color(CFG.COLOR_FLAG_FRAME.r, CFG.COLOR_FLAG_FRAME.g, CFG.COLOR_FLAG_FRAME.b, 0.4f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, Menu_InGame_View_Recruitable.this.getPosX() + iTranslateX, Menu_InGame_View_Recruitable.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.line32Off1).getHeight(), Menu_InGame_View_Recruitable.this.getWidthM(), 1);
                oSB.setColor(new Color(0.0f, 0.0f, 0.0f, 0.6f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, Menu_InGame_View_Recruitable.this.getPosX() + iTranslateX, Menu_InGame_View_Recruitable.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.line32Off1).getHeight() - 1, Menu_InGame_View_Recruitable.this.getWidthM(), 1);
                oSB.setColor(new Color(0.0f, 0.0f, 0.0f, 0.8f));
                IMGManager.getIMG(Images.sliderGradient).drawO(oSB, Menu_InGame_View_Recruitable.this.getPosX() + iTranslateX, Menu_InGame_View_Recruitable.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.sliderGradient).getHeight(), Menu_InGame_View_Recruitable.this.getWidthM() / 4, 1);
                IMGManager.getIMG(Images.sliderGradient).drawO(oSB, Menu_InGame_View_Recruitable.this.getPosX() + Menu_InGame_View_Recruitable.this.getWidthM() - Menu_InGame_View_Recruitable.this.getWidthM() / 4 + iTranslateX, Menu_InGame_View_Recruitable.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.sliderGradient).getHeight(), Menu_InGame_View_Recruitable.this.getWidthM() / 4, 1, true, false);
                oSB.setColor(Color.WHITE);
                IMGManager.getIMG(Images.diploArmy).drawO(oSB, nPosX + CFG.PADD * 2 + iTranslateX, Menu_InGame_View_Recruitable.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(Images.diploArmy).getHeight() / 2);
