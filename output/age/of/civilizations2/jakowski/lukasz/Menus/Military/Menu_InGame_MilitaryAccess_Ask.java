...
public class Menu_InGame_MilitaryAccess_Ask
extends Menu {
    private int iOnCivID = -1;

    public Menu_InGame_MilitaryAccess_Ask() {
    ...
    public Menu_InGame_MilitaryAccess_Ask(int onCivID) {
        ...
        menuElements.add(new ButtonN_Civs(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), CFG.getActiveCivInfoId(), 2, tY, tempWidth - 4){
            ...
            public int getWidthE() {
                return Menu_InGame_MilitaryAccess_Ask.this.getElementW() * 2;
            }
        });
        ((MenuElemUI)menuElements.get(menuElements.size() - 1)).setCurr(1);
        menuElements.add(new ButtonN_Opinion(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), CFG.getActiveCivInfoId(), Images.diploAccessHas, 0, GameValues.gvDipMilitaryAccess.COST_OFFER_MILITARY_ACCESS_ASK_DIPLOMACY_POINTS, 2, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE(), CFG.BUTTON_W * 2){
        ...
        menuElements.add(new Text_Desc(CFG.lang.get("MilitaryAccessAskDesc"), 2, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, tempWidth - 4){
        ...
        tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
        menuElements.add(new Slider_InGame_Date(CFG.lang.get("NumberOfTurns"), CFG.PADD * 2, ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, tempWidth - CFG.PADD * 3 - CFG.BUTTON_W, Math.max(CFG.BUTTON_H * 4 / 5, CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 2 + CFG.PADD * 4), 1, GameValues.gvDipMilitaryAccess.DIPLOMACY_MAX_NUMBER_OF_TURNS_FOR_MILITARY_ACCESS, GameValues.gvDipMilitaryAccess.DIPLOMACY_MAX_NUMBER_OF_TURNS_FOR_MILITARY_ACCESS, 0.65f){

            @Override
            public int getWidthE() {
                return Menu_InGame_MilitaryAccess_Ask.this.getElementW() * 2 - CFG.PADD * 4;
            }
            ...
            public Color getColorLEFT() {
                return new Color(CFG.diplomacyColors_GameData.COLOR_DIPLOMACY_MILITARY_ACCESS.getR(), CFG.diplomacyColors_GameData.COLOR_DIPLOMACY_MILITARY_ACCESS.getG(), CFG.diplomacyColors_GameData.COLOR_DIPLOMACY_MILITARY_ACCESS.getB(), 0.65f);
            }
        });
        ...
        menuElements.add(new Button_InGameAction(CFG.lang.get("Cancel"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_MilitaryAccess_Ask.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }
        });
        menuElements.add(new Button_InGameAction(CFG.lang.get("SendProposal"), -1, 2, tY, CFG.BUTTON_W, true){
            ...
            @Override
            public int getPosXE() {
                return Menu_InGame_MilitaryAccess_Ask.this.getElementW() + CFG.PADD / 2;
            }
            ...
            public void buildElemHover() {
                ...
                nData.add(new ME_Hover_2Type_Flag_Big(Menu_InGame_MilitaryAccess_Ask.this.iOnCivID, CFG.PADD, CFG.PADD));
                nData.add(new ME_Hover_2Type_Text_Big(CFG.core.getCiv(Menu_InGame_MilitaryAccess_Ask.this.iOnCivID).getCivName()));
                nElements.add(new MEHover_2E(nData));
                nData.clear();
                nData.add(new ME_Hover_2Type_Text(CFG.lang.get("DiplomacyPoints") + ": ", CFG.COLOR_HOVER_TITLE));
                nData.add(new ME_Hover_2Type_Text("-" + (float)GameValues.gvDipMilitaryAccess.COST_OFFER_MILITARY_ACCESS_ASK_DIPLOMACY_POINTS / 10.0f, CFG.COLOR_NEGATIVE_2));
            ...
            public void drawTextE(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean isActive) {
                IMGManager.getIMG(Images.diploAccessHas).drawO(oSB, this.getPosXE() + this.getWidthE() / 2 - (this.getTextWidthU() + IMGManager.getIMG(Images.diploAccessHas).getWidth() + CFG.PADD) / 2 + iTranslateX, this.getPosY() + this.getHeightE() / 2 - IMGManager.getIMG(Images.diploAccessHas).getHeight() / 2 + iTranslateY);
                Renderer.drawText(oSB, this.fontID, this.getTextE(), this.getPosXE() + (this.getTextPosElem() < 0 ? this.getWidthE() / 2 - (this.getTextWidthU() + IMGManager.getIMG(Images.diploAccessHas).getWidth() + CFG.PADD) / 2 + IMGManager.getIMG(Images.diploAccessHas).getWidth() + CFG.PADD : this.getTextPosElem()) + iTranslateX, this.getPosY() + this.getHeightE() / 2 - this.getTextHeight() / 2 + iTranslateY, this.getColorE(isActive));
            }

            @Override
            public boolean getIsClickable() {
                return CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getDiploPoints() >= GameValues.gvDipMilitaryAccess.COST_OFFER_MILITARY_ACCESS_ASK_DIPLOMACY_POINTS;
            }
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("AskForMilitaryAccess"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                oSB.setColor(new Color(0.33333334f, 0.41960785f, 0.17254902f, 0.165f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, nPosX + iTranslateX, nPosY - this.getHeightT() + 2 - IMGManager.getIMG(Images.line32Off1).getHeight(), nWidth, this.getHeightT() - 2, false, true);
                oSB.setColor(new Color(0.33333334f, 0.41960785f, 0.17254902f, 0.375f));
                ...
                int imgID = Images.diploAccessHas;
                IMGManager.getIMG(imgID).drawO(oSB, nPosX + (nWidth - (this.getTextWidth() + IMGManager.getIMG(imgID).getWidth() + CFG.PADD)) / 2 + iTranslateX, Menu_InGame_MilitaryAccess_Ask.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(imgID).getHeight() / 2);
        ...
        this.updateLang();
        this.getMenuElem(3).setCurr(GameValues.gvDipMilitaryAccess.DIPLOMACY_MAX_NUMBER_OF_TURNS_FOR_MILITARY_ACCESS);
    ...
    public final void actionEL(int iID) {
        if (iID == this.getMenuElemsSize() - 1) {
            GameManager.sendMilitaryAccess_AskProposal(this.iOnCivID, CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), this.getMenuElem(3).getCurr());
