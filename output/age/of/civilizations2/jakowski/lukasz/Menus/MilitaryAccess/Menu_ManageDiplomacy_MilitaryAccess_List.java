 ...
 */
package age.of.civilizations2.jakowski.lukasz.Menus.MilitaryAccess;
...
import age.of.civilizations2.jakowski.lukasz.Sliders.InGame.Diplomacy.Slider_MilitartAccess;
import age.of.civilizations2.jakowski.lukasz.Title.TitleM;
import com.badlogic.gdx.graphics.Color;
import com.badlogic.gdx.graphics.g2d.SpriteBatch;
import java.util.ArrayList;

public class Menu_ManageDiplomacy_MilitaryAccess_List
extends Menu {
    public Menu_ManageDiplomacy_MilitaryAccess_List() {
        ...
        if (CFG.MANAGE_DIPLOMACY_CUSTOMIZE_ALLIANCE_ID > 0) {
            ...
            for (int i = 1; i < CFG.core.getCivsSize(); ++i) {
                if (i == CFG.MANAGE_DIPLOMACY_CUSTOMIZE_ALLIANCE_ID || CFG.core.getMilitaryAccess(CFG.MANAGE_DIPLOMACY_CUSTOMIZE_ALLIANCE_ID, i) <= 0) continue;
                menuElements.add(new Slider_MilitartAccess(i, CFG.MANAGE_DIPLOMACY_CUSTOMIZE_ALLIANCE_ID, CFG.PADD, CFG.PADD * (multiplePosY + 1) + CFG.BUTTON_H * multiplePosY + CFG.PADD, tempW - CFG.BUTTON_H * 3 / 4 - CFG.PADD * 2, CFG.BUTTON_H - CFG.PADD * 2, 1, GameValues.gvDipMilitaryAccess.DIPLOMACY_MAX_NUMBER_OF_TURNS_FOR_MILITARY_ACCESS, CFG.core.getMilitaryAccess(CFG.MANAGE_DIPLOMACY_CUSTOMIZE_ALLIANCE_ID, i)));
        ...
        this.initMenu(new TitleM(null, CFG.BUTTON_H * 3 / 4, false, false){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                IMGManager.getIMG(Images.gameTopEdgeTitle).draw2O(oSB, Menu_ManageDiplomacy_MilitaryAccess_List.this.getPosX() + iTranslateX, Menu_ManageDiplomacy_MilitaryAccess_List.this.getPosY() - Core.PADDING - IMGManager.getIMG(Images.gameTopEdgeTitle).getHeight() - this.getHeightT(), Menu_ManageDiplomacy_MilitaryAccess_List.this.getWidthM() + 2 + Core.PADDING, this.getHeightT() + Core.PADDING, true, false);
                oSB.setColor(new Color(0.011f, 0.014f, 0.019f, 0.25f));
                IMGManager.getIMG(Images.gradient).drawO(oSB, Menu_ManageDiplomacy_MilitaryAccess_List.this.getPosX() + iTranslateX, Menu_ManageDiplomacy_MilitaryAccess_List.this.getPosY() - IMGManager.getIMG(Images.gradient).getHeight() - this.getHeightT() * 3 / 4, Menu_ManageDiplomacy_MilitaryAccess_List.this.getWidthM(), this.getHeightT() * 3 / 4, false, true);
                oSB.setColor(CFG.COLOR_NEW_GAME_EDGE_LINE);
                IMGManager.getIMG(Images.pix255).drawO(oSB, Menu_ManageDiplomacy_MilitaryAccess_List.this.getPosX() + iTranslateX, Menu_ManageDiplomacy_MilitaryAccess_List.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight(), Menu_ManageDiplomacy_MilitaryAccess_List.this.getWidthM());
                oSB.setColor(new Color(CFG.COLOR_FLAG_FRAME.r, CFG.COLOR_FLAG_FRAME.g, CFG.COLOR_FLAG_FRAME.b, 0.4f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, Menu_ManageDiplomacy_MilitaryAccess_List.this.getPosX() + iTranslateX, Menu_ManageDiplomacy_MilitaryAccess_List.this.getPosY() - IMGManager.getIMG(Images.pix255).getHeight() - IMGManager.getIMG(Images.line32Off1).getHeight(), Menu_ManageDiplomacy_MilitaryAccess_List.this.getWidthM(), 1);
    ...
    public void updateLang() {
        this.getTitleM().setText(CFG.lang.get("MilitaryAccess"));
    }
    ...
    public void actionEL(int iID) {
        if (iID % 2 == 0) {
            this.updateMilitary(iID / 2, this.getMenuElem(iID).getCurr());
        } else if (iID % 2 == 1) {
            this.updateMilitary(iID / 2, 0);
            CFG.menus.rebuildManageDiplomacy_MilitaryAccess_List();
        }
    }

    private final void updateMilitary(int pactID, int iNumOfTurns) {
        if (CFG.MANAGE_DIPLOMACY_CUSTOMIZE_ALLIANCE_ID > 0) {
            int foundPacts = 0;
            for (int i = 1; i < CFG.core.getCivsSize(); ++i) {
                if (i == CFG.MANAGE_DIPLOMACY_CUSTOMIZE_ALLIANCE_ID || CFG.core.getMilitaryAccess(CFG.MANAGE_DIPLOMACY_CUSTOMIZE_ALLIANCE_ID, i) <= 0) continue;
                if (foundPacts == pactID) {
                    CFG.core.setMilitaryAccess(CFG.MANAGE_DIPLOMACY_CUSTOMIZE_ALLIANCE_ID, i, iNumOfTurns);
                    return;
                }
                ++foundPacts;
            }
