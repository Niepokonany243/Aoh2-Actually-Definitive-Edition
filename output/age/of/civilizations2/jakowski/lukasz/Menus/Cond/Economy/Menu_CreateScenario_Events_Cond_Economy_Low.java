...
package age.of.civilizations2.jakowski.lukasz.Menus.Cond.Economy;
...
public class Menu_CreateScenario_Events_Cond_Economy_Low
extends Menu {
    public Menu_CreateScenario_Events_Cond_Economy_Low() {
        ...
        menuElements.add(new Slider_BG(0, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD * 2, CFG.GAMEWIDTH, CFG.BUTTON_H - CFG.PADD * 2, 0, 10000, CFG.eventsManager.createScenarioEvents.getTrigger((int)CFG.eventsManager.createEvent_EditTriggerID).lConditions.get(CFG.eventsManager.createEvent_EditConditionID).getValue() / 1000){

            @Override
            public String getDrawText() {
                return this.getTextE() + this.getCurr() * 1000;
            }
        });
    ...
    public void updateLang() {
        ...
        this.getMenuElem(5).setTextE(CFG.lang.get("Economy") + " < ");
        this.getTitleM().setText(CFG.lang.get("Economy"));
    }
    ...
    public void draw(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean sliderMenuIsActive) {
        ...
        int tempButtonID = 4;
        try {
            CFG.core.getCiv(CFG.eventsManager.createScenarioEvents.getTrigger((int)CFG.eventsManager.createEvent_EditTriggerID).lConditions.get(CFG.eventsManager.createEvent_EditConditionID).getCivID()).getFlagC().drawO(oSB, this.getMenuElem(tempButtonID).getPosXE() + this.getMenuElem(tempButtonID).getTextPosElem() / 2 - CFG.CIV_FLAG_WIDTH / 2 + iTranslateX, this.getMenuElem(tempButtonID).getPosY() + this.getMenuPosY() + this.getMenuElem(tempButtonID).getHeightE() / 2 - CFG.CIV_FLAG_HEIGHT / 2 + iTranslateY);
        }
        catch (IndexOutOfBoundsException ex) {
            IMGManager.getIMG(Images.randomCivilizationFlag).drawO(oSB, this.getMenuElem(tempButtonID).getPosXE() + this.getMenuElem(tempButtonID).getTextPosElem() / 2 - CFG.CIV_FLAG_WIDTH / 2 + iTranslateX, this.getMenuElem(tempButtonID).getPosY() + this.getMenuPosY() + this.getMenuElem(tempButtonID).getHeightE() / 2 - CFG.CIV_FLAG_HEIGHT / 2 + iTranslateY);
        }
    ...
    public final void actionEL(int iID) {
        switch (iID) {
            ...
            case 4: {
                CFG.eventsManager.eSelectCivAction = Event_SelectCivAction.SELECT_CIV_ECONOMY_LOW;
                CFG.menus.setMenuID(View.eCREATE_SCENARIO_EVENTS_SELECT_CIV);
                break;
            }
            case 5: {
                CFG.eventsManager.createScenarioEvents.getTrigger((int)CFG.eventsManager.createEvent_EditTriggerID).lConditions.get(CFG.eventsManager.createEvent_EditConditionID).setValue(this.getMenuElem(iID).getCurr() * 1000);
            }
        }
    }
