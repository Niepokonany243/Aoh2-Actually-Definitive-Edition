...
package age.of.civilizations2.jakowski.lukasz.Menus.Cond.Relation;
...
public class Menu_CreateScenario_Events_Cond_Relation_Low
extends Menu {
    public Menu_CreateScenario_Events_Cond_Relation_Low() {
        ...
        menuElements.add(new Button_Classic(null, (int)(50.0f * CFG.GUI_SCALE), 0, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, CFG.GAMEWIDTH, CFG.BUTTON_H, true));
        menuElements.add(new Slider_BG(0, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD * 2, CFG.GAMEWIDTH, CFG.BUTTON_H - CFG.PADD * 2, -100, 100, CFG.eventsManager.createScenarioEvents.getTrigger((int)CFG.eventsManager.createEvent_EditTriggerID).lConditions.get(CFG.eventsManager.createEvent_EditConditionID).getValue()){

            @Override
            public String getDrawText() {
                return this.getTextE() + (this.getCurr() > 0 ? "+" : "") + this.getCurr();
            }
        });
    ...
    public void updateLang() {
        ...
        try {
            this.getMenuElem(5).setTextE(CFG.eventsManager.createScenarioEvents.getTrigger((int)CFG.eventsManager.createEvent_EditTriggerID).lConditions.get(CFG.eventsManager.createEvent_EditConditionID).getCivID2() > 0 ? CFG.lang.get("Civilization") + ": " + CFG.core.getCiv(CFG.eventsManager.createScenarioEvents.getTrigger((int)CFG.eventsManager.createEvent_EditTriggerID).lConditions.get(CFG.eventsManager.createEvent_EditConditionID).getCivID2()).getCivName() : CFG.lang.get("SelectCivilization"));
        }
        catch (IndexOutOfBoundsException ex) {
            this.getMenuElem(5).setTextE(CFG.lang.get("SelectCivilization"));
        }
        this.getMenuElem(6).setTextE(CFG.lang.get("Relation") + " < ");
        this.getTitleM().setText(CFG.lang.get("Relation"));
    }
    ...
    public void draw(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean sliderMenuIsActive) {
        ...
        tempButtonID = 5;
        try {
            CFG.core.getCiv(CFG.eventsManager.createScenarioEvents.getTrigger((int)CFG.eventsManager.createEvent_EditTriggerID).lConditions.get(CFG.eventsManager.createEvent_EditConditionID).getCivID2()).getFlagC().drawO(oSB, this.getMenuElem(tempButtonID).getPosXE() + this.getMenuElem(tempButtonID).getTextPosElem() / 2 - CFG.CIV_FLAG_WIDTH / 2 + iTranslateX, -CFG.core.getCiv(CFG.eventsManager.createScenarioEvents.getTrigger((int)CFG.eventsManager.createEvent_EditTriggerID).lConditions.get(CFG.eventsManager.createEvent_EditConditionID).getCivID2()).getFlagC().getHeight() + this.getMenuElem(tempButtonID).getPosY() + this.getMenuPosY() + this.getMenuElem(tempButtonID).getHeightE() / 2 - CFG.CIV_FLAG_HEIGHT / 2 + iTranslateY, CFG.CIV_FLAG_WIDTH, CFG.CIV_FLAG_HEIGHT);
        }
        catch (IndexOutOfBoundsException ex) {
            IMGManager.getIMG(Images.randomCivilizationFlag).drawO(oSB, this.getMenuElem(tempButtonID).getPosXE() + this.getMenuElem(tempButtonID).getTextPosElem() / 2 - CFG.CIV_FLAG_WIDTH / 2 + iTranslateX, this.getMenuElem(tempButtonID).getPosY() - IMGManager.getIMG(Images.randomCivilizationFlag).getHeight() + this.getMenuPosY() + this.getMenuElem(tempButtonID).getHeightE() / 2 - CFG.CIV_FLAG_HEIGHT / 2 + iTranslateY, CFG.CIV_FLAG_WIDTH, CFG.CIV_FLAG_HEIGHT);
        }
    ...
    public final void actionEL(int iID) {
        switch (iID) {
            ...
            case 4: {
                CFG.eventsManager.eSelectCivAction = Event_SelectCivAction.SELECT_CIV_RELATION_LOW;
                CFG.menus.setMenuID(View.eCREATE_SCENARIO_EVENTS_SELECT_CIV);
                break;
            }
            case 5: {
                CFG.eventsManager.eSelectCivAction = Event_SelectCivAction.SELECT_CIV_RELATION_LOW2;
                CFG.menus.setMenuID(View.eCREATE_SCENARIO_EVENTS_SELECT_CIV);
                break;
            }
            case 6: {
