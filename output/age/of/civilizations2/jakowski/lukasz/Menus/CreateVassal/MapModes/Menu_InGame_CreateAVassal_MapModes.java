...
public class Menu_InGame_CreateAVassal_MapModes
extends Menu {
    public static final int ANIMATION_TIME = 155;
    private long lTime = 0L;

    public Menu_InGame_CreateAVassal_MapModes() {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        int tempElemH = CFG.isAndroid() ? CFG.BUTTON_H * 3 / 5 : Math.max(CFG.BUTTON_H, CFG.BUTTON_H * 3 / 5);
        for (int i = 0; i < 7; ++i) {
            if (i % 2 == 0) {
        ...
        }, -1, -1, CFG.BUTTON_W * 2 + CFG.BUTTON_W * 3 / 4, Math.min(Math.min(tempElemH * 5 + tempElemH / 2, CFG.GAMEHEIGHT - CFG.BUTTON_H), menuElements.size() * tempElemH), menuElements, false, true);
        this.updateLang();
    }
    ...
    public void updateLang() {
        ...
        this.getMenuElem(1).setCurr(MapModesManager.VIEW_ECONOMY_MODE);
        this.getMenuElem(2).setTextE(CFG.lang.get("Population"));
        this.getMenuElem(2).setCurr(MapModesManager.VIEW_POPULATION_MODE);
        this.getMenuElem(3).setTextE(CFG.lang.get("Diplomacy"));
        this.getMenuElem(3).setCurr(MapModesManager.VIEW_DIPLOMACY_MODE);
        this.getMenuElem(4).setTextE(CFG.lang.get("TerrainType"));
        this.getMenuElem(4).setCurr(MapModesManager.VIEW_TERRAIN_TYPE_MODE);
        this.getMenuElem(5).setTextE(CFG.lang.get("GrowthRate"));
        this.getMenuElem(5).setCurr(MapModesManager.VIEW_GROWTH_RATE_MODE);
        this.getMenuElem(6).setTextE(CFG.lang.get("Development"));
        this.getMenuElem(6).setCurr(MapModesManager.VIEW_DEVELOPMENT_MODE);
    }
