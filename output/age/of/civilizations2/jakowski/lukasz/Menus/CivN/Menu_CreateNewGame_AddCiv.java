...
public class Menu_CreateNewGame_AddCiv
extends Menu {
    ...
    public static Image civFlag = null;
    public static boolean addProvinceMode = true;

    public Menu_CreateNewGame_AddCiv() {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        int tempW = CFG.CIV_INFO_MENU_WIDTH;
        int tY = CFG.PADD;
        int tempElemH = Menu_InGame_Civ_Decisions.getButtonH();
        menuElements.add(new Button_InGameBox(CFG.lang.get("CreateaCivilization"), -1, CFG.PADD, tY, tempW - CFG.PADD * 2, true){
        ...
        menuElements.add(new Button_InGameBox(CFG.lang.get("SelectCivilization"), -1, CFG.PADD, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, tempW - CFG.PADD * 2, true){
            ...
            @Override
            public void actionElem(int iID) {
                CFG.menus.setMenuID(View.eNEW_GAME_ADD_CIV_SELECT_CIV);
                CFG.map.getMpB().updateWorldMap_Shaders();
            }
        });
        ...
        if (civTag != null) {
            menuElements.add(new Button_Diplomacy_Action_Government(CFG.ideologiesMgr.getIdeologyID(civTag), CFG.lang.get("ChangeTypeOfGovernment"), 0, 2, tY, tempW - 4, tempElemH, true){
                ...
                @Override
                public void actionElem(int iID) {
                    if (!CFG.menus.getVisible_CreateNewGame_AddCiv_Gov()) {
                        CFG.menus.setVisible_CreateNewGame_AddCiv_Gov(true);
                    } else {
            ...
            menuElements.add(new Button_InGameAction_Animated(CFG.lang.get("AddCivilization"), -1, CFG.PADD, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, tempW - CFG.PADD * 2, true){
                ...
                @Override
                public void actionElem(int iID) {
                    Menu_CreateNewGame_AddCiv.this.addCivilization();
                }
            ...
            if (provinces.isEmpty()) {
                ...
                for (int a = 0; a < provinces.size(); ++a) {
                    menuElements.add(new Button_View_PopulationEconomy(0, CFG.core.getProv(provinces.get(a)).getName(), provinces.get(a), 2, tY, tempW - 4, false){
                        ...
                        @Override
                        public void actionElem(int iID) {
                            Menu_CreateNewGame_AddCiv.removeProvince(this.getCurr());
                            CFG.menus.setVisible_CreateNewGame_AddCiv(true);
                        }
        ...
        }, AoCGame.LEFT + CFG.PADD * 2, IMGManager.getIMG(Images.topBar2).getHeight() + CFG.PADD * 2 + CFG.BUTTON_H * 3 / 4, tempW, Math.min(CFG.GAMEHEIGHT - CFG.BUTTON_H, ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD), menuElements, false, true);
        this.updateLang();
    }
    ...
    public final void loadFlag() {
        block13: {
            if (civFlag != null || civTag == null || civTag.isEmpty()) {
                return;
            }
            try {
                try {
                    if (FileManager.loadFile("game/flagsXH/" + civTag + ".png").exists()) {
                        civFlag = new Image(new Texture(FileManager.loadFile("game/flagsXH/" + civTag + ".png"), Pixmap.Format.RGB888, false), Texture.TextureFilter.Linear);
                        break block13;
                    }
                    if (FileManager.loadFile("game/flagsXH/" + CFG.ideologiesMgr.getRealTag(civTag) + ".png").exists()) {
                        civFlag = new Image(new Texture(FileManager.loadFile("game/flagsXH/" + CFG.ideologiesMgr.getRealTag(civTag) + ".png"), Pixmap.Format.RGB888, false), Texture.TextureFilter.Linear);
                        break block13;
                    }
                    if (FileManager.loadFile("game/flagsH/" + civTag + ".png").exists()) {
                        civFlag = new Image(new Texture(FileManager.loadFile("game/flagsH/" + civTag + ".png"), Pixmap.Format.RGB888, false), Texture.TextureFilter.Linear);
                        break block13;
                    }
                    if (FileManager.loadFile("game/flagsH/" + CFG.ideologiesMgr.getRealTag(civTag) + ".png").exists()) {
                        civFlag = new Image(new Texture(FileManager.loadFile("game/flagsH/" + CFG.ideologiesMgr.getRealTag(civTag) + ".png"), Pixmap.Format.RGB888, false), Texture.TextureFilter.Linear);
                        break block13;
                    }
                    if (FileManager.loadFile("game/flags/" + civTag + ".png").exists()) {
                        civFlag = new Image(new Texture(FileManager.loadFile("game/flags/" + civTag + ".png"), Pixmap.Format.RGB888, false), Texture.TextureFilter.Nearest);
                        break block13;
                    }
                    if (FileManager.loadFile("game/flags/" + CFG.ideologiesMgr.getRealTag(civTag) + ".png").exists()) {
                        civFlag = new Image(new Texture(FileManager.loadFile("game/flags/" + CFG.ideologiesMgr.getRealTag(civTag) + ".png"), Pixmap.Format.RGB888, false), Texture.TextureFilter.Nearest);
                        break block13;
                    }
                    if (CFG.isAndroid() && FileManager.loadFile("game/civilizations_editor/" + CFG.ideologiesMgr.getRealTag(civTag) + "/" + CFG.ideologiesMgr.getRealTag(civTag) + "_FL.png").exists()) {
                        civFlag = new Image(new Texture(FileManager.loadFile("game/civilizations_editor/" + CFG.ideologiesMgr.getRealTag(civTag) + "/" + CFG.ideologiesMgr.getRealTag(civTag) + "_FL.png"), Pixmap.Format.RGB888, false), Texture.TextureFilter.Nearest);
                        break block13;
                    }
                    ...
                    civFlag = new Image(new Texture(FileManager.loadFile("game/flagsXH/ran.png"), Pixmap.Format.RGB888, false), Texture.TextureFilter.Nearest);
                }
                catch (RuntimeException ex) {
                    civFlag = new Image(new Texture(FileManager.loadFile("game/flagsXH/ran.png")), Texture.TextureFilter.Nearest);
                }
            }
            catch (Exception exception) {
                // empty catch block
    ...
    public void addCivilization() {
        try {
            ...
            if (CFG.core.createScenarioAddCivilization(civTag, provinceID, false, true, false)) {
                ...
                CFG.core.getPlayer(0).setCivId(civID);
