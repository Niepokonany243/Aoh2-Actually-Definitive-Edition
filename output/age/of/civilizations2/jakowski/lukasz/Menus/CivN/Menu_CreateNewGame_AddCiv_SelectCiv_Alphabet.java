...
public class Menu_CreateNewGame_AddCiv_SelectCiv_Alphabet
extends Menu {
    private List<Character> lCharacters;
    private String nSearch = null;

    public Menu_CreateNewGame_AddCiv_SelectCiv_Alphabet() {
        int a;
        boolean addChar;
        int i;
        ...
        menuElements.add(new Button_Classic_Classic_Search("", CFG.PADD * 2, 0, CFG.PADD, CFG.BUTTON_W * 2, CFG.BUTTON_H, true){
            ...
            public String getTextToDrawElem() {
                return Menu_CreateNewGame_AddCiv_SelectCiv_Alphabet.this.nSearch + ": " + super.getTextToDrawElem();
            }
        });
        ...
        for (i = 0; i < iSize; ++i) {
            if (CFG.isInTheGame_OrIsFormableCiv(tagsSPLITED[i])) continue;
            addChar = true;
        ...
        for (i = 0; i < iSize; ++i) {
            if (CFG.isInTheGame_OrIsFormableCiv(tagsSPLITED_ED[i])) continue;
            addChar = true;
            for (a = 0; a < this.lCharacters.size(); ++a) {
