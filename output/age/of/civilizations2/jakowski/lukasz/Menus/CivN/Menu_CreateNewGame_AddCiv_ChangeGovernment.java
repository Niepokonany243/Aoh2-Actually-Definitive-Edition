...
public class Menu_CreateNewGame_AddCiv_ChangeGovernment
extends Menu {
    public Menu_CreateNewGame_AddCiv_ChangeGovernment() {
    ...
    public Menu_CreateNewGame_AddCiv_ChangeGovernment(int nID) {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        menuElements.add(new Button_InGameAction(CFG.lang.get("Close"), -1, 2, tY += CFG.PADD, CFG.BUTTON_W, true){

            @Override
            public int getWidthE() {
                return Menu_CreateNewGame_AddCiv_ChangeGovernment.this.getW();
            }

            @Override
            public void actionElem(int iID) {
                Menu_CreateNewGame_AddCiv_ChangeGovernment.this.setVisibleM(false);
            }
        });
        tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD;
        List<Boolean> canChangeToIdeology = CFG.ideologiesMgr.canChangeToIdeology_All();
        int j = 0;
        for (int i = 0; i < CFG.ideologiesMgr.getIdeologiesSize(); ++i) {
            if (CFG.ideologiesMgr.getIdeologyID((int)i).REVOLUTIONARY || !canChangeToIdeology.get(i).booleanValue()) continue;
            menuElements.add(new Button_Diplomacy_ChangeGovernment2_NewGame(i, 2, tY, CFG.BUTTON_W * 2, true){
                ...
                @Override
                public void actionElem(int iID) {
                    if (Menu_CreateNewGame_AddCiv.civTag != null && !Menu_CreateNewGame_AddCiv.civTag.isEmpty()) {
                        Menu_CreateNewGame_AddCiv.setCivTag(CFG.ideologiesMgr.getRealTag(Menu_CreateNewGame_AddCiv.civTag) + CFG.ideologiesMgr.getIdeologyID(this.getCurr()).getExtraTag());
                    }
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("Government"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                oSB.setColor(new Color(CFG.COLOR_RESEARCH.r, CFG.COLOR_RESEARCH.g, CFG.COLOR_RESEARCH.b, 0.165f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, nPosX + iTranslateX, nPosY - this.getHeightT() + 2 - IMGManager.getIMG(Images.line32Off1).getHeight(), nWidth, this.getHeightT() - 2, false, true);
                oSB.setColor(new Color(CFG.COLOR_RESEARCH.r, CFG.COLOR_RESEARCH.g, CFG.COLOR_RESEARCH.b, 0.375f));
