...
public class Menu_InGame_Loans
extends Menu {
    private int iOnCivID = -1;

    public Menu_InGame_Loans() {
        ...
        this.initMenu(new TitleM(CFG.lang.get("Loans"), CFG.BUTTON_H * 3 / 5, true, true), CFG.GAMEWIDTH / 2 - tempWidth / 2, tempMenuPosY, tempWidth, ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD + tempMenuPosY > CFG.GAMEHEIGHT - CFG.BUTTON_H - CFG.PADD * 2 ? Math.max(CFG.GAMEHEIGHT - CFG.BUTTON_H - CFG.PADD * 2 - tempMenuPosY, (CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 2) * 6) : ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, menuElements, false, true);
        this.updateLang();
    }

    public Menu_InGame_Loans(int onCivID) {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        this.iOnCivID = onCivID;
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        if (CFG.core.getCiv(onCivID).getLoansSize() > 0) {
            for (int i = 0; i < CFG.core.getCiv(onCivID).getLoansSize(); ++i) {
                menuElements.add(new Buton_Diplomacy_Loan(i, onCivID, CFG.core.getCiv((int)onCivID).getLoan((int)i).iGoldPerTurn, CFG.core.getCiv((int)onCivID).getLoan((int)i).iGoldPerTurn * CFG.core.getCiv((int)onCivID).getLoan((int)i).iTurnsLeft, CFG.core.getCiv((int)onCivID).getLoan((int)i).iTurnsLeft, 2, tY, CFG.BUTTON_W * 2){

                    @Override
                    public int getWidthE() {
                        return Menu_InGame_Loans.this.getElementW() * 2;
                    }

                    @Override
                    public void actionElem(int iID) {
                        CFG.menus.rebuildInGame_RepayLoan(Menu_InGame_Loans.this.iOnCivID, iID);
                    }

                    @Override
                    public void actionElemPPM() {
                        try {
                            int iLoanValue = CFG.core.getCiv((int)((Menu_InGame_Loans)Menu_InGame_Loans.this).iOnCivID).getLoan((int)this.id).iGoldPerTurn * CFG.core.getCiv((int)((Menu_InGame_Loans)Menu_InGame_Loans.this).iOnCivID).getLoan((int)this.id).iTurnsLeft;
                            if (CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getGold() >= (long)iLoanValue) {
                                Loans.repayLoan(Menu_InGame_Loans.this.iOnCivID, this.id);
                                CFG.menus.rebuildInGame_Loans(Menu_InGame_Loans.this.iOnCivID);
        ...
        menuElements.add(new Button_InGameAction(CFG.lang.get("Close"), -1, CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_Loans.this.getElementW() * 2 - CFG.PADD * 2;
            }
        });
        int tempMenuPosY = IMGManager.getIMG(Images.topBar).getHeight() + CFG.PADD * 2 + CFG.BUTTON_H * 3 / 4;
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("Loans"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                oSB.setColor(new Color(0.49803922f, 0.16470589f, 0.16470589f, 0.165f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, nPosX + iTranslateX, nPosY - this.getHeightT() + 2 - IMGManager.getIMG(Images.line32Off1).getHeight(), nWidth, this.getHeightT() - 2, false, true);
                oSB.setColor(new Color(0.49803922f, 0.16470589f, 0.16470589f, 0.375f));
                ...
                IMGManager.getIMG(Images.diploLoan).drawO(oSB, nPosX + (nWidth - (this.getTextWidth() + IMGManager.getIMG(Images.diploLoan).getWidth() + CFG.PADD)) / 2 + iTranslateX, Menu_InGame_Loans.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(Images.diploLoan).getHeight() / 2);
                Renderer.drawText(oSB, CFG.FONT_BOLD_SMALL, this.getText(), nPosX + (nWidth - (this.getTextWidth() + IMGManager.getIMG(Images.diploLoan).getWidth() + CFG.PADD)) / 2 + IMGManager.getIMG(Images.diploLoan).getWidth() + CFG.PADD + iTranslateX, 2 + nPosY - this.getHeightT() + (this.getHeightT() - this.getTextHeight()) / 2, Color.WHITE);
            }
