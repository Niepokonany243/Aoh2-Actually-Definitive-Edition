...
public class Menu_InGame_Message_LoanRequest
extends Menu {
    ...
    public Menu_InGame_Message_LoanRequest(int onCivID, int iMessageID, int iGold, int iTurns) {
        ...
        menuElements.add(new ButtonN_Civs(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), this.iOnCivID, 2, tY, tempWidth - 4){
            ...
            public int getWidthE() {
                return Menu_InGame_Message_LoanRequest.this.getElementW() * 2;
            }
        });
        ((MenuElemUI)menuElements.get(menuElements.size() - 1)).setCurr(1);
        tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
        menuElements.add(new Text_Desc(CFG.lang.get("LoanRequestDesc"), 2, tY += CFG.PADD, tempWidth - 4){
        ...
        menuElements.add(new Button_Diplomacy_TakeLoan_Interest(CFG.lang.get("Duration") + ": " + CFG.lang.get("TurnsX", iTurns) + ", " + CFG.lang.get("Interest") + ": ", "" + CFG.getNumberWthSpaces("" + iGold), 2, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, CFG.BUTTON_W * 2){
        ...
        menuElements.add(new Button_InGameAction(CFG.lang.get("Refuse"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_Message_LoanRequest.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }
        });
        menuElements.add(new Button_InGameAction(CFG.lang.get("Accept"), -1, 2, tY, CFG.BUTTON_W, true){
            ...
            @Override
            public int getPosXE() {
                return Menu_InGame_Message_LoanRequest.this.getElementW() + CFG.PADD / 2;
            }
            ...
            public void buildElemHover() {
                ...
                nData.add(new ME_Hover_2Type_Flag_Big(Menu_InGame_Message_LoanRequest.this.iOnCivID, CFG.PADD, 0));
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("LoanRequest"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                oSB.setColor(new Color(CFG.COLOR_MESSAGE_TITLE.r * 0.75f, CFG.COLOR_MESSAGE_TITLE.g * 0.75f, CFG.COLOR_MESSAGE_TITLE.b * 0.75f, 0.165f));
                IMGManager.getIMG(Images.line32Off1).drawO(oSB, nPosX + iTranslateX, nPosY - this.getHeightT() + 2 - IMGManager.getIMG(Images.line32Off1).getHeight(), nWidth, this.getHeightT() - 2, false, true);
                oSB.setColor(new Color(CFG.COLOR_MESSAGE_TITLE.r * 0.75f, CFG.COLOR_MESSAGE_TITLE.g * 0.75f, CFG.COLOR_MESSAGE_TITLE.b * 0.75f, 0.375f));
                ...
                IMGManager.getIMG(Images.diploMessage).drawO(oSB, Menu_InGame_Message_LoanRequest.this.getPosX() + CFG.PADD * 2 + iTranslateX, Menu_InGame_Message_LoanRequest.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(Images.diploMessage).getHeight() / 2);
        ...
        this.getMenuElem(2).setCurr((int)(Loans.takeLoan_InterestRate(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), iGold, iTurns) * 100.0f));
        this.getMenuElem(2).setMin((int)((float)iGold * Loans.takeLoan_InterestRate(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), iGold, iTurns) / 100.0f));
    }
    ...
    public final void actionEL(int iID) {
        if (iID == this.getMenuElemsSize() - 1) {
            ...
            CFG.toastM.addM(CFG.lang.get("LoanApproved") + "!", CFG.COLOR_POSITIVE);
            CFG.toastM.setTimeInView(3500);
            CFG.menus.rebuildMenu_InGame_Infobox(CFG.lang.get("LoanApproved"), this.iOnCivID, CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), Images.infoDiplomacy);
        ...
        if (iID == this.getMenuElemsSize() - 2) {
            ...
            CFG.toastM.addM(CFG.lang.get("LoanProposalRejected"), CFG.COLOR_TEXT_NUM_OF_PROVINCES);
            CFG.toastM.setTimeInView(3500);
            CFG.menus.rebuildMenu_InGame_Infobox(CFG.lang.get("LoanProposalRejected"), this.iOnCivID, CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), Images.infoDiplomacy);
            this.setVisibleM(false);
            return;
        }
