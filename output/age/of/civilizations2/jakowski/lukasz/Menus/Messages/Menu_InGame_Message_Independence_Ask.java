...
public class Menu_InGame_Message_Independence_Ask
extends Menu {
    ...
    public Menu_InGame_Message_Independence_Ask() {
        ...
        this.initMenu(new TitleM(CFG.lang.get("GuaranteeIndependence"), CFG.BUTTON_H * 3 / 5, true, true), CFG.CIV_INFO_MENU_WIDTH + CFG.PADD * 2, tempMenuPosY, tempWidth, ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD + tempMenuPosY > CFG.GAMEHEIGHT - CFG.BUTTON_H - CFG.PADD * 2 ? Math.max(CFG.GAMEHEIGHT - CFG.BUTTON_H - CFG.PADD * 2 - tempMenuPosY, (CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 2) * 6) : ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, menuElements, false, true);
        this.updateLang();
    }

    public Menu_InGame_Message_Independence_Ask(int onCivID, int iMessageID, int iValue) {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        this.iOnCivID = onCivID;
        this.iMessageID = iMessageID;
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        menuElements.add(new Button_Diplomacy_Message_Pact(Images.diploGuaranteeGives, CFG.lang.get("CivAWantsToGuaranteeOurIndependence", CFG.core.getCiv(this.iOnCivID).getCivName()), this.iOnCivID, CFG.lang.get("TurnsX", iValue), 2, tY, CFG.BUTTON_W * 2){

            @Override
            public int getWidthE() {
                return Menu_InGame_Message_Independence_Ask.this.getElementW() * 2;
            }
        });
        tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
        menuElements.add(new Button_InGameAction(CFG.lang.get("Refuse"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_Message_Independence_Ask.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }
        ...
        menuElements.add(new Button_InGameAction(CFG.lang.get("Accept"), -1, 2, tY, CFG.BUTTON_W, true){
            ...
            @Override
            public int getPosXE() {
                return Menu_InGame_Message_Independence_Ask.this.getElementW() + CFG.PADD / 2;
            }
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("GuaranteeIndependence"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                IMGManager.getIMG(Images.diploMessage).drawO(oSB, Menu_InGame_Message_Independence_Ask.this.getPosX() + CFG.PADD * 2 + iTranslateX, Menu_InGame_Message_Independence_Ask.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(Images.diploMessage).getHeight() / 2);
