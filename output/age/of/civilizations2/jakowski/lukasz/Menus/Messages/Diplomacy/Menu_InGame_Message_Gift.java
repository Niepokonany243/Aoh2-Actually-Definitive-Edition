 ...
 */
package age.of.civilizations2.jakowski.lukasz.Menus.Messages.Diplomacy;

import age.of.civilizations2.jakowski.lukasz.Button.Diplomacy.Message.Button_Diplomacy_Message_Gift;
...
public class Menu_InGame_Message_Gift
extends Menu {
    ...
    public Menu_InGame_Message_Gift() {
        ...
        this.initMenu(new TitleM(CFG.lang.get("NonAggressionPact"), CFG.BUTTON_H * 3 / 5, true, true), CFG.CIV_INFO_MENU_WIDTH + CFG.PADD * 2, tempMenuPosY, tempWidth, ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD + tempMenuPosY > CFG.GAMEHEIGHT - CFG.BUTTON_H - CFG.PADD * 2 ? Math.max(CFG.GAMEHEIGHT - CFG.BUTTON_H - CFG.PADD * 2 - tempMenuPosY, (CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 2) * 6) : ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, menuElements, false, true);
        this.updateLang();
    }

    public Menu_InGame_Message_Gift(int onCivID, int iMessageID, long iValue) {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        this.iOnCivID = onCivID;
        this.iMessageID = iMessageID;
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        menuElements.add(new Button_Diplomacy_Message_Gift(CFG.lang.get("AGiftFromCivA", CFG.core.getCiv(this.iOnCivID).getCivName()), this.iOnCivID, CFG.getNumberWthSpaces("" + iValue), 2, tY, CFG.BUTTON_W * 2){

            @Override
            public int getWidthE() {
                return Menu_InGame_Message_Gift.this.getElementW() * 2;
            }
        });
        tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
        menuElements.add(new Button_InGameAction(CFG.lang.get("Refuse"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_Message_Gift.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }
            ...
            public void buildElemHover() {
                ...
                nData.add(new ME_Hover_2Type_Flag_Big(Menu_InGame_Message_Gift.this.iOnCivID));
                nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("SendGidtBack"), CFG.COLOR_HOVER_TITLE));
                nElements.add(new MEHover_2E(nData));
                nData.clear();
                nData.add(new ME_Hover_2Type_Text(CFG.lang.get("Relation") + ": "));
                nData.add(new ME_Hover_2Type_Text("" + GameValues.gvGift.GIFT_REFUSE_OPINION_CHANGE, CFG.COLOR_NEGATIVE_2));
                nData.add(new ME_Hover_2Type_Flag(Menu_InGame_Message_Gift.this.iOnCivID, CFG.PADD, 0));
        ...
        menuElements.add(new Button_InGameAction(CFG.lang.get("Accept"), -1, 2, tY, CFG.BUTTON_W, true){
            ...
            @Override
            public int getPosXE() {
                return Menu_InGame_Message_Gift.this.getElementW() + CFG.PADD / 2;
            }
            ...
            public void buildElemHover() {
                ArrayList<MEHover_2E> nElements = new ArrayList<MEHover_2E>();
                ArrayList<ME_Hover_2Type> nData = new ArrayList<ME_Hover_2Type>();
                nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("Thanks") + "!", CFG.COLOR_HOVER_TITLE));
                nData.add(new ME_Hover_2Type_Flag_Big(Menu_InGame_Message_Gift.this.iOnCivID, CFG.PADD, 0));
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("Gift"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                IMGManager.getIMG(Images.diploMessage).drawO(oSB, Menu_InGame_Message_Gift.this.getPosX() + CFG.PADD * 2 + iTranslateX, Menu_InGame_Message_Gift.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(Images.diploMessage).getHeight() / 2);
    ...
    public final void actionEL(int iID) {
        ...
        if (iID == this.getMenuElemsSize() - 2) {
            ...
            CFG.toastM.addM(CFG.lang.get("GiftSentBack"), CFG.COLOR_TEXT_NUM_OF_PROVINCES);
            CFG.toastM.setTimeInView(3500);
            this.setVisibleM(false);
            return;
        }
