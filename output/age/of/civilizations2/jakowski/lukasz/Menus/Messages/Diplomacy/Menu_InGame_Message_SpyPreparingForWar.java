...
public class Menu_InGame_Message_SpyPreparingForWar
extends Menu {
    public static long lTime = 0L;

    public Menu_InGame_Message_SpyPreparingForWar(int onCivID) {
        ...
        menuElements.add(new ButtonN_Civs2(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), onCivID, 2, tY, tempWidth - 4){
            ...
            public int getWidthE() {
                return Menu_InGame_Message_SpyPreparingForWar.this.getElementW() * 2;
            }
        });
        ((MenuElemUI)menuElements.get(menuElements.size() - 1)).setCurr(1);
        menuElements.add(new ButtonN_FlagIcon_3_TextRight(onCivID, Images.diploWar, 0, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE(), CFG.BUTTON_W * 2, CFG.getNumberWthSpaces("" + CFG.core.getCiv(onCivID).getNumOfProvs()), Images.provinces, CFG.COLOR_TEXT_NUM_OF_PROVINCES, CFG.getNumberWthSpaces("" + CFG.core.getCiv(onCivID).countPop()), Images.pop, CFG.COLOR_POPULATION){
        ...
        menuElements.add(new Text_Desc(CFG.lang.get("SpyPreparingForWarDesc") + " " + CFG.lang.get("SpyPreparingForWarDesc2"), 2, tY += CFG.PADD, tempWidth - 4){
        ...
        menuElements.add(new Button_InGameAction_War(CFG.lang.get("Close"), -1, CFG.PADD, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, CFG.BUTTON_W, true){

            @Override
            public int getWidthE() {
                return Menu_InGame_Message_SpyPreparingForWar.this.getElementW() * 2 - CFG.PADD * 2;
            }

            @Override
            public void actionElem(int iID) {
                Menu_InGame_Message_SpyPreparingForWar.this.setVisibleM(false);
            }
            ...
            public void buildElemHover() {
                ArrayList<MEHover_2E> nElements = new ArrayList<MEHover_2E>();
                ArrayList<ME_Hover_2Type> nData = new ArrayList<ME_Hover_2Type>();
                nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("Close"), CFG.COLOR_HOVER_TITLE));
                nElements.add(new MEHover_2E(nData));
                nData.clear();
                this.menuElemHover = new ME_Hover_v2(nElements);
            }
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("MobilizationForWar"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                int imgID = Images.spy;
                IMGManager.getIMG(imgID).drawO(oSB, nPosX + (nWidth - (this.getTextWidth() + IMGManager.getIMG(imgID).getWidth() + CFG.PADD)) / 2 + iTranslateX, Menu_InGame_Message_SpyPreparingForWar.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(imgID).getHeight() / 2);
