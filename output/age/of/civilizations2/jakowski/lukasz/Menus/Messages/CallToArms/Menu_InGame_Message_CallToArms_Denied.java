 ...
 */
package age.of.civilizations2.jakowski.lukasz.Menus.Messages.CallToArms;

import age.of.civilizations2.jakowski.lukasz.Button.Diplomacy.Message.Button_Diplomacy_Message_JoinAWar_Denied;
...
public class Menu_InGame_Message_CallToArms_Denied
extends Menu {
    ...
    public Menu_InGame_Message_CallToArms_Denied(int onCivID, int iMessageID, int iValue) {
        ...
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        menuElements.add(new Button_Diplomacy_Message_JoinAWar_Denied(CFG.lang.get("XDeniedToJoinTheWar", CFG.core.getCiv(this.iOnCivID).getCivName()) + ":", this.iOnCivID, iValue, CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), 2, tY, CFG.BUTTON_W * 2){
        ...
        tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
        menuElements.add(new Button_InGameAction(CFG.lang.get("SendAnInsult"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){

            @Override
            public int getWidthE() {
                return Menu_InGame_Message_CallToArms_Denied.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }
            ...
            public void buildElemHover() {
                ...
                nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("Relation") + ": "));
                nData.add(new ME_Hover_2Type_Text_Big("" + GameValues.gvDipCallToArms.CALL_TO_ARMS_DENY_INSULT_RELATION_CHANGE, CFG.COLOR_NEGATIVE_2));
                nData.add(new ME_Hover_2Type_Flag_Big(Menu_InGame_Message_CallToArms_Denied.this.iOnCivID, CFG.PADD, 0));
                nData.add(new ME_Hover_2Type_Image_Big(Images.diploRelations, CFG.PADD, CFG.PADD));
                nData.add(new ME_Hover_2Type_Flag_Big(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), 0, 0));
        ...
        menuElements.add(new Button_InGameAction(CFG.lang.get("Fine") + "!", -1, 2, tY, CFG.BUTTON_W, true){

            @Override
            public int getPosXE() {
                return Menu_InGame_Message_CallToArms_Denied.this.getElementW() + CFG.PADD / 2;
            }
        ...
        this.initMenu(new TitleM(CFG.lang.get("CallToArms"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                IMGManager.getIMG(Images.diploMessage).drawO(oSB, Menu_InGame_Message_CallToArms_Denied.this.getPosX() + CFG.PADD * 2 + iTranslateX, Menu_InGame_Message_CallToArms_Denied.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(Images.diploMessage).getHeight() / 2);
    ...
    public void draw(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean sliderMenuIsActive) {
        if (this.lTime + (long)this.ANIMATION_TIME >= System.currentTimeMillis()) {
            Rectangle clipBounds = new Rectangle(this.getPosX() - 2, CFG.GAMEHEIGHT - this.getPosY(), this.getWidthM() + 4, -((int)((float)(this.getHeightM() + CFG.PADD) * ((float)(System.currentTimeMillis() - this.lTime) / (float)this.ANIMATION_TIME))));
            oSB.flush();
