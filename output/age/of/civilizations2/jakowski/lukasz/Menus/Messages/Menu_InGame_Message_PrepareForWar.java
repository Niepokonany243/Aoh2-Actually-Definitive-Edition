...
public class Menu_InGame_Message_PrepareForWar
extends Menu {
    public int ANIMATION_TIME = 200;
    public long lTime = 0L;
    public int iWarAgainstCivID = -1;
    private int iMessageID = 0;

    public Menu_InGame_Message_PrepareForWar() {
        ...
        this.initMenu(new TitleM(CFG.lang.get("PrepareForWar"), CFG.BUTTON_H * 3 / 5, true, true), CFG.CIV_INFO_MENU_WIDTH + CFG.PADD * 2, tempMenuPosY, tempWidth, ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD + tempMenuPosY > CFG.GAMEHEIGHT - CFG.BUTTON_H - CFG.PADD * 2 ? Math.max(CFG.GAMEHEIGHT - CFG.BUTTON_H - CFG.PADD * 2 - tempMenuPosY, (CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 2) * 6) : ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getPosY() + ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, menuElements, false, true);
        this.updateLang();
    }

    public Menu_InGame_Message_PrepareForWar(int onCivID, int iMessageID, int warAgainstCivID, int numOfTurns) {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        this.iWarAgainstCivID = warAgainstCivID;
        this.iMessageID = iMessageID;
        int tempWidth = CFG.CIV_INFO_MENU_WIDTH * 2;
        int tY = 0;
        menuElements.add(new Button_PrepareForWar(CFG.lang.get("PrepareForTheWarAgainst") + ": ", onCivID, warAgainstCivID, numOfTurns, 0, tY, CFG.BUTTON_W * 2){
            ...
            public void actionElem(int iID) {
                if (CFG.core.getCiv(Menu_InGame_Message_PrepareForWar.this.iWarAgainstCivID).getCapitalProvID() >= 0) {
                    CFG.core.setActiveProvID(CFG.core.getCiv(Menu_InGame_Message_PrepareForWar.this.iWarAgainstCivID).getCapitalProvID());
                    CFG.map.getMpC().centerToProvID(CFG.core.getActiveProvID());
                    CFG.toastM.addM(CFG.core.getCiv(Menu_InGame_Message_PrepareForWar.this.iWarAgainstCivID).getCivName(), CFG.COLOR_NEGATIVE_2);
                }
            }
        });
        tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE();
        menuElements.add(new Button_InGameAction(CFG.lang.get("Deny"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            @Override
            public int getWidthE() {
                return Menu_InGame_Message_PrepareForWar.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }
            ...
            public void buildElemHover() {
                ...
                nData.add(new ME_Hover_2Type_Text(CFG.lang.get("DenyToJoinTheWar"), CFG.COLOR_NEGATIVE_2));
        ...
        menuElements.add(new Button_InGameAction(CFG.lang.get("Confirm"), -1, 2, tY, CFG.BUTTON_W, true){
            ...
            @Override
            public int getPosXE() {
                return Menu_InGame_Message_PrepareForWar.this.getElementW() + CFG.PADD / 2;
            }
            ...
            public void buildElemHover() {
                ArrayList<MEHover_2E> nElements = new ArrayList<MEHover_2E>();
                ArrayList<ME_Hover_2Type> nData = new ArrayList<ME_Hover_2Type>();
                nData.add(new ME_Hover_2Type_Text(CFG.lang.get("PrepareForWar"), CFG.COLOR_HOVER_TITLE));
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("PrepareForWar"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                IMGManager.getIMG(Images.diploMessage).drawO(oSB, Menu_InGame_Message_PrepareForWar.this.getPosX() + CFG.PADD * 2 + iTranslateX, Menu_InGame_Message_PrepareForWar.this.getPosY() - this.getHeightT() / 2 - IMGManager.getIMG(Images.diploMessage).getHeight() / 2);
    ...
    public final void actionEL(int iID) {
        ...
        if (iID == this.getMenuElemsSize() - 2) {
            ...
            CFG.toastM.addM(CFG.lang.get("Refused") + "!", CFG.COLOR_POSITIVE);
            CFG.toastM.setTimeInView(3500);
            this.setVisibleM(false);
            return;
        }
