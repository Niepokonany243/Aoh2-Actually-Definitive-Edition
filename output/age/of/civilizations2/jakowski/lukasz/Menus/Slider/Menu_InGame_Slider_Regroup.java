            ...
            try {
                ...
                if (CFG.chosenProvinces_Regroup.size() > 1) {
                    ...
                    for (int a = 0; a < CFG.chosenProvinces_Regroup.size(); ++a) {
                        RegroupArmy nRegroup = new RegroupArmy(moveCivID, moveFromProvinceID, CFG.chosenProvinces_Regroup.get(a));
                        if (nRegroup.getRouteSize() <= 0) continue;
                        regroupData.add(nRegroup);
                    }
                    if (!regroupData.isEmpty() && (armyPerMove = (long)Math.ceil((float)this.getMenuElem(2).getCurr() / (float)regroupData.size())) > 1L) {
                        for (int a = 0; a < regroupData.size(); ++a) {
                            if (((RegroupArmy)regroupData.get(a)).getRouteSize() == 1) {
                                int toProv = ((RegroupArmy)regroupData.get(a)).getToProvinceID();
                                if (CFG.gameAction.moveArmyAction(moveFromProvinceID, toProv, armyPerMove, moveCivID, true, true)) {
                                    armyMoved = true;
                                }
                                continue;
                            }
                            if (!CFG.gameAction.moveArmyAction(moveFromProvinceID, ((RegroupArmy)regroupData.get(a)).getRoute(0), armyPerMove, moveCivID, true, true)) continue;
                            ...
                            CFG.core.getCiv(moveCivID).addRegroupArmy((RegroupArmy)regroupData.get(a));
                            armyMoved = true;
                        }
                    }
                }
                ...
                if (!armyMoved) {
                    if (CFG.core.currentRegroupArmy.getRouteSize() == 1) {
                        CFG.gameAction.moveArmyAction(CFG.core.getActiveProvID(), CFG.chosenProvinceID, this.getMenuElem(2).getCurr(), moveCivID, true, true);
                    } else if (CFG.gameAction.moveArmyAction(CFG.core.getActiveProvID(), CFG.core.currentRegroupArmy.getRoute(0), this.getMenuElem(2).getCurr(), moveCivID, true, true)) {
