    ...
    public Menu_InGame_WorldEconomyContinent() {
        ...
        for (i3 = 0; i3 < CFG.core.getProvinSize(); ++i3) {
            ...
            for (j = 0; j < tCivilizations.size(); ++j) {
                if (tCivilizations.get(j).intValue() != CFG.core.getProv(i3).getCivId()) continue;
                tAdd2 = false;
                break;
            }
            if (!tAdd2) continue;
        ...
        for (i3 = 0; i3 < CFG.core.getProvinSize(); ++i3) {
            ...
            tEconomy.set(tCivID, tEconomy.get(tCivID) + CFG.core.getProv(i3).getEco());
            tProvinces.set(tCivID, (Integer)tProvinces.get(tCivID) + 1);
            if ((long)CFG.core.getProv(tLargestCity.get(tCivID)).getEco() >= (long)CFG.core.getProv(i3).getEco()) continue;
        ...
        for (int i4 = 0; i4 < tCivilizations.size(); ++i4) {
            tTotalEco += tEconomy.get(i4).longValue();
            tProvincesTotal += ((Integer)tProvinces.get(i4)).intValue();
            if (tLargestCityTotal < 0) {
                if (tLargestCity.get(i4) < 0) continue;
                tLargestCityTotal = tLargestCity.get(i4);
                continue;
            }
            if (tLargestCity.get(i4) < 0 || (long)CFG.core.getProv(tLargestCityTotal).getEco() >= (long)CFG.core.getProv(tLargestCity.get(i4)).getEco()) continue;
            tLargestCityTotal = tLargestCity.get(i4);
        }
