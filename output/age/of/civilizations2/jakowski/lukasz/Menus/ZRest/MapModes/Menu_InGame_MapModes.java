...
public class Menu_InGame_MapModes
extends Menu {
    ...
    public Menu_InGame_MapModes() {
        ...
        menuElements.add(new Button_Opt_NS_MapModes_R2(-2, null, CFG.PADD * 2, 2, 0, CFG.BUTTON_W - CFG.PADD * 4, tempElemH, true, true, 0){
            ...
            public String getTextToDrawElem() {
                return Menu_InGame_MapModes.this.nSearch + ": " + searchText;
            }
        });
        for (i = 1; i < 51; ++i) {
            if (i == 48) {
                if (i % 2 == 0) {
                    menuElements.add(new Button_Opt_MapModes_R(-2, null, -1, 2, tempElemH * i, CFG.BUTTON_W - CFG.PADD * 4, tempElemH, true, true, i){
                ...
                menuElements.add(new Button_Opt_NS_MapModes_R2(-2, null, -1, 2, tempElemH * i, CFG.BUTTON_W - CFG.PADD * 4, tempElemH, true, true, i){
        ...
        }, -1, -1, CFG.BUTTON_W * 2 + CFG.BUTTON_W * 3 / 5 + Core.PADDING * 2, Math.min(Math.min(tempElemH * 6 + tempElemH / 2, CFG.GAMEHEIGHT - CFG.BUTTON_H), menuElements.size() * tempElemH), menuElements, false, true);
        this.updateLang();
        if (searchText.length() > 0) {
            int tY = this.getMenuElem(0).getHeightE();
            for (i = 1; i < this.getMenuElemsSize(); ++i) {
                if (this.getMenuElem(i).getTextE().toLowerCase().contains(searchText.toLowerCase())) {
                    this.getMenuElem(i).setPosY(tY);
                    tY += this.getMenuElem(i).getHeightE();
                    continue;
                }
    ...
    public void updateLang() {
        ...
        try {
            ...
            this.getMenuElem(id++).setText2("" + Images.topGold());
            this.getMenuElem(id).setTextE("Age of History Fans");
            this.getMenuElem(id).setCurr(MapModesManager.VIEW_FANS_MODE);
            this.getMenuElem(id++).setText2("" + Images.peace);
            this.getMenuElem(id).setTextE("Age of History Haters");
            this.getMenuElem(id).setCurr(MapModesManager.VIEW_HATERS_MODE);
            this.getMenuElem(id++).setText2("" + Images.diploRevolution);
        }
        catch (Exception ex) {
