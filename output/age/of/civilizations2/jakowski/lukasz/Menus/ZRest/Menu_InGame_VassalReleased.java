...
public class Menu_InGame_VassalReleased
extends Menu {
    ...
    public Menu_InGame_VassalReleased(int nCivID) {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        menuElements.add(new Button_Transparent(0, 0, 1, 1, false));
        this.initMenu(null, 0, 0, CFG.GAMEWIDTH, CFG.GAMEHEIGHT, menuElements);
        this.iCivA = nCivID;
        this.sWar = CFG.lang.get("Vassal") + ": " + CFG.core.getCiv(nCivID).getCivName();
        CFG.glyphLay.setText(CFG.fontMain.get(0), this.sWar);
        this.iWarWidth = (int)(CFG.glyphLay.width * 0.8f);
        this.sDate = GameCalendar.getCurrDate();
        CFG.glyphLay.setText(CFG.fontMain.get(0), this.sDate);
        this.iDateWidth = (int)(CFG.glyphLay.width * 0.7f);
    ...
    public void draw(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean sliderMenuIsActive) {
        ...
        CFG.drawTextDefault(oSB, this.sWar, this.getPosX2() + this.getWidth2() / 2 - (this.iWarWidth + (int)((float)IMGManager.getIMG(Images.flagRectSmall).getWidth() * this.getImageScale(Images.flagRectSmall)) + CFG.PADD) / 2 + (int)((float)IMGManager.getIMG(Images.flagRectSmall).getWidth() * this.getImageScale(Images.flagRectSmall)) + CFG.PADD + iTranslateX, this.getPosY2() + CFG.PADD * 2 + (int)(((float)CFG.TEXT_HEIGHT_DEFAULT - (float)CFG.TEXT_HEIGHT_DEFAULT * 0.8f) / 2.0f) + iTranslateY, new Color(CFG.COLOR_NEUTRAL2.r, CFG.COLOR_NEUTRAL2.g, CFG.COLOR_NEUTRAL2.b, 1.0f * tAlpha));
        CFG.fontMain.get(0).getData().setScale(0.7f);
        CFG.drawTextDefault(oSB, this.sDate, this.getPosX2() + this.getWidth2() / 2 - this.iDateWidth / 2 + iTranslateX, this.getPosY2() + this.getHeight2() / 2 + CFG.PADD + (int)(((float)CFG.TEXT_HEIGHT_DEFAULT - (float)CFG.TEXT_HEIGHT_DEFAULT * 0.7f) / 2.0f) + iTranslateY, new Color(CFG.COLOR_NEUTRAL.r, CFG.COLOR_NEUTRAL.g, CFG.COLOR_NEUTRAL.b, CFG.COLOR_TEXT_NUM_OF_PROVINCES.a * tAlpha));
