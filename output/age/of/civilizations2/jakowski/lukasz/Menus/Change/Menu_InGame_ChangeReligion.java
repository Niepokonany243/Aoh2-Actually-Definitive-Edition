    ...
    public Menu_InGame_ChangeReligion() {
        ...
        menuElements.add(new Button_InGameAction(CFG.lang.get("Cancel"), -1, 2 + CFG.PADD, tY += CFG.PADD, CFG.BUTTON_W, true){
            ...
            public int getWidthE() {
                return Menu_InGame_ChangeReligion.this.getElementW() - CFG.PADD - CFG.PADD / 2;
            }

            @Override
            public void actionElem(int iID) {
                Menu_InGame_ChangeReligion.this.setVisibleM(false);
            }
        });
        menuElements.add(new Button_InGameAction(CFG.lang.get("Confirm"), -1, 2, tY, CFG.BUTTON_W, true){
            ...
            @Override
            public int getPosXE() {
                return Menu_InGame_ChangeReligion.this.getElementW() + CFG.PADD / 2;
            }
            ...
            public void buildElemHover() {
                ...
                ArrayList<ME_Hover_2Type> nData = new ArrayList<ME_Hover_2Type>();
                int toReligionID = -1;
                for (int i = 1; i < Menu_InGame_ChangeReligion.this.getMenuElemsSize() - 2; ++i) {
                    if (!Menu_InGame_ChangeReligion.this.getMenuElem(i).getCheckboxSt()) continue;
                    toReligionID = Menu_InGame_ChangeReligion.this.getMenuElem(i).getCurr();
                    break;
                }
                nData.add(new ME_Hover_2Type_Flag_Big(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()));
                nData.add(new ME_Hover_2Type_Text_Big(CFG.lang.get("ChangeReligion") + (toReligionID >= 0 ? ": " : ""), toReligionID >= 0 ? new Color(0.9843137f, 0.9843137f, 0.9843137f, 1.0f) : CFG.COLOR_TEXT_NUM_OF_PROVINCES));
                if (toReligionID >= 0) {
                    nData.add(new ME_Hover_2Type_Text_Big(CFG.religionManager.getReligion(toReligionID).getName(), CFG.COLOR_TEXT_NUM_OF_PROVINCES));
                    nData.add(new ME_Hover_2Type_Religion_Big(toReligionID, CFG.PADD, 0));
                }
                nElements.add(new MEHover_2E(nData));
                nData.clear();
                ...
                nData.clear();
                if (toReligionID >= 0) {
                    nData.add(new ME_Hover_2Type_Text(CFG.religionManager.getReligion(toReligionID).getName()));
                    nData.add(new ME_Hover_2Type_Religion(toReligionID, CFG.PADD, 0));
                    nElements.add(new MEHover_2E(nData));
                    nData.clear();
                }
                this.menuElemHover = new ME_Hover_v2(nElements);
            ...
            public void actionElem(int iID) {
                int i;
                int toReligionID = -1;
                for (i = 1; i < Menu_InGame_ChangeReligion.this.getMenuElemsSize(); ++i) {
                    if (!Menu_InGame_ChangeReligion.this.getMenuElem(i).getCheckboxSt()) continue;
                    toReligionID = Menu_InGame_ChangeReligion.this.getMenuElem(i).getCurr();
                    break;
                }
                if (toReligionID >= 0) {
                    if (GameManager.changeReligion(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId(), toReligionID)) {
                        ...
                        tColor.add(new Color(Color.WHITE));
                        tMess.add(CFG.religionManager.getReligion(CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getReligionID()).getName());
                        tColor.add(CFG.COLOR_HOVER_TITLE);
                        CFG.menus.rebuildMenu_InGame_Infobox(CFG.lang.get("Religion") + ": " + CFG.religionManager.getReligion(CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getReligionID()).getName(), GameCalendar.getCurrDate(), Images.infoDiplomacy);
            ...
            public boolean getIsClickable() {
                return CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getGold() >= (long)IdeologiesManager.getChangeReligionCost(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId());
            }
        ...
        menuElements.add(new TextBuildTitle(CFG.lang.get("Total") + ": " + CFG.religionManager.getReligionsSize(), -1, 0, tY += ((MenuElemUI)menuElements.get(menuElements.size() - 1)).getHeightE() + CFG.PADD, tempWidth, CFG.TEXT_HEIGHT_DEFAULT + CFG.PADD * 4){
        ...
        for (int i = 0; i < CFG.religionManager.getReligionsSize(); ++i) {
            menuElements.add(new Button_Diplomacy_Action_Religion(i, CFG.religionManager.getReligion(i).getName(), 0, 0, tY, tempWidth - 2, tempElemH, true, true){

                @Override
                public void actionElem(int iID) {
                    for (int i = 0; i < Menu_InGame_ChangeReligion.this.getMenuElemsSize(); ++i) {
                        Menu_InGame_ChangeReligion.this.getMenuElem(i).setCheckboxSt(false);
                    }
                    Menu_InGame_ChangeReligion.this.getMenuElem(iID).setCheckboxSt(!Menu_InGame_ChangeReligion.this.getMenuElem(iID).getCheckboxSt());
                }
        ...
        this.initMenu(new TitleM_TextSmall(CFG.lang.get("ChangeReligion"), CFG.BUTTON_H * 3 / 4, true, true){
            ...
            public void drawT(SpriteBatch oSB, int iTranslateX, int nPosX, int nPosY, int nWidth, boolean sliderMenuIsActive) {
                ...
                CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getFlagC().drawO(oSB, Menu_InGame_ChangeReligion.this.getPosX() + CFG.PADD * 2 + iTranslateX, Menu_InGame_ChangeReligion.this.getPosY() - this.getHeightT() / 2 - CFG.CIV_FLAG_HEIGHT / 2 - CFG.core.getCiv(CFG.core.getPlayer(CFG.PLAYER_TURN_ID).getCivId()).getFlagC().getHeight(), CFG.CIV_FLAG_WIDTH, CFG.CIV_FLAG_HEIGHT);
                IMGManager.getIMG(Images.flagRectSmall).drawO(oSB, Menu_InGame_ChangeReligion.this.getPosX() + CFG.PADD * 2 + iTranslateX, Menu_InGame_ChangeReligion.this.getPosY() - this.getHeightT() / 2 - CFG.CIV_FLAG_HEIGHT / 2);
