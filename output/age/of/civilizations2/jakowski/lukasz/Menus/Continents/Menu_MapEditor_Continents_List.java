...
public class Menu_MapEditor_Continents_List
extends Menu {
    public Menu_MapEditor_Continents_List() {
        ArrayList<MenuElemUI> menuElements = new ArrayList<MenuElemUI>();
        int tempWidth = (CFG.GAMEWIDTH - CFG.PADD * 2 - CFG.BUTTON_W * 2 - CFG.PADD * 2 - CFG.PADD * (CFG.map.getMapContinents().getContinentsSize() - 1)) / CFG.map.getMapContinents().getContinentsSize();
        if (tempWidth < CFG.BUTTON_W) {
            tempWidth = CFG.BUTTON_W;
        }
        for (int i = 0; i < CFG.map.getMapContinents().getContinentsSize(); ++i) {
            menuElements.add(new Button_Game_Checkbox(CFG.map.getMapContinents().getName(i), -1, CFG.PADD + tempWidth * i + CFG.PADD * i, CFG.PADD, tempWidth, true, false){
                int iCurrent;
                {
                    this.iCurrent = 0;
                }
                ...
                public boolean getCheckboxSt() {
                    return Editor_Continents.iActiveContinentID == this.getCurr();
                }
                ...
                public void buildElemHover() {
                    try {
                        ...
                        nData.add(new ME_Hover_2Type_Text(CFG.getNumberWthSpaces("" + CFG.core.countContinentProvinces(this.getCurr())), CFG.COLOR_HOVER_TITLE));
                        nElements.add(new MEHover_2E(nData));
                        nData.clear();
    ...
    public void draw(SpriteBatch oSB, int iTranslateX, int iTranslateY, boolean sliderMenuIsActive) {
        ...
        for (int i = 0; i < this.getMenuElemsSize(); ++i) {
            ...
            IMGManager.getIMG(Images.sliderGradient).drawO(oSB, this.getMenuPosX() + this.getMenuElem(i).getTextWidthU() - CFG.PADD + this.getMenuElem(i).getPosXE() + this.getMenuElem(i).getWidthE() / 2 - this.getMenuElem(i).getTextWidthU() / 2 + iTranslateX, this.getMenuElem(i).getPosY() + this.getMenuPosY() + this.getMenuElem(i).getHeightE() / 2 + this.getMenuElem(i).getTextHeight() / 2 + CFG.PADD + iTranslateY, CFG.PADD, CFG.CIV_COLOR_W);
