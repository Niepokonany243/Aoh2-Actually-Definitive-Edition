...
implements Disposable {
    ...
    public FreeTypeFontGenerator(FileHandle fontFile, int faceIndex) {
        ...
        this.face = this.library.newFace(fontFile, faceIndex);
    ...
    public BitmapFont generateFont(FreeTypeFontParameter parameter, FreeTypeBitmapFontData data) {
        boolean updateTextureRegions;
        boolean bl = updateTextureRegions = data.regions == null && parameter.packer != null;
        ...
        BitmapFont font = this.newBitmapFont(data, data.regions, true);
    ...
    public FreeTypeBitmapFontData generateData(FreeTypeFontParameter parameter, FreeTypeBitmapFontData data) {
        ...
        data.name = this.name + "-" + parameter.size;
        ...
        data.spaceXadvance = this.loadChar(32, flags) || this.loadChar(108, flags) ? (float)FreeType.toInt(this.face.getGlyph().getMetrics().getHoriAdvance()) : (float)this.face.getMaxAdvanceWidth();
        ...
        for (char capChar : data.capChars) {
            ...
            data.capHeight = FreeType.toInt(this.face.getGlyph().getMetrics().getHeight()) + Math.abs(parameter.shadowOffsetY);
            break;
        }
        ...
        for (int i = 0; i < charactersLength; ++i) {
            ...
            int height;
            char c = characters[i];
            heights[i] = height = this.loadChar(c, flags) ? FreeType.toInt(this.face.getGlyph().getMetrics().getHeight()) : 0;
            if (c != '\u0000' || (missingGlyph = this.createGlyph('\u0000', data, parameter, stroker, baseLine, packer)) == null || missingGlyph.width == 0 || missingGlyph.height == 0) continue;
        ...
        while (heightsCount > 0) {
            ...
            if (data.getGlyph(c) == null && (glyph = this.createGlyph(c, data, parameter, stroker, baseLine, packer)) != null) {
    ...
    @Null
    protected BitmapFont.Glyph createGlyph(char c, FreeTypeBitmapFontData data, FreeTypeFontParameter parameter, FreeType.Stroker stroker, float baseLine, PixmapPacker packer) {
        boolean missing;
        ...
        if (mainBitmap.getWidth() != 0 && mainBitmap.getRows() != 0) {
            ...
            if (parameter.padTop > 0 || parameter.padLeft > 0 || parameter.padBottom > 0 || parameter.padRight > 0) {
                Pixmap padPixmap = new Pixmap(mainPixmap.getWidth() + parameter.padLeft + parameter.padRight, mainPixmap.getHeight() + parameter.padTop + parameter.padBottom, mainPixmap.getFormat());
